...
int vj_dfb_update_yuv_overlay(void *ptr , uint8_t **frame) {
	...
	for(i=0; i < framebuffer->height; i++) {
		memcpy(output+i*pitch, frame[0]+i*framebuffer->width, p);
	}

	output += pitch * framebuffer->height;
	p = p / 2;
	for(i=0; i < framebuffer->height/2; i++) {
		memcpy(output+i*pitch/2, frame[1] + i * framebuffer->width/2, p);
	}	
			
	output += pitch * framebuffer->height/4;
	for(i=0; i < framebuffer->height/2; i++) {
		memcpy(output+i * pitch/2, frame[2] + i * framebuffer->width/2, p);
	}
...
int vj_dfb_update_yuv_overlay(void *ptr, uint8_t ** frame)
{
    ...
    for (i = 0; i < framebuffer->height; i++) {
	memcpy(dst + i * pitch, frame[0] + i * framebuffer->width, p);
    }

    dst += pitch * framebuffer->height;
    p = p / 2;
    for (i = 0; i < framebuffer->height / 2; i++) {
	memcpy(dst + i * pitch / 2, frame[1] + i * framebuffer->width / 2,
	       p);
    }

    dst += pitch * framebuffer->height / 4;
    for (i = 0; i < framebuffer->height / 2; i++) {
	memcpy(dst + i * pitch / 2, frame[2] + i * framebuffer->width / 2,
	       p);
    }
