...
static int decode_thread(void *arg)
{
	...
	printf("seek requested %ld, start_time = %ld, ret=%d", (long)timestamp, (long)ic->start_time,ret);
        if (ret < 0) {
