...
static void sequence_vims_pattern_flush(void);
...
static void nav_reparent_widget(GtkWidget *child, GtkWidget *new_parent)
{
    ...
    g_object_ref(child);
    if(GTK_IS_CONTAINER(old_parent))
...
static void init_video_navigation_buttons(void)
{
    ...
    if(hbox638)
        gtk_widget_set_margin_left(hbox638, 10);
    if(GTK_IS_BOX(hbox899))
        gtk_box_set_spacing(GTK_BOX(hbox899), 0);
    if(GTK_IS_BOX(hbox900))
...
static gboolean is_fxanim_displayed(void)
{
    ...
    if(fxanim_page == NOTEBOOK18_PAGE_FX_ANIM)
        return TRUE;
    return FALSE;
}
...
gboolean   periodic_pull(gpointer data)
{
    ...
    if(pm == MODE_STREAM && deckpage == NOTEBOOK18_PAGE_SAMPLE && info->status_tokens[STREAM_TYPE] == STREAM_GENERATOR)
    {
...
static void ui_size_probe(const char *stage)
{
    ...
    veejay_msg(VEEJAY_MSG_DEBUG, "UI size probe: %s", stage ? stage : "unknown");
...
static gboolean preview_image_draw(GtkWidget *widget, cairo_t *cr, gpointer data)
{
    ...
    if(aw > 0 && ah > 0)
        gtk_render_background(style, cr, 0.0, 0.0, aw, ah);
...
static gboolean preview_is_native_resolution(void)
{
    int source_w;
    int source_h;
    ...
    source_w = preview_even(info->el.width);
    source_h = preview_even(info->el.height);

    return preview_base_w_ >= source_w &&
           preview_base_h_ >= source_h;
}

static void preview_apply_native_compact_layout(void)
{
    ...
    if(frame) {
        row = GTK_WIDGET(g_object_get_data(
            G_OBJECT(frame),
            "gvr-preview-action-row"));
        center = GTK_WIDGET(g_object_get_data(
            G_OBJECT(frame),
            "gvr-preview-action-center"));
    }
    ...
    if(compact) {
        ...
        if(alignment) {
            gtk_widget_set_size_request(
                ...
                preview_base_w_,
                preview_base_h_);
            gtk_widget_set_hexpand(alignment, FALSE);
            gtk_widget_set_vexpand(alignment, FALSE);
        ...
        if(alignment) {
            gtk_widget_set_size_request(alignment, -1, -1);
            gtk_widget_set_hexpand(alignment, TRUE);
            gtk_widget_set_vexpand(alignment, TRUE);
            gtk_widget_set_halign(alignment, GTK_ALIGN_FILL);
            gtk_widget_set_valign(alignment, GTK_ALIGN_FILL);
        ...
        if(center) {
            ...
            gtk_widget_set_valign(center, GTK_ALIGN_FILL);
        }
        ...
        if(frame) {
            ...
            gtk_widget_set_valign(frame, GTK_ALIGN_FILL);
            remove_class(frame, "preview-native-compact");
...
static void preview_set_widget_minimum(void)
{
    ...
    preview_apply_native_compact_layout();
}
...
void msg_vims(char *message)
{
    ...
    int message_id = sequence_vims_message_id(message);
    sequence_vims_note_outgoing_id(message_id);
    if(message_id == VIMS_SAMPLE_SAVE_SAMPLELIST ||
       message_id == VIMS_SAMPLE_SYNC_SAMPLELIST)
...
static void multi_vims(int id, const char format[],...)
{
    ...
    sequence_vims_note_outgoing_id(id);

    if(id == VIMS_SAMPLE_SAVE_SAMPLELIST ||
       id == VIMS_SAMPLE_SYNC_SAMPLELIST)
        sequence_vims_pattern_flush();
    va_start(args, format);
    vsnprintf(tmp, sizeof(tmp), format, args );
...
static gboolean sequence_vims_replaying = FALSE;
...
static gboolean sequence_vims_id_in_list(int id, const int *ids, size_t count)
{
...
static const int sequence_vims_query_ids[] = {
    VIMS_STATUS_TOKENS,
    VIMS_SAMPLE_LIST,
    VIMS_STREAM_LIST,
    VIMS_TRACK_LIST,
...
static const int sequence_vims_sequencer_ids[] = {
    VIMS_SEQUENCE_ADD,
    VIMS_SEQUENCE_DEL,
    VIMS_SEQUENCE_COPY,
    VIMS_SEQUENCE_QUEUE,
...
static const int sequence_vims_automation_ids[] = {
    VIMS_MACRO,
...
static const int sequence_vims_source_switch_ids[] = {
    VIMS_SAMPLE_SELECT,
    VIMS_STREAM_SELECT,
...
static const int sequence_vims_lifecycle_ids[] = {
    VIMS_SAMPLE_NEW,
    VIMS_SAMPLE_COPY,
    VIMS_SAMPLE_DEL,
    VIMS_SAMPLE_DEL_ALL,
...
static const int sequence_vims_project_edit_ids[] = {
...
static const int sequence_vims_recording_ids[] = {
...
static const int sequence_vims_engine_config_ids[] = {
    VIMS_FULLSCREEN,
...
{
    if(id >= 400 && id < 500)
        return "reserved/query/control VIMS range 400-499";

    if(sequence_vims_id_in_list(id, sequence_vims_query_ids, G_N_ELEMENTS(sequence_vims_query_ids)))
        return "query/readback command";
    if(sequence_vims_id_in_list(id, sequence_vims_sequencer_ids, G_N_ELEMENTS(sequence_vims_sequencer_ids)))
        return "sequencer control command";
    if(sequence_vims_id_in_list(id, sequence_vims_automation_ids, G_N_ELEMENTS(sequence_vims_automation_ids)))
        return "macro or bundle automation command";
    if(sequence_vims_id_in_list(id, sequence_vims_source_switch_ids, G_N_ELEMENTS(sequence_vims_source_switch_ids)))
        return "source or playback-mode switch";
    if(sequence_vims_id_in_list(id, sequence_vims_lifecycle_ids, G_N_ELEMENTS(sequence_vims_lifecycle_ids)))
        return "sample or stream lifecycle command";
    if(sequence_vims_id_in_list(id, sequence_vims_project_edit_ids, G_N_ELEMENTS(sequence_vims_project_edit_ids)))
        return "timeline or project-structure edit";
    if(sequence_vims_id_in_list(id, sequence_vims_recording_ids, G_N_ELEMENTS(sequence_vims_recording_ids)))
        return "recording, file-output or calibration command";
    if(sequence_vims_id_in_list(id, sequence_vims_fx_structure_ids, G_N_ELEMENTS(sequence_vims_fx_structure_ids)))
        return "FX-chain structure or automation edit";
    if(sequence_vims_id_in_list(id, sequence_vims_engine_config_ids, G_N_ELEMENTS(sequence_vims_engine_config_ids)))
        return "global UI or engine configuration";

    return NULL;
}

static void sequence_vims_report_rejected(int id, const char *reason)
{
    static int last_id = -1;
    static gint64 last_time = 0;
    const gint64 now = g_get_monotonic_time();

    if(id == last_id && now - last_time < 750000)
        return;

    last_id = id;
    last_time = now;
    vj_msg(VEEJAY_MSG_INFO,
           "Pattern Learn ignored VIMS %03d (%s)",
           id,
           reason ? reason : "not pattern-safe");
}
...
typedef struct {
    ...
    gint64 total;
    int entries;
    sequence_vims_timeline_slot_t slots[MAX_SEQUENCES];
} sequence_vims_timeline_t;

static sequence_vims_timeline_t sequence_vims_timelines[VJ_SEQUENCE_BANKS];
static guint sequence_vims_pattern_store_timeout_id = 0;
static gboolean sequence_vims_pattern_loading = FALSE;
static gboolean sequence_vims_pattern_dirty = FALSE;
static unsigned int sequence_vims_pattern_backend_revision = 0;

static int sequence_vims_tick_bank = -1;
static int sequence_vims_tick_slot = -1;
static gint64 sequence_vims_tick_frame = 0;
static gboolean sequence_vims_tick_valid = FALSE;

static int sequence_vims_active_source_bank = -999;
static int sequence_vims_active_source_slot = -1;
static int sequence_vims_active_cell_bank = -1;
static int sequence_vims_active_cell_slot = -1;
static int sequence_vims_active_bank_pattern = -1;
...
static int sequence_vims_stream_length_id = -1;
static int sequence_vims_stream_length_type = -1;

static guint64 sequence_vims_source_length_key(int sample_id, int sample_type)
{
    return (((guint64)(guint32)sample_type) << 32) | (guint32)sample_id;
}

static void sequence_vims_source_length_store(int sample_id, int sample_type, int frame_count)
{
    guint64 *key;

    if(sample_id <= 0 || frame_count <= 0)
        return;

    if(!sequence_vims_source_lengths)
        sequence_vims_source_lengths = g_hash_table_new_full(g_int64_hash,
                                                              g_int64_equal,
                                                              g_free,
                                                              NULL);

    key = g_new(guint64, 1);
    *key = sequence_vims_source_length_key(sample_id, sample_type);
    g_hash_table_replace(sequence_vims_source_lengths,
                         key,
                         GINT_TO_POINTER(frame_count));
}

static int sequence_vims_source_length_lookup(int sample_id, int sample_type)
{
    guint64 key;
    gpointer value = NULL;

    if(!sequence_vims_source_lengths || sample_id <= 0)
        return 0;

    key = sequence_vims_source_length_key(sample_id, sample_type);
    if(!g_hash_table_lookup_extended(sequence_vims_source_lengths,
                                     &key,
                                     NULL,
                                     &value))
        return 0;
...
static void sequence_vims_source_lengths_clear(void)
{
    if(sequence_vims_source_lengths)
        g_hash_table_remove_all(sequence_vims_source_lengths);
}

static void sequence_vims_bind_cell(int bank, int slot, int sample_id, int sample_type)
{
    if(!info || !info->vims_pattern_view)
        return;

    gvr_vims_pattern_view_bind_cell(info->vims_pattern_view,
                                    bank,
                                    slot,
                                    sample_id,
                                    sample_type);
    sequence_vims_refresh_cell_badge(bank, slot);
}

static void sequence_vims_stream_length_cache(int frame_count)
{
    if(!info ||
       info->status_tokens[PLAY_MODE] != MODE_STREAM ||
       info->status_tokens[CURRENT_ID] <= 0) {
        sequence_vims_stream_length_frames = 0;
        sequence_vims_stream_length_id = -1;
        sequence_vims_stream_length_type = -1;
        return;
    }

    sequence_vims_stream_length_frames = MAX(0, frame_count);
    sequence_vims_stream_length_id = info->status_tokens[CURRENT_ID];
    sequence_vims_stream_length_type = info->status_tokens[STREAM_TYPE];

    sequence_vims_source_length_store(sequence_vims_stream_length_id,
                                      sequence_vims_stream_length_type,
                                      sequence_vims_stream_length_frames);
}

static int sequence_vims_stream_pattern_length(void)
{
    if(!info)
        return 0;
    ...
    if(info->status_tokens[PLAY_MODE] == MODE_STREAM &&
       sequence_vims_stream_length_id == info->status_tokens[CURRENT_ID] &&
       sequence_vims_stream_length_type == info->status_tokens[STREAM_TYPE])
        return sequence_vims_stream_length_frames;

    return MAX(0, info->status_tokens[SAMPLE_MARKER_END]);
}
                                             ...
                                             gint64 *value)
{
    ...
    buffer[width] = '\0';
    parsed = g_ascii_strtoll(buffer, &end, 10);
    if(end == buffer || *end != '\0')
        return FALSE;
...
static gboolean sequence_vims_parse_fixed_uint_local(const char *text,
                                                     int width,
                                                     unsigned int *value)
{
    char buffer[32];
    ...
    memcpy(buffer, text, (size_t)width);
    buffer[width] = '\0';
    parsed = g_ascii_strtoull(buffer, &end, 10);
    if(end == buffer || *end != '\0' || parsed > G_MAXUINT)
        return FALSE;
...
{
    if(bank < 0) {
        memset(sequence_vims_timelines, 0, sizeof(sequence_vims_timelines));
    ...
    memset(&sequence_vims_timelines[bank],
           0,
           sizeof(sequence_vims_timelines[bank]));

    if(sequence_vims_tick_bank == bank) {
        sequence_vims_tick_valid = FALSE;
        sequence_vims_tick_bank = -1;
        sequence_vims_tick_slot = -1;
    }
...
static void sequence_vims_note_outgoing_id(int id)
{
    ...
    if(id == VIMS_SAMPLE_SET_START ||
       id == VIMS_SAMPLE_SET_END ||
       id == VIMS_SAMPLE_SET_LOOPTYPE ||
       id == VIMS_SAMPLE_SET_LOOPS ||
       id == VIMS_VIDEO_SET_SPEED ||
       id == VIMS_SAMPLE_SET_SPEED ||
       id == VIMS_VIDEO_SET_SLOW ||
       id == VIMS_STREAM_SET_LENGTH)
        sequence_vims_timeline_invalidate(-1);
}

static gboolean sequence_vims_timeline_fetch(int bank)
{
    char command[32];
    gchar *body;
    gint body_len = 0;
    ...
    int entries;
    int offset;
    sequence_vims_timeline_t parsed;

    if(!info || !info->client ||
       bank < 0 || bank >= VJ_SEQUENCE_BANKS)
        return FALSE;

    snprintf(command, sizeof(command), "%03d:%d;", VIMS_SEQUENCE_TIMELINE, bank);
    if(!sequence_vims_send_raw(command))
        return FALSE;

    body = recv_vims(8, &body_len);
    if(!body || body_len < VJ_SEQUENCE_TIMELINE_HEADER_LENGTH) {
        if(body)
            free(body);
        return FALSE;
    }
    ...
    gint64 temporary = 0;
    if(!sequence_vims_parse_fixed_ll(body, 2, &temporary) ||
       temporary < 0 || temporary >= VJ_SEQUENCE_BANKS)
    {
        free(body);
        return FALSE;
    }
    reply_bank = (int)temporary;

    if(!sequence_vims_parse_fixed_uint_local(body + 2, 10, &revision) ||
       !sequence_vims_parse_fixed_ll(body + 12, 1, &temporary) ||
    ...
    if(!sequence_vims_parse_fixed_ll(body + 13, 12, &total) ||
       !sequence_vims_parse_fixed_ll(body + 25, 3, &temporary))
    {
        free(body);
        return FALSE;
    }

    entries = (int)temporary;
    if(reply_bank != bank || entries < 0 || entries > MAX_SEQUENCES ||
       body_len != VJ_SEQUENCE_TIMELINE_HEADER_LENGTH +
                   entries * VJ_SEQUENCE_TIMELINE_ENTRY_LENGTH)
    {
        free(body);
        return FALSE;
    }
    ...
    parsed.loaded = TRUE;
    parsed.finite = finite ? TRUE : FALSE;
    parsed.revision = revision;
    parsed.total = finite ? total : -1;
    ...
    for(int i = 0; i < entries; i++) {
        ...
        if(!sequence_vims_parse_fixed_ll(body + offset, 3, &temporary)) {
            free(body);
            return FALSE;
        }
        slot = (int)temporary;

        if(!sequence_vims_parse_fixed_ll(body + offset + 3, 5, &temporary)) {
            free(body);
            return FALSE;
        }
        source_id = (int)temporary;

        if(!sequence_vims_parse_fixed_ll(body + offset + 8, 2, &temporary)) {
            free(body);
            return FALSE;
        }
        source_type = (int)temporary;

        if(!sequence_vims_parse_fixed_ll(body + offset + 10, 12, &start) ||
           !sequence_vims_parse_fixed_ll(body + offset + 22, 12, &length) ||
        ...
        parsed.slots[slot].present = 1;
        parsed.slots[slot].source_id = source_id;
        parsed.slots[slot].source_type = source_type;
        parsed.slots[slot].start = start;
        parsed.slots[slot].length = length;
...
static gboolean sequence_vims_timeline_ensure(int bank)
{
    unsigned int revision;
    ...
    revision = gvr_sequence_bank_view_get_bank_revision(info->sequence_bank_view,
                                                         bank);

    if(sequence_vims_timelines[bank].loaded &&
       sequence_vims_timelines[bank].revision == revision)
        return TRUE;

    return sequence_vims_timeline_fetch(bank);
}

static int sequence_vims_timeline_frame_count(int bank)
{
    if(!sequence_vims_timeline_ensure(bank) ||
       !sequence_vims_timelines[bank].finite ||
       sequence_vims_timelines[bank].total <= 0)
        return 0;

    return (int)MIN(sequence_vims_timelines[bank].total,
                    (gint64)G_MAXINT);
}

static sequence_vims_timeline_slot_t *sequence_vims_timeline_slot(int bank,
                                                                  int slot)
{
    if(!sequence_vims_timeline_ensure(bank) ||
       slot < 0 || slot >= MAX_SEQUENCES ||
       !sequence_vims_timelines[bank].slots[slot].present)
        return NULL;

    return &sequence_vims_timelines[bank].slots[slot];
}

static gboolean sequence_vims_pattern_send_document(void)
{
    gchar *serialized;
    gchar *encoded;
    ...
    command = g_strdup_printf("%03d:1 %s;",
...
static gboolean sequence_vims_pattern_store_timeout(gpointer user_data)
{
    (void)user_data;
    sequence_vims_pattern_store_timeout_id = 0;

    if(sequence_vims_pattern_dirty && !sequence_vims_pattern_loading)
        sequence_vims_pattern_send_document();

    return G_SOURCE_REMOVE;
}

static void sequence_vims_pattern_schedule_store(void)
{
    if(sequence_vims_pattern_loading)
        return;

    sequence_vims_pattern_dirty = TRUE;

    if(sequence_vims_pattern_store_timeout_id)
        g_source_remove(sequence_vims_pattern_store_timeout_id);

    sequence_vims_pattern_store_timeout_id =
        g_timeout_add(250, sequence_vims_pattern_store_timeout, NULL);
}
...
static void sequence_vims_pattern_load_backend(void)
{
    ...
    gint body_len = 0;
    unsigned int revision;
    gint64 format_length_ll;
    gint64 data_length_ll;
    int format_length;
    int data_length;
    gchar *format;
    guchar *decoded = NULL;
    ...
    format_length = (int)format_length_ll;
    data_length = (int)data_length_ll;

    if(format_length < 0 || format_length > 31 ||
       data_length < 0 || data_length > VJ_SEQUENCE_PATTERN_DATA_MAX ||
       body_len != VJ_SEQUENCE_PATTERN_HEADER_LENGTH +
                   format_length + data_length)
    {
        free(body);
        return;
    }

    format = g_strndup(body + VJ_SEQUENCE_PATTERN_HEADER_LENGTH,
                       (gsize)format_length);

    sequence_vims_pattern_loading = TRUE;

    if(data_length == 0) {
        gvr_vims_pattern_view_deserialize(info->vims_pattern_view,
                                          empty_document,
                                          strlen(empty_document));
    }
    else if(g_str_has_prefix(format, "gvr-vims-pattern-b64-v")) {
        decoded = g_base64_decode(
            body + VJ_SEQUENCE_PATTERN_HEADER_LENGTH + format_length,
            &decoded_length);
    ...
    sequence_vims_pattern_loading = FALSE;
    sequence_vims_pattern_backend_revision = revision;
    sequence_vims_pattern_dirty = FALSE;
...
static void sequence_vims_refresh_timing_signature(void)
{
    ...
    memcpy(sequence_vims_timing_signature,
           signature,
           sizeof(signature));
}
...
void vj_gui_vims_pattern_set_stream_length(int frame_count)
{
    sequence_vims_stream_length_cache(frame_count);
    sequence_vims_sync_target();
}

static gboolean sequence_vims_grid_button_release(GtkWidget *widget,
...
static gboolean sequence_vims_grid_key_release(GtkWidget *widget,
                                                GdkEventKey *event,
                                                gpointer user_data)
{
    (void)widget;
                                                   ...
                                                   gboolean forced_user_event)
{
    ...
    learning = info->vims_pattern_view &&
    ...
    int capture_frame = vj_kf_timeline_position();

    if(info->status_tokens[SEQ_ACT] != 0 &&
       info->sequence_bank_view) {
        int selected_bank =
            gvr_sequence_bank_view_get_selected_bank(
                info->sequence_bank_view);
        int selected_slot = -1;

        if(selected_bank >= 0 &&
           !gvr_sequence_bank_view_get_selected_slot(
               info->sequence_bank_view,
               NULL,
               &selected_slot) &&
           sequence_vims_tick_valid &&
           sequence_vims_tick_bank == selected_bank)
        {
            capture_frame =
                (int)MIN(sequence_vims_tick_frame,
                         (gint64)G_MAXINT);
        }
        else {
            int playing = normalize_sequence_slot(
                info->status_tokens[SEQ_CUR],
                info->sequencer_col * info->sequencer_row);
            int source_id = 0;
            int source_type = 0;

            if(playing >= 0 &&
               gvr_sequence_bank_view_get_slot(
                   info->sequence_bank_view,
                   ...
                   &source_type) &&
               source_type != 0)
                capture_frame = ui_stream_transport_position();
        }
    }

    gvr_vims_pattern_view_capture_message(
        info->vims_pattern_view,
        message,
        MAX(0, capture_frame));
}
                                            ...
                                            gpointer user_data)
{
    ...
    if(!info || !ui_playmode_is_sample(info->status_tokens[PLAY_MODE]) ||
       sample_id <= 0 || info->status_tokens[CURRENT_ID] != sample_id)
        return;
    ...
    if(info->status_tokens[SEQ_ACT] != 0) {
        ...
        playing = normalize_sequence_slot(info->status_tokens[SEQ_CUR],
                                          info->sequencer_col * info->sequencer_row);
        if(bank != active_bank || slot != playing)
            return;
    }
    else if(bank != GVR_VIMS_PATTERN_SAMPLE_BANK) {
        return;
    }

    start = info->status_tokens[SAMPLE_START];
    end = info->status_tokens[SAMPLE_END];
    if(end < start) {
        int swap = start;
        start = end;
        end = swap;
    ...
    absolute = start + MAX(0, frame);
    if(absolute > end)
        absolute = end;

    sequence_vims_replaying = TRUE;
    multi_vims(VIMS_VIDEO_SET_FRAME, "%d", absolute);
    if(flags & GVR_VIMS_PATTERN_TRANSPORT_PLAY)
        single_vims(VIMS_VIDEO_PLAY_FORWARD);
    sequence_vims_replaying = FALSE;
}

static void sequence_vims_build_pattern_summary(int bank,
                                                int slot,
                                                GvrSequencePatternSummary *summary)
{
    GvrVimsPatternSummary source = { 0 };
    GvrVimsPatternSummary cell = { 0 };
    int sample_id = 0;
    int sample_type = 0;

    memset(summary, 0, sizeof(*summary));

    if(!info || !info->vims_pattern_view || !info->sequence_bank_view ||
       bank < 0 || bank >= VJ_SEQUENCE_BANKS ||
       slot < 0 || slot >= MAX_SEQUENCES)
        return;
    ...
    if(sample_id > 0)
        gvr_vims_pattern_view_get_source_summary(info->vims_pattern_view,
                                                 sample_id,
                                                 sample_type,
                                                 &source);

    gvr_vims_pattern_view_get_cell_summary(info->vims_pattern_view,
                                            bank,
                                            slot,
                                            &cell);

    if(source.flags & GVR_VIMS_PATTERN_HAS_VIMS)
        summary->flags |= GVR_SEQUENCE_PATTERN_HAS_VIMS |
                          GVR_SEQUENCE_PATTERN_SOURCE_VIMS;
    if(source.flags & GVR_VIMS_PATTERN_HAS_HOLD)
        summary->flags |= GVR_SEQUENCE_PATTERN_HAS_HOLD |
                          GVR_SEQUENCE_PATTERN_SOURCE_HOLD;
    if(cell.flags & GVR_VIMS_PATTERN_HAS_VIMS)
        summary->flags |= GVR_SEQUENCE_PATTERN_HAS_VIMS |
                          GVR_SEQUENCE_PATTERN_CELL_VIMS;
    if(cell.flags & GVR_VIMS_PATTERN_HAS_HOLD)
        summary->flags |= GVR_SEQUENCE_PATTERN_HAS_HOLD |
                          GVR_SEQUENCE_PATTERN_CELL_HOLD;

    summary->source_events = source.event_count;
    summary->source_rows = source.row_count;
    summary->cell_events = cell.event_count;
    summary->cell_rows = cell.row_count;
}
...
static void sequence_vims_refresh_cell_badge(int bank, int slot)
{
    ...
    if(!info || !info->sequence_bank_view || !info->vims_pattern_view)
        return;

    sequence_vims_build_pattern_summary(bank, slot, &summary);
    gvr_sequence_bank_view_set_pattern_summary(info->sequence_bank_view,
                                               bank,
                                               slot,
                                               &summary);
}
...
static void sequence_vims_refresh_bank_badge(int bank)
{
    GvrVimsPatternSummary pattern = { 0 };
    GvrSequenceBankPatternSummary summary = { 0 };

    if(!info || !info->sequence_bank_view || !info->vims_pattern_view ||
       bank < 0 || bank >= VJ_SEQUENCE_BANKS)
        return;

    gvr_vims_pattern_view_get_bank_summary(info->vims_pattern_view,
                                           bank,
                                           &pattern);

    summary.flags = pattern.flags;
    summary.event_count = pattern.event_count;
    summary.row_count = pattern.row_count;

    if(sequence_vims_timelines[bank].loaded &&
       sequence_vims_timelines[bank].finite &&
       sequence_vims_timelines[bank].total > 0) {
        summary.timeline_known = TRUE;
        summary.timeline_length = sequence_vims_timelines[bank].total;
    }

    gvr_sequence_bank_view_set_bank_pattern_summary(info->sequence_bank_view,
                                                    bank,
                                                    &summary);
}

static void sequence_vims_refresh_source_badges(int source_bank, int source_id)
{
    if(!info || !info->sequence_bank_view || source_id <= 0)
        return;
    ...
    for(int bank = 0; bank < VJ_SEQUENCE_BANKS; bank++) {
        for(int slot = 0; slot < MAX_SEQUENCES; slot++) {
            ...
            if(source_bank == GVR_VIMS_PATTERN_SAMPLE_BANK && sample_type != 0)
                continue;
            if(source_bank == GVR_VIMS_PATTERN_STREAM_BANK && sample_type == 0)
                continue;
...
static void sequence_vims_pattern_changed(GtkWidget *widget,
                                          gint bank,
                                          gint slot,
                                          guint flags,
                                          gpointer user_data)
{
    ...
    if(bank >= 0)
        ...
        sequence_vims_refresh_source_badges(bank, slot);
    else if(bank == GVR_VIMS_PATTERN_SEQUENCE_BANK)
        sequence_vims_refresh_bank_badge(slot);

    sequence_vims_pattern_schedule_store();
}

static void sequence_vims_clipboard_reset(GtkWidget *widget, gpointer user_data)
{
    (void)widget;
    (void)user_data;
    if(info && info->vims_pattern_view)
        gvr_vims_pattern_view_clipboard_clear(info->vims_pattern_view);
}

static void sequence_vims_clipboard_copy(GtkWidget *widget,
                                         gint bank,
                                         gint slot,
                                         gint offset,
                                         gpointer user_data)
{
    ...
    (void)user_data;
    if(info && info->vims_pattern_view)
        gvr_vims_pattern_view_clipboard_copy(info->vims_pattern_view,
                                             bank,
                                             slot,
                                             offset);
}

static void sequence_vims_clipboard_paste(GtkWidget *widget,
                                          gint offset,
                                          gint bank,
                                          gint slot,
                                          gpointer user_data)
{
    ...
    (void)user_data;
    if(info && info->vims_pattern_view)
        gvr_vims_pattern_view_clipboard_paste(info->vims_pattern_view,
                                              offset,
                                              bank,
                                              slot);
}
...
static void sequence_vims_update_playback(void)
{
    ...
    if(info->status_tokens[SEQ_ACT] != 0 &&
       info->sequence_bank_view) {
        ...
        int slot = normalize_sequence_slot(info->status_tokens[SEQ_CUR],
                                           info->sequencer_col *
                                           info->sequencer_row);
        int source_id = 0;
        int source_type = 0;
        ...
        if(slot >= 0 &&
           gvr_sequence_bank_view_get_slot(info->sequence_bank_view,
                                           ...
                                           &source_type) &&
           source_id > 0) {
            int source_bank = source_type == 0 ?
                              GVR_VIMS_PATTERN_SAMPLE_BANK :
                              GVR_VIMS_PATTERN_STREAM_BANK;
            int local_frame = source_type == 0 ?
                              vj_kf_timeline_position() :
                              ui_stream_transport_position();
            sequence_vims_timeline_slot_t *layout =
                sequence_vims_timeline_slot(bank, slot);

            if(local_frame < 0)
                local_frame = 0;

            if(sequence_vims_active_source_bank != source_bank ||
               sequence_vims_active_source_slot != source_id) {
                if(sequence_vims_active_source_bank != -999)
                    gvr_vims_pattern_view_update_playback(
                        info->vims_pattern_view,
                        sequence_vims_active_source_bank,
                        sequence_vims_active_source_slot,
                        0,
                        FALSE,
                        0);

                sequence_vims_active_source_bank = source_bank;
                sequence_vims_active_source_slot = source_id;
            }

            if(sequence_vims_active_cell_bank != bank ||
               sequence_vims_active_cell_slot != slot) {
                if(sequence_vims_active_cell_bank >= 0)
                    gvr_vims_pattern_view_update_playback(
                        info->vims_pattern_view,
                        sequence_vims_active_cell_bank,
                        sequence_vims_active_cell_slot,
                        0,
                        FALSE,
                        0);

                sequence_vims_active_cell_bank = bank;
                sequence_vims_active_cell_slot = slot;
            }

            if(sequence_vims_active_bank_pattern != bank) {
                if(sequence_vims_active_bank_pattern >= 0)
                    gvr_vims_pattern_view_update_bank_playback(
                        info->vims_pattern_view,
                        sequence_vims_active_bank_pattern,
                        0,
                        FALSE,
                        0);

                sequence_vims_active_bank_pattern = bank;
                sequence_vims_tick_valid = FALSE;
            }

            if(layout && layout->start >= 0 && layout->length > 0) {
                gint64 slot_end = layout->start + layout->length - 1;

                if(!sequence_vims_tick_valid ||
                   sequence_vims_tick_bank != bank ||
                   sequence_vims_tick_slot != slot) {
                    sequence_vims_tick_bank = bank;
                    sequence_vims_tick_slot = slot;
                    sequence_vims_tick_frame = layout->start;
                    sequence_vims_tick_valid = TRUE;
                }
                else if(speed > 0) {
                    if(sequence_vims_tick_frame < slot_end)
                        sequence_vims_tick_frame++;
                    else if(sequence_vims_timelines[bank].entries == 1)
                        sequence_vims_tick_frame = layout->start;
                }

                if(sequence_vims_timelines[bank].finite &&
                   sequence_vims_timelines[bank].total > 0 &&
                   sequence_vims_tick_frame >=
                       sequence_vims_timelines[bank].total)
                    sequence_vims_tick_frame =
                        sequence_vims_timelines[bank].total - 1;
            }
            else {
                sequence_vims_tick_valid = FALSE;
            }
            ...
            gvr_vims_pattern_view_update_bank_playback(
                ...
                bank,
                sequence_vims_tick_valid ?
                    (int)MIN(sequence_vims_tick_frame, (gint64)G_MAXINT) : 0,
                sequence_vims_tick_valid,
                speed > 0 ? 64 : 0);
            return;
        }
    ...
    if(sequence_vims_active_cell_bank >= 0)
        gvr_vims_pattern_view_update_playback(info->vims_pattern_view,
                                              sequence_vims_active_cell_bank,
                                              sequence_vims_active_cell_slot,
                                              0,
                                              FALSE,
                                              0);
    if(sequence_vims_active_bank_pattern >= 0)
        gvr_vims_pattern_view_update_bank_playback(info->vims_pattern_view,
                                                   sequence_vims_active_bank_pattern,
                                                   0,
                                                   FALSE,
                                                   0);

    sequence_vims_active_cell_bank = -1;
    sequence_vims_active_cell_slot = -1;
    ...
    if(play_mode == MODE_SAMPLE && info->status_tokens[CURRENT_ID] > 0) {
        int source_id = info->status_tokens[CURRENT_ID];
        int frame = vj_kf_timeline_position();

        if(sequence_vims_active_source_bank !=
               GVR_VIMS_PATTERN_SAMPLE_BANK ||
           sequence_vims_active_source_slot != source_id) {
            if(sequence_vims_active_source_bank != -999)
                gvr_vims_pattern_view_update_playback(
    ...
    if(play_mode == MODE_STREAM && info->status_tokens[CURRENT_ID] > 0) {
        int source_id = info->status_tokens[CURRENT_ID];
        int length = sequence_vims_stream_pattern_length();
        int frame = ui_stream_transport_position();
...
static void fx_chain_controls_sync_from_status(int play_mode, int active)
{
    ...
    if(GTK_IS_TOGGLE_BUTTON(curve_toggle)) {
        if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(curve_toggle)) != active)
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(curve_toggle), active);
    }

    if(GTK_IS_TOGGLE_BUTTON(mode_toggle)) {
        if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(mode_toggle)) != active)
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(mode_toggle), active);
    }

    if(GTK_IS_TOGGLE_BUTTON(panel_toggle)) {
...
static void update_current_slot(int *history, int pm, int last_pm) {
    ...
    if (pm == MODE_SAMPLE)
    {
        ...
        if(source_changed || history[SAMPLE_SPEED] != info->status_tokens[SAMPLE_SPEED])
        {
            ...
            if(speed < 0) info->play_direction = -1;
            else if(speed > 0) info->play_direction = 1;
            if( speed < 0 ) speed *= -1;
...
static void load_sequence_list_legacy(void)
{
    ...
    while( offset < nlen )
    {
        ...
        sequence_vims_bind_cell(bank, id, sample_id, type);
    ...
    for(; id < (SEQUENCER_COL * SEQUENCER_ROW); id++)
    ...
    if( len >= min_payload && text != NULL ) {
        ...
        for(int bank_i = 0; bank_i < VJ_SEQUENCE_BANKS; bank_i++) {
            ...
            for(int slot = 0; slot < (SEQUENCER_COL * SEQUENCER_ROW); slot++) {
                ...
                sequence_vims_bind_cell(bank, slot, sample_id, type);
...
static void load_samplelist_info(void)
{
    ...
    reset_samplebank();
    sequence_vims_source_lengths_clear();
    ...
    if(fxlen > 0 && fxtext != NULL)
    {
        while(offset + 3 <= fxlen)
        {
            ...
            if(len >= header_len)
            {
                ...
                int sample_length = values[2] - values[1] + 1;
                ...
                if(sample_length < 1)
                    sample_length = values[1] - values[2] + 1;
                sequence_vims_source_length_store(int_id, sample_type, sample_length);
...
static void update_status_accessibility(int old_pm, int new_pm, int force)
{
    ...
    if(new_pm == MODE_STREAM)
    {
        for(i = 0; sample_widget_map[i].sample_widget_id != SAMPLE_WIDGET_NONE; i++)
            set_widget_sensitive_cached(sample_widget_map[i].widget_id, FALSE);

        for(i = 0; plain_widget_map[i].plain_widget_id != PLAIN_WIDGET_NONE; i++)
            set_widget_sensitive_cached(plain_widget_map[i].widget_id, FALSE);

        for(i = 0; stream_widget_map[i].stream_widget_id != STREAM_WIDGET_NONE; i++)
            set_widget_sensitive_cached(stream_widget_map[i].widget_id, TRUE);
    }

    if(new_pm == MODE_SAMPLE)
    {
        for(i = 0; stream_widget_map[i].stream_widget_id != STREAM_WIDGET_NONE; i++)
            set_widget_sensitive_cached(stream_widget_map[i].widget_id, FALSE);

        for(i = 0; plain_widget_map[i].plain_widget_id != PLAIN_WIDGET_NONE; i++)
            set_widget_sensitive_cached(plain_widget_map[i].widget_id, FALSE);

        for(i = 0; sample_widget_map[i].sample_widget_id != SAMPLE_WIDGET_NONE; i++)
            set_widget_sensitive_cached(sample_widget_map[i].widget_id, TRUE);
    }
...
static GtkWidget *playmode_notebook_tab_label(void)
{
    ...
    if(GTK_IS_NOTEBOOK(notebook)) {
        if(GTK_IS_WIDGET(page)) {
            if(gtk_notebook_page_num(GTK_NOTEBOOK(notebook), page) >= 0)
...
static void audio_sync_apply_widget_visibility(GtkWidget *w, int visible)
{
    if(!w)
        return;
...
static void audio_sync_set_widget_visible(int widget_id, int visible)
{
    ...
    audio_sync_apply_widget_visibility(w, visible);
}
...
static void update_globalinfo(int *history, int pm, int last_pm)
{
    ...
    if(total_frames_ != history_frames_ || source_changed)
    {
        ...
        if( pm == MODE_STREAM )
        {
            sequence_vims_stream_length_cache(info->status_tokens[SAMPLE_MARKER_END]);
    ...
    if( info->status_tokens[FEEDBACK] != history[FEEDBACK] ) {
        if(info->status_tokens[FEEDBACK] == 1) {
            for ( i = 0; fb_widget_map[i].fb_widget_id != FB_WIDGET_NONE; i ++ )
                set_widget_sensitive_cached(fb_widget_map[i].widget_id, FALSE);
        }
        else {
            for ( i = 0; fb_widget_map[i].fb_widget_id != FB_WIDGET_NONE; i ++ )
                set_widget_sensitive_cached(fb_widget_map[i].widget_id, TRUE);
        }
    ...
    if( info->status_tokens[MACRO] != history[MACRO] )
    {
        ...
        {
            case 1:
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget_cache[WIDGET_MACRORECORD]), TRUE);
                break;
            case 2:
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget_cache[WIDGET_MACROPLAY]), TRUE);
                break;
            default:
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget_cache[WIDGET_MACROSTOP]), TRUE);
                break;
        }
    }
...
{
    const gboolean pattern_document_reload =
        info->uc.reload_hint[HINT_SLIST] ? TRUE : FALSE;
    const gboolean sequence_layout_reload =
        info->uc.reload_hint[HINT_SEQ_ACT] == 1 ? TRUE : FALSE;
    ...
    if(sequence_layout_reload)
    {
        sequence_vims_timeline_invalidate(-1);
        load_sequence_list();
    }

    if(pattern_document_reload)
...
static void update_stream_trickplay_status(int *history, int initial)
{
    ...
    if(GTK_IS_SPIN_BUTTON(widget_cache[WIDGET_STREAM_BUFFER_LENGTH])) {
...
void update_gui(void)
{
    ...
    if(last_pm < 0 ||
       history[CURRENT_FPS] != info->status_tokens[CURRENT_FPS])
    {
        update_slider_gvalue(
            "framerate",
            ui_clampd(
                (gdouble)info->status_tokens[CURRENT_FPS] * 0.01,
                0.0,
                120.0));
    }
...
void vj_gui_free(void)
{
    ...
    if(sequence_vims_source_lengths) {
        g_hash_table_destroy(sequence_vims_source_lengths);
        sequence_vims_source_lengths = NULL;
    }
    sequence_vims_stream_length_frames = 0;
    sequence_vims_stream_length_id = -1;
    sequence_vims_stream_length_type = -1;
    ...
    sequence_vims_pattern_dirty = FALSE;
    sequence_vims_pattern_loading = FALSE;
    sequence_vims_pattern_backend_revision = 0;
    sequence_vims_timeline_invalidate(-1);
    sequence_vims_active_source_bank = -999;
    sequence_vims_active_source_slot = -1;
    sequence_vims_active_cell_bank = -1;
...
static void reset_sequencer_ui_state(void)
{
    ...
    if(GTK_IS_TOGGLE_BUTTON(widget_cache[WIDGET_SEQACTIVE]))
...
#define VJ_DETACH_DATA_READY "gveejay-detach-ready"
#define VJ_DETACH_DATA_ORIGIN "gveejay-detach-origin"
#define VJ_DETACH_DATA_ORIGIN_POS "gveejay-detach-origin-pos"
#define VJ_DETACH_DATA_ORIGIN_TITLE "gveejay-detach-origin-title"
#define VJ_DETACH_DATA_IS_PLACEHOLDER "gveejay-detach-is-placeholder"
#define VJ_DETACH_DATA_PLACEHOLDER "gveejay-detach-placeholder"
#define VJ_DETACH_DATA_PLACEHOLDER_PAGE "gveejay-detach-placeholder-page"
#define VJ_DETACH_DATA_WINDOW "gveejay-detach-window"
#define VJ_DETACH_DATA_TAB_READY "gveejay-detach-tab-ready"
#define VJ_DETACH_DATA_TAB_PAGE "gveejay-detach-tab-page"
#define VJ_DETACH_DATA_DETACHED "gveejay-detach-record"
#define VJ_DETACH_DATA_DETACHED_NOTEBOOK "gveejay-detach-detached-notebook"
#define VJ_DETACH_DATA_RESTORE_ONCE "gveejay-detach-restore-once"
...
{
    GtkWidget *page;
    detached_notebook_t *detached;
} detachable_target_action_t;

static GList *detached_notebooks_ = NULL;
static gboolean detachable_reordering_ = FALSE;
static gboolean detachable_in_explicit_detach_ = FALSE;
static gint detachable_next_window_id_ = 1;

static void detachable_notebook_setup(GtkNotebook *notebook, detached_notebook_t *detached);
static gboolean detachable_window_delete(GtkWidget *window, GdkEvent *event, gpointer user_data);
static gboolean detachable_tab_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data);
static gboolean detachable_window_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data);
static void detachable_notebook_update_visibility(GtkNotebook *notebook);
static void init_window_configuration_menu(void);
static void detachable_restore_window_configuration(GtkMenuItem *item, gpointer user_data);
static void detachable_restore_default_window_configuration(GtkMenuItem *item, gpointer user_data);
...
typedef struct
{
    ...
    GtkWidget *right;
    GtkWidget *footer;
    GtkWidget *footer_actions;
    gboolean detached;
} samplebank_action_layout_t;

static samplebank_action_layout_t samplebank_action_layout_ = { 0 };

static void samplebank_action_reparent(GtkWidget *widget,
                                       GtkWidget *parent,
                                       gboolean pack_end)
{
    GtkWidget *old_parent;
...
static GtkWidget *samplebank_action_pack_button_sized(GtkWidget *box,
                                                      const char *name,
                                                      int width,
                                                      int height,
                                                      gboolean fill)
{
    ...
    if(!box || !name || !info || !info->main_window)
        return NULL;

    button = glade_xml_get_widget_(info->main_window, name);
    if(!button)
        return NULL;

    samplebank_action_reparent(button, box, FALSE);
    gtk_widget_set_size_request(button, width, height);
    gtk_widget_set_halign(button,
                          fill ? GTK_ALIGN_FILL : GTK_ALIGN_CENTER);
    gtk_widget_set_valign(button, GTK_ALIGN_CENTER);
    gtk_widget_show(button);
    return button;
}

static GtkWidget *samplebank_action_pack_button(GtkWidget *column,
                                                const char *name)
{
    return samplebank_action_pack_button_sized(
        column,
        name,
        30,
        28,
...
static GtkWidget *samplebank_footer_pack_button(GtkWidget *footer,
                                                const char *name)
{
    GtkWidget *button =
        samplebank_action_pack_button_sized(
            footer,
            name,
            26,
            20,
    ...
    if(button)
        add_class(button, "samplebank-footer-action");

    return button;
}

static void samplebank_action_layout_set_detached(gboolean detached)
{
    samplebank_action_layout_t *layout = &samplebank_action_layout_;

    if(!layout->host || !layout->top || !layout->body ||
       !layout->center || !layout->right)
        return;

    layout->detached = detached ? TRUE : FALSE;

    samplebank_action_reparent(layout->right,
                               layout->top,
                               FALSE);

    gtk_orientable_set_orientation(
        GTK_ORIENTABLE(layout->right),
        GTK_ORIENTATION_HORIZONTAL);
    gtk_widget_set_halign(layout->right,
                          GTK_ALIGN_START);
    gtk_widget_set_valign(layout->right,
                          GTK_ALIGN_CENTER);

    gtk_widget_set_no_show_all(layout->top,
                               FALSE);
    gtk_widget_show_all(layout->top);
    gtk_widget_show(layout->body);
    gtk_widget_show(layout->center);
    gtk_widget_show(layout->right);

    if(layout->detached)
        add_class(layout->host,
                  "samplebank-actions-detached");
    else
        remove_class(layout->host,
                     "samplebank-actions-detached");

    if(layout->footer_actions)
        gtk_widget_show_all(layout->footer_actions);

    gtk_widget_queue_resize(layout->host);
}


static void samplebank_action_layout_update_for_page(GtkWidget *page)
{
    if(!page || page != samplebank_action_layout_.page)
        return;

    samplebank_action_layout_set_detached(
        g_object_get_data(G_OBJECT(page),
                          VJ_DETACH_DATA_DETACHED) != NULL);
}

static GtkWidget *samplebank_action_layout_mount(void)
{
    static const char *right_buttons[] = {
        "sampleadd",
        "streamnew",
        "new_colorstream",
        "new_generatorstream"
    };
    static const char *footer_buttons[] = {
    ...
    samplebank_action_layout_t *layout =
        &samplebank_action_layout_;
    ...
    gtk_orientable_set_orientation(
        GTK_ORIENTABLE(layout->host),
        GTK_ORIENTATION_VERTICAL);
    add_class(layout->host,
              "samplebank-action-shell");
    ...
    add_class(layout->top,
              "samplebank-action-top");
    add_class(layout->body,
              "samplebank-action-body");
    add_class(layout->center,
              "samplebank-action-center");
    add_class(layout->right,
              "samplebank-action-toolbar");

    gtk_widget_set_hexpand(layout->top, TRUE);
    gtk_widget_set_hexpand(layout->body, TRUE);
    gtk_widget_set_vexpand(layout->body, TRUE);
    gtk_widget_set_hexpand(layout->center, TRUE);
    gtk_widget_set_vexpand(layout->center, TRUE);
    gtk_widget_set_halign(layout->center,
                          GTK_ALIGN_FILL);
    gtk_widget_set_valign(layout->center,
                          GTK_ALIGN_FILL);

    gtk_box_pack_start(GTK_BOX(layout->host),
                       layout->top,
                       FALSE,
                       FALSE,
                       0);
    gtk_box_pack_start(GTK_BOX(layout->host),
                       ...
                       TRUE,
                       0);
    gtk_box_pack_start(GTK_BOX(layout->top),
                       layout->right,
                       FALSE,
                       FALSE,
                       0);
    gtk_box_pack_start(GTK_BOX(layout->body),
                       layout->center,
                       TRUE,
                       TRUE,
                       0);
    ...
    for(guint i = 0;
        ...
        i++)
    {
        samplebank_action_pack_button(
            layout->right,
            right_buttons[i]);
    }

    if(GTK_IS_BOX(layout->footer)) {
        layout->footer_actions =
            gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 1);

        add_class(layout->footer_actions,
                  "samplebank-footer-actions");

        gtk_widget_set_halign(layout->footer_actions,
                              GTK_ALIGN_START);
        gtk_widget_set_valign(layout->footer_actions,
                              GTK_ALIGN_CENTER);
        gtk_widget_set_hexpand(layout->footer_actions,
                               FALSE);
        gtk_widget_set_vexpand(layout->footer_actions,
                               FALSE);

        gtk_box_pack_start(GTK_BOX(layout->footer),
                           layout->footer_actions,
                           FALSE,
                           FALSE,
                           0);
        gtk_box_reorder_child(GTK_BOX(layout->footer),
                              layout->footer_actions,
                              0);

        for(guint i = 0;
            i < G_N_ELEMENTS(footer_buttons);
            i++)
        {
            samplebank_footer_pack_button(
                layout->footer_actions,
                footer_buttons[i]);
        }
    ...
    gtk_widget_show(layout->center);
    gtk_widget_show(layout->right);
    samplebank_action_layout_set_detached(FALSE);
...
static void detachable_preserve_hidden_descendants(GtkWidget *widget)
{
    ...
    if(GTK_IS_CONTAINER(widget)) {
        ...
        for(GList *node = children; node; node = node->next)
            if(GTK_IS_WIDGET(node->data))
                detachable_preserve_hidden_descendants(GTK_WIDGET(node->data));
        g_list_free(children);
    }
}

static void detachable_refresh_mode_visibility(void)
{
    if(!info)
        return;

    audio_sync_update_mode_sensitivity(info->status_tokens[AUDIO_SYNC_MODE],
                                       info->status_tokens[AUDIO_SYNC_SOURCE],
                                       info->status_tokens[AUDIO_SYNC_TARGET_MODE],
                                       info->status_tokens[RECORD_AUDIO_SOURCE]);
}

static GtkWidget *detachable_main_window(void)
{
    if(!info || !info->main_window)
        return NULL;

    return glade_xml_get_widget_(info->main_window, "gveejay_window");
}

static gboolean detachable_page_is_placeholder(GtkWidget *page)
{
    if(!GTK_IS_WIDGET(page))
        return FALSE;

    return g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_IS_PLACEHOLDER) != NULL;
}

static gint detachable_notebook_real_page_count(GtkNotebook *notebook)
{
    gint count = 0;
    ...
    for(gint i = 0; i < n_pages; i++) {
        ...
        if(GTK_IS_WIDGET(page)) {
            if(!detachable_page_is_placeholder(page))
                count++;
        }
    }
...
static void detachable_notebook_select_real_page(GtkNotebook *notebook)
{
    gint page_num = detachable_notebook_first_real_page(notebook);

    if(page_num >= 0)
        gtk_notebook_set_current_page(notebook, page_num);
}

static gboolean detachable_notebook_current_page_is_placeholder(GtkNotebook *notebook)
{
    gint page_num;
    GtkWidget *page;
    ...
    return detachable_page_is_placeholder(page);
}
...
static void detachable_notebook_update_visibility(GtkNotebook *notebook)
{
    ...
    if(detachable_notebook_real_page_count(notebook) <= 0) {
...
static gchar *detachable_label_text(GtkWidget *widget)
{
    ...
    if(GTK_IS_CONTAINER(widget)) {
        GList *children = gtk_container_get_children(GTK_CONTAINER(widget));
        for(GList *node = children; node; node = node->next) {
            gchar *text = GTK_IS_WIDGET(node->data) ?
                detachable_label_text(GTK_WIDGET(node->data)) : NULL;
            if(text && *text) {
                g_list_free(children);
                return text;
            }
...
static gchar *detachable_widget_name(GtkWidget *widget)
{
...
static gchar *detachable_page_title(GtkNotebook *notebook, GtkWidget *page)
{
    ...
    title = detachable_label_text(tab);
    if(title) {
        g_strstrip(title);
        if(*title)
            return title;
    ...
    if(GTK_IS_WIDGET(page))
        title = g_strdup((const gchar *) g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_TITLE));
    if(title) {
        g_strstrip(title);
        if(*title)
            return title;
    ...
    title = detachable_widget_name(page);
    if(title)
        return title;

    return g_strdup("Panel");
}

static GtkWidget *detachable_make_tab_label(GtkWidget *page, const gchar *title)
{
    GtkWidget *event_box = gtk_event_box_new();
    GtkWidget *label = gtk_label_new(title && *title ? title : "Panel");
...
static void detachable_install_tab(GtkNotebook *notebook, GtkWidget *page)
{
    GtkWidget *tab = NULL;
    gchar *title = NULL;
    ...
    if(GTK_IS_WIDGET(tab)) {
        if(g_object_get_data(G_OBJECT(tab), VJ_DETACH_DATA_TAB_READY)) {
            g_object_set_data(G_OBJECT(tab), VJ_DETACH_DATA_TAB_PAGE, page);
            return;
        }
    }

    title = detachable_page_title(notebook, page);
    tab = detachable_make_tab_label(page, title);
    gtk_notebook_set_tab_label(notebook, page, tab);
    gtk_widget_show_all(tab);
    g_free(title);
}

static gint detachable_window_page_count(detached_notebook_t *detached)
{
    if(!detached)
        return 0;

    return g_list_length(detached->pages);
}
...
static GtkWidget *detachable_window_current_page(detached_notebook_t *detached)
{
    gint page_num;
    ...
    return detachable_window_nth_page(detached, page_num);
}

static void detachable_window_update_title(detached_notebook_t *detached)
{
    ...
    if(count == 1) {
        page_title = detachable_page_title(NULL, detached->pages ? GTK_WIDGET(detached->pages->data) : NULL);
        title = g_strdup_printf("Reloaded - %s", page_title && *page_title ? page_title : "Detached panel");
    }
    else {
        title = g_strdup_printf("Reloaded - Detached panels (%d)", count);
    }

    gtk_window_set_title(GTK_WINDOW(detached->window), title);
    g_free(title);
    g_free(page_title);
}

static void detachable_window_update_notebook(detached_notebook_t *detached)
{
    gint count = detachable_window_page_count(detached);
    ...
    gtk_notebook_set_show_border(GTK_NOTEBOOK(detached->notebook), count > 1);
    detachable_window_update_title(detached);
}

static void detachable_present_page_window(GtkWidget *page)
{
...
static gboolean detachable_placeholder_press(GtkWidget *widget,
                                             GdkEventButton *event,
                                             gpointer user_data)
{
    GtkWidget *page = GTK_IS_WIDGET(user_data) ? GTK_WIDGET(user_data) : NULL;

    if(event && event->button == 3)
        return detachable_tab_button_press(widget, event, page);

    detachable_present_page_window(page);
    return TRUE;
}

static GtkWidget *detachable_make_placeholder(GtkWidget *page, const gchar *title)
{
    GtkWidget *event_box = gtk_event_box_new();
    GtkWidget *box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
    GtkWidget *label = gtk_label_new(NULL);
    gchar *markup = g_markup_printf_escaped("<b>%s</b>\nDetached panel\nLeft click: show window\nRight click: attach menu",
...
static void detachable_remove_placeholder_for_page(GtkNotebook *origin,
                                                   GtkWidget *page)
{
    ...
    if(gtk_widget_get_parent(placeholder) != GTK_WIDGET(origin)) {
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER, NULL);
        return;
    }

    pos = gtk_notebook_page_num(origin, placeholder);
    if(pos >= 0) {
        detachable_reordering_ = TRUE;
        gtk_notebook_remove_page(origin, pos);
        detachable_reordering_ = FALSE;
    }

    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER, NULL);
    detachable_notebook_update_visibility(origin);
}

static void detachable_create_placeholder_for_page(GtkNotebook *origin,
                                                   GtkWidget *page,
                                                   guint page_num)
{
    GtkWidget *placeholder = NULL;
    ...
    if(detachable_notebook_real_page_count(origin) <= 0) {
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER, NULL);
        detachable_notebook_update_visibility(origin);
        return;
    }

    title = detachable_page_title(origin, page);
    tab_title = g_strdup_printf("%s ↗", title && *title ? title : "Panel");
    placeholder = detachable_make_placeholder(page, title);
    tab = detachable_make_tab_label(page, tab_title);
    g_object_set_data(G_OBJECT(tab), VJ_DETACH_DATA_TAB_PAGE, page);

    n_pages = gtk_notebook_get_n_pages(origin);
    if(pos < 0)
        pos = 0;
    if(pos > n_pages)
        pos = n_pages;
    ...
    detachable_reordering_ = TRUE;
    gtk_notebook_insert_page(origin, placeholder, tab, pos);
    gtk_notebook_set_tab_detachable(origin, placeholder, FALSE);
    gtk_notebook_set_tab_reorderable(origin, placeholder, FALSE);
    gtk_widget_show_all(placeholder);
    gtk_widget_show_all(tab);
    detachable_reordering_ = FALSE;

    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER, placeholder);
    if(detachable_notebook_current_page_is_placeholder(origin))
        detachable_notebook_select_real_page(origin);
    detachable_notebook_update_visibility(origin);
...
static void detachable_notebook_prepare_page(GtkNotebook *notebook,
                                             GtkWidget *page,
                                             gint page_num)
{
    if(!notebook || !page || !GTK_IS_NOTEBOOK(notebook) || !GTK_IS_WIDGET(page))
    ...
    if(!g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN)) {
        gchar *title = detachable_page_title(notebook, page);
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN, notebook);
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_POS, GINT_TO_POINTER(page_num));
        g_object_set_data_full(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_TITLE, title, g_free);
    }

    gtk_notebook_set_tab_reorderable(notebook, page, FALSE);
    gtk_notebook_set_tab_detachable(notebook, page, FALSE);
    detachable_install_tab(notebook, page);

    if(GTK_IS_NOTEBOOK(page))
        detachable_notebook_setup(GTK_NOTEBOOK(page), NULL);
}

static void detachable_notebook_page_added(GtkNotebook *notebook,
                                           GtkWidget *page,
                                           guint page_num,
                                           gpointer user_data)
{
    GtkNotebook *origin = NULL;
    ...
    origin = GTK_IS_NOTEBOOK(origin_data) ? GTK_NOTEBOOK(origin_data) : NULL;
    if(origin == notebook) {
        gint pos = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_POS));
        detachable_remove_placeholder_for_page(origin, page);
        if(pos >= 0 && pos < gtk_notebook_get_n_pages(origin) && !detachable_reordering_) {
            detachable_reordering_ = TRUE;
            gtk_notebook_reorder_child(origin, page, pos);
            detachable_reordering_ = FALSE;
        }
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_WINDOW, NULL);
        g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED, NULL);
    }

    detachable_notebook_update_visibility(notebook);
}

static void detachable_notebook_page_removed(GtkNotebook *notebook,
                                             GtkWidget *page,
                                             guint page_num,
                                             gpointer user_data)
{
    ...
    origin_data = g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN);
    origin = GTK_IS_NOTEBOOK(origin_data) ? GTK_NOTEBOOK(origin_data) : NULL;
    if(origin == notebook && !detachable_reordering_ && !detachable_in_explicit_detach_)
        detachable_create_placeholder_for_page(origin, page, page_num);

    detachable_notebook_update_visibility(notebook);
}

static void detachable_notebook_switch_page(GtkNotebook *notebook,
                                            GtkWidget *page,
                                            guint page_num,
                                            gpointer user_data)
{
    ...
    if(GTK_IS_WIDGET(page)) {
        if(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_IS_PLACEHOLDER)) {
            GtkWidget *detached_page = GTK_WIDGET(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER_PAGE));
            detachable_present_page_window(detached_page);
        }
    }
}
...
static void detachable_notebook_setup(GtkNotebook *notebook, detached_notebook_t *detached)
{
    ...
    if(!g_object_get_data(G_OBJECT(notebook), VJ_DETACH_DATA_READY)) {
        g_signal_connect(notebook, "page-added", G_CALLBACK(detachable_notebook_page_added), NULL);
        g_signal_connect(notebook, "page-removed", G_CALLBACK(detachable_notebook_page_removed), NULL);
        g_signal_connect(notebook, "switch-page", G_CALLBACK(detachable_notebook_switch_page), NULL);
        g_object_set_data(G_OBJECT(notebook), VJ_DETACH_DATA_READY, GINT_TO_POINTER(1));
    }

    gint n_pages = gtk_notebook_get_n_pages(notebook);
    for(gint i = 0; i < n_pages; i++) {
        GtkWidget *page = gtk_notebook_get_nth_page(notebook, i);
        detachable_notebook_prepare_page(notebook, page, i);
    }

    detachable_notebook_update_visibility(notebook);
}

static detached_notebook_t *detachable_create_window(GtkNotebook *source, GtkWidget *page)
{
    GtkWidget *window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    GtkWidget *box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    GtkWidget *notebook = gtk_notebook_new();
    GtkWidget *mainw = detachable_main_window();
    detached_notebook_t *detached = g_new0(detached_notebook_t, 1);
    gchar *title = detachable_page_title(source, page);

    detached->window = window;
    detached->box = box;
    detached->notebook = notebook;
    detached->id = detachable_next_window_id_++;
    ...
    g_object_set_data(G_OBJECT(notebook), VJ_DETACH_DATA_DETACHED_NOTEBOOK, GINT_TO_POINTER(1));
    ...
    gtk_window_set_title(GTK_WINDOW(window), title && *title ? title : "Reloaded - Detached panel");
    gtk_widget_add_events(window, GDK_BUTTON_PRESS_MASK);

    if(GTK_IS_WINDOW(mainw))
        gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER_ON_PARENT);

    g_object_set_data(G_OBJECT(window), VJ_DETACH_DATA_DETACHED, detached);
    g_signal_connect(window, "delete-event", G_CALLBACK(detachable_window_delete), detached);
    g_signal_connect(window, "button-press-event", G_CALLBACK(detachable_window_button_press), detached);

    detached_notebooks_ = g_list_append(detached_notebooks_, detached);
    g_free(title);

    return detached;
}

static void detachable_detach_page_to_window(GtkNotebook *from,
                                             GtkWidget *page,
                                             detached_notebook_t *detached)
{
    GtkWidget *tab = NULL;
    ...
    title = detachable_page_title(from, page);
    tab = detachable_make_tab_label(page, title);

    detachable_preserve_hidden_descendants(page);
    g_object_ref(page);

    detachable_in_explicit_detach_ = TRUE;
    gtk_notebook_remove_page(from, page_num);
    detachable_in_explicit_detach_ = FALSE;

    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_WINDOW, detached->window);
    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED, detached);

    detached_pos = gtk_notebook_append_page(GTK_NOTEBOOK(detached->notebook), page, tab);
    gtk_notebook_set_tab_reorderable(GTK_NOTEBOOK(detached->notebook), page, TRUE);
    gtk_notebook_set_current_page(GTK_NOTEBOOK(detached->notebook), detached_pos);
    gtk_widget_show_all(tab);
    gtk_widget_show(page);

    detached->pages = g_list_append(detached->pages, page);
    samplebank_action_layout_update_for_page(page);

    detachable_create_placeholder_for_page(from, page, (guint) page_num);
    detachable_notebook_update_visibility(from);
    detachable_window_update_notebook(detached);

    gtk_widget_show(detached->notebook);
    gtk_widget_show(detached->box);
    gtk_widget_show(detached->window);

    detachable_refresh_mode_visibility();

    vj_msg(VEEJAY_MSG_INFO, "Detached panel '%s'", title && *title ? title : "Panel");

    g_object_unref(page);
    g_free(title);
}

static void detachable_reattach_page(GtkNotebook *from, GtkWidget *page)
{
    GtkNotebook *origin = NULL;
    GtkWidget *parent = NULL;
    GtkWidget *tab = NULL;
    ...
    if(parent == GTK_WIDGET(origin))
        return;

    detached = (detached_notebook_t *) g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED);
    pos = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_POS));
    detachable_remove_placeholder_for_page(origin, page);
    ...
    if(!gtk_widget_get_visible(GTK_WIDGET(origin)))
        gtk_widget_show(GTK_WIDGET(origin));

    title = detachable_page_title(NULL, page);
    tab = detachable_make_tab_label(page, title);
    menu = gtk_label_new(title && *title ? title : "Panel");
    ...
    if(detached)
        detached->pages = g_list_remove(detached->pages, page);

    gtk_notebook_insert_page_menu(origin, page, tab, menu, pos);
    detachable_notebook_prepare_page(origin, page, pos);
    gtk_notebook_set_current_page(origin, pos);
    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_WINDOW, NULL);
    g_object_set_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED, NULL);
    samplebank_action_layout_update_for_page(page);
    gtk_widget_show(page);
    ...
    if(detached)
        detachable_window_update_notebook(detached);
    detachable_refresh_mode_visibility();
    vj_msg(VEEJAY_MSG_INFO, "Reattached panel '%s'", title && *title ? title : "Panel");

    g_object_unref(page);
    g_free(title);
}

static void detachable_reattach_all(detached_notebook_t *detached)
{
    GList *copy;
    ...
    copy = g_list_copy(detached->pages);
    for(GList *node = copy; node; node = node->next)
        if(GTK_IS_WIDGET(node->data))
            detachable_reattach_page(NULL, GTK_WIDGET(node->data));
    g_list_free(copy);
}

static gboolean detachable_window_delete(GtkWidget *window,
                                         GdkEvent *event,
                                         gpointer user_data)
{
    detached_notebook_t *detached = (detached_notebook_t *) user_data;
    (void) event;

    detachable_reattach_all(detached);
    detached_notebooks_ = g_list_remove(detached_notebooks_, detached);

    if(GTK_IS_WIDGET(window))
        gtk_widget_destroy(window);

    if(detached) {
        g_list_free(detached->pages);
        g_free(detached);
    }
    return TRUE;
}

static void detachable_destroy_empty_window(detached_notebook_t *detached)
{
    if(!detached)
        return;

    if(detachable_window_page_count(detached) > 0)
        return;

    detached_notebooks_ = g_list_remove(detached_notebooks_, detached);
    if(GTK_IS_WIDGET(detached->window))
        gtk_widget_destroy(detached->window);
    g_list_free(detached->pages);
    g_free(detached);
}

static void detachable_detach_page(GtkNotebook *from, GtkWidget *page)
{
    detached_notebook_t *detached = NULL;
    ...
    if(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_IS_PLACEHOLDER)) {
        GtkWidget *real_page = GTK_WIDGET(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_PLACEHOLDER_PAGE));
        detachable_present_page_window(real_page);
        return;
    }

    if(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_WINDOW)) {
        detachable_present_page_window(page);
        return;
    }

    if(gtk_notebook_page_num(from, page) < 0)
        return;

    detached = detachable_create_window(from, page);
    detachable_detach_page_to_window(from, page, detached);
    gtk_window_present(GTK_WINDOW(detached->window));
}

static void detachable_menu_detach(GtkMenuItem *item, gpointer user_data)
{
    GtkWidget *page = GTK_IS_WIDGET(user_data) ? GTK_WIDGET(user_data) : NULL;
    ...
    if(GTK_IS_NOTEBOOK(parent))
        detachable_detach_page(GTK_NOTEBOOK(parent), page);
}

static void detachable_target_action_free(gpointer user_data, GClosure *closure)
{
    detachable_target_action_t *action = (detachable_target_action_t *) user_data;
    (void) closure;

    if(!action)
        return;

    if(GTK_IS_WIDGET(action->page))
        g_object_unref(action->page);
    g_free(action);
}

static void detachable_menu_detach_to_window(GtkMenuItem *item, gpointer user_data)
{
    detachable_target_action_t *action = (detachable_target_action_t *) user_data;
    GtkWidget *page = action ? action->page : NULL;
    GtkWidget *parent = GTK_IS_WIDGET(page) ? gtk_widget_get_parent(page) : NULL;
    (void) item;

    if(page) {
        if(GTK_IS_NOTEBOOK(parent)) {
            if(action->detached) {
                detachable_detach_page_to_window(GTK_NOTEBOOK(parent), page, action->detached);
                gtk_window_present(GTK_WINDOW(action->detached->window));
            }
        }
    }
}

static void detachable_menu_reattach(GtkMenuItem *item, gpointer user_data)
{
    GtkWidget *page = GTK_IS_WIDGET(user_data) ? GTK_WIDGET(user_data) : NULL;
    detached_notebook_t *detached = NULL;
    (void) item;
    ...
    detached = (detached_notebook_t *) g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED);
    detachable_reattach_page(NULL, page);
    detachable_destroy_empty_window(detached);
}

static void detachable_menu_show(GtkMenuItem *item, gpointer user_data)
{
    GtkWidget *page = GTK_IS_WIDGET(user_data) ? GTK_WIDGET(user_data) : NULL;
    (void) item;
    detachable_present_page_window(page);
...
static void detachable_append_existing_window_menu(GtkWidget *menu, GtkWidget *page)
{
    ...
    for(GList *node = detached_notebooks_; node; node = node->next) {
        detached_notebook_t *detached = (detached_notebook_t *) node->data;
        detachable_target_action_t *action = NULL;
        GtkWidget *item = NULL;
        ...
        window_title = gtk_window_get_title(GTK_WINDOW(detached->window));
        label = g_strdup_printf("%s  [%d page%s]",
                                window_title && *window_title ? window_title : "Detached window",
                                detachable_window_page_count(detached),
                                detachable_window_page_count(detached) == 1 ? "" : "s");
        item = gtk_menu_item_new_with_label(label);
        action = g_new0(detachable_target_action_t, 1);
        action->page = page;
        action->detached = detached;
        g_object_ref(page);
        g_signal_connect_data(item,
                              "activate",
                              G_CALLBACK(detachable_menu_detach_to_window),
                              action,
                              detachable_target_action_free,
                              0);
    ...
    root = gtk_menu_item_new_with_label("Detach page to existing window");
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(root), submenu);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), root);
}

static void detachable_popup_for_page(GtkWidget *page, GdkEventButton *event)
{
    GtkWidget *menu = NULL;
    GtkWidget *item = NULL;
    GtkWidget *parent = NULL;
    ...
    if(is_placeholder || g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_WINDOW)) {
        item = gtk_menu_item_new_with_label("Show detached window");
        g_signal_connect(item, "activate", G_CALLBACK(detachable_menu_show), page);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

        item = gtk_menu_item_new_with_label("Reattach page");
        g_signal_connect(item, "activate", G_CALLBACK(detachable_menu_reattach), page);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    }
    else if(GTK_IS_NOTEBOOK(parent) && (!origin || origin == GTK_NOTEBOOK(parent))) {
        item = gtk_menu_item_new_with_label("Detach page to new window");
        g_signal_connect(item, "activate", G_CALLBACK(detachable_menu_detach), page);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
        detachable_append_existing_window_menu(menu, page);
    }
    else if(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_DETACHED)) {
        item = gtk_menu_item_new_with_label("Reattach page");
        g_signal_connect(item, "activate", G_CALLBACK(detachable_menu_reattach), page);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    }
    ...
    g_object_ref(page);
    g_object_set_data_full(G_OBJECT(menu), VJ_DETACH_DATA_TAB_PAGE, page, g_object_unref);
...
static gboolean detachable_tab_button_press(GtkWidget *widget,
                                            GdkEventButton *event,
                                            gpointer user_data)
{
    GtkWidget *page = GTK_IS_WIDGET(user_data) ? GTK_WIDGET(user_data) : NULL;
    ...
    if(GTK_IS_WIDGET(widget)) {
        if(g_object_get_data(G_OBJECT(widget), VJ_DETACH_DATA_TAB_PAGE))
            page = GTK_WIDGET(g_object_get_data(G_OBJECT(widget), VJ_DETACH_DATA_TAB_PAGE));
    }
...
static gboolean detachable_window_button_press(GtkWidget *widget,
                                               GdkEventButton *event,
                                               gpointer user_data)
{
    detached_notebook_t *detached = (detached_notebook_t *) user_data;
    ...
    if(!event || event->button != 3 || !detached)
        return FALSE;
...
static gchar *detachable_window_config_path(void)
{
    const gchar *home = g_get_home_dir();
    gchar *dir = home ? g_build_filename(home, ".veejay", NULL) : NULL;
    gchar *path = NULL;

    if(dir)
        g_mkdir_with_parents(dir, 0700);

    path = g_build_filename(dir ? dir : g_get_user_config_dir(),
                            "reloaded-layout.ini",
                            NULL);
    g_free(dir);
...
static const gchar *detachable_buildable_name(GtkWidget *widget)
{
    if(GTK_IS_WIDGET(widget)) {
        if(GTK_IS_BUILDABLE(widget)) {
            const gchar *name = gtk_buildable_get_name(GTK_BUILDABLE(widget));
...
static void detachable_key_file_set_window(GKeyFile *kf, const gchar *group, GtkWidget *window)
{
    gint x = 0;
    gint y = 0;
    gint w = 0;
    ...
    g_key_file_set_integer(kf, group, "x", x);
    g_key_file_set_integer(kf, group, "y", y);
    g_key_file_set_integer(kf, group, "width", w);
    g_key_file_set_integer(kf, group, "height", h);
}

static void detachable_save_page_group(GKeyFile *kf, const gchar *group, GtkWidget *page)
{
    gpointer origin_data = NULL;
    GtkNotebook *origin = NULL;
    gchar *title = detachable_page_title(NULL, page);

    if(!kf || !group)
        return;

    origin_data = GTK_IS_WIDGET(page) ? g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN) : NULL;
    origin = GTK_IS_NOTEBOOK(origin_data) ? GTK_NOTEBOOK(origin_data) : NULL;

    g_key_file_set_string(kf, group, "title", title && *title ? title : "Panel");
    g_key_file_set_string(kf, group, "page", detachable_buildable_name(page));
    g_key_file_set_string(kf, group, "origin", origin ? detachable_buildable_name(GTK_WIDGET(origin)) : "");
    g_key_file_set_integer(kf, group, "origin-position",
                           GTK_IS_WIDGET(page) ?
                           GPOINTER_TO_INT(g_object_get_data(G_OBJECT(page), VJ_DETACH_DATA_ORIGIN_POS)) : 0);
    g_free(title);
}

static void detachable_save_window_configuration(GtkMenuItem *item, gpointer user_data)
{
    GKeyFile *kf = g_key_file_new();
    gchar *path = detachable_window_config_path();
    gchar *data = NULL;
    gsize len = 0;
    GError *error = NULL;
    GtkWidget *mainw = detachable_main_window();
    ...
    g_key_file_set_string(kf, "layout", "format", "gveejay-detached-windows-v2");

    if(GTK_IS_WINDOW(mainw))
        detachable_key_file_set_window(kf, "main-window", mainw);

    count = g_list_length(detached_notebooks_);
    g_key_file_set_integer(kf, "layout", "detached-count", count);

    for(GList *node = detached_notebooks_; node; node = node->next) {
        detached_notebook_t *detached = (detached_notebook_t *) node->data;
        gchar *group = g_strdup_printf("detached-%d", idx);
        gint page_count = detachable_window_page_count(detached);

        if(detached) {
            if(GTK_IS_WINDOW(detached->window))
                detachable_key_file_set_window(kf, group, detached->window);
        }

        g_key_file_set_integer(kf, group, "page-count", page_count);

        for(gint p = 0; p < page_count; p++) {
            GtkWidget *page = detachable_window_nth_page(detached, p);
            gchar *page_group = g_strdup_printf("detached-%d-page-%d", idx, p);
            detachable_save_page_group(kf, page_group, page);
            g_free(page_group);
        }
    ...
    if(data && path && g_file_set_contents(path, data, len, &error))
        vj_msg(VEEJAY_MSG_INFO, "Saved window configuration to %s", path);
    else
        vj_msg(VEEJAY_MSG_WARNING, "Unable to save window configuration%s%s",
               error && error->message ? ": " : "",
               error && error->message ? error->message : "");
...
static gboolean detachable_key_file_get_integer(GKeyFile *kf, const gchar *group, const gchar *key, gint *value)
{
    GError *error = NULL;
    gint v;

    if(!kf || !group || !key || !value || !g_key_file_has_key(kf, group, key, NULL))
        return FALSE;

    v = g_key_file_get_integer(kf, group, key, &error);
    if(error) {
        g_error_free(error);
        return FALSE;
    }
...
static void detachable_apply_window_geometry(GKeyFile *kf, const gchar *group, GtkWidget *window)
{
    gint x = -1;
    gint y = -1;
    gint w = -1;
    ...
    detachable_key_file_get_integer(kf, group, "y", &y);
    detachable_key_file_get_integer(kf, group, "width", &w);
    detachable_key_file_get_integer(kf, group, "height", &h);
...
static GtkWidget *detachable_find_widget_by_name(const gchar *name)
{
    if(!info || !info->main_window || !name || !*name)
        return NULL;

    return glade_xml_get_widget_(info->main_window, name);
}

static GtkWidget *detachable_find_page_from_config(GKeyFile *kf, const gchar *group)
{
    gchar *page_name = NULL;
    gchar *origin_name = NULL;
    GtkWidget *page = NULL;
    GtkWidget *origin_widget = NULL;
    gint pos = -1;
    GError *error = NULL;

    if(!kf || !group || !g_key_file_has_group(kf, group))
        return NULL;

    page_name = g_key_file_get_string(kf, group, "page", &error);
    if(error) {
        g_error_free(error);
        error = NULL;
    }
    ...
    if(page_name && *page_name)
        page = detachable_find_widget_by_name(page_name);

    origin_name = g_key_file_get_string(kf, group, "origin", &error);
    if(error) {
        g_error_free(error);
        error = NULL;
    }

    detachable_key_file_get_integer(kf, group, "origin-position", &pos);
    if(!page && origin_name && *origin_name && pos >= 0) {
        origin_widget = detachable_find_widget_by_name(origin_name);
        if(GTK_IS_NOTEBOOK(origin_widget))
            page = gtk_notebook_get_nth_page(GTK_NOTEBOOK(origin_widget), pos);
    }
...
static gboolean detachable_restore_page_group(GKeyFile *kf,
                                              const gchar *group,
                                              detached_notebook_t **detached)
{
    ...
    if(!*detached)
        *detached = detachable_create_window(GTK_NOTEBOOK(parent), page);

    detachable_detach_page_to_window(GTK_NOTEBOOK(parent), page, *detached);
    return TRUE;
}
...
static void detachable_restore_window_configuration(GtkMenuItem *item, gpointer user_data)
{
    ...
    GError *error = NULL;
    gint count = 0;
    gint restored_windows = 0;
    GtkWidget *mainw = detachable_main_window();
    (void) item;
    (void) user_data;

    if(!path || !g_key_file_load_from_file(kf, path, G_KEY_FILE_NONE, &error)) {
        vj_msg(VEEJAY_MSG_DEBUG, "No saved window configuration found at %s", path ? path : "(null)");
        if(error)
            g_error_free(error);
        g_free(path);
        g_key_file_free(kf);
    ...
    if(GTK_IS_WINDOW(mainw))
        detachable_apply_window_geometry(kf, "main-window", mainw);

    detachable_key_file_get_integer(kf, "layout", "detached-count", &count);
    for(gint i = 0; i < count; i++) {
        gchar *group = g_strdup_printf("detached-%d", i);
        detached_notebook_t *detached = NULL;
        gint page_count = 0;
        gboolean restored_any = FALSE;
        ...
        if(!detachable_key_file_get_integer(kf, group, "page-count", &page_count))
            page_count = 1;

        for(gint p = 0; p < page_count; p++) {
            gchar *page_group = page_count == 1 && g_key_file_has_key(kf, group, "page", NULL) ?
                g_strdup(group) : g_strdup_printf("detached-%d-page-%d", i, p);
            if(detachable_restore_page_group(kf, page_group, &detached))
                restored_any = TRUE;
            g_free(page_group);
        }
        ...
        if(detached) {
            if(restored_any) {
                if(GTK_IS_WINDOW(detached->window)) {
                    detachable_apply_window_geometry(kf, group, detached->window);
                    gtk_widget_show(detached->notebook);
                    gtk_widget_show(detached->box);
                    gtk_widget_show(detached->window);
                    detachable_window_update_notebook(detached);
    ...
    detachable_refresh_mode_visibility();
    vj_msg(VEEJAY_MSG_INFO, "Restored %d detached panel window%s from %s",
           restored_windows,
           restored_windows == 1 ? "" : "s",
           path);
...
static void detachable_restore_default_window_configuration(GtkMenuItem *item, gpointer user_data)
{
    ...
    if(path && *path && g_file_test(path, G_FILE_TEST_EXISTS)) {
        if(unlink(path) == 0)
            vj_msg(VEEJAY_MSG_INFO, "Removed saved window configuration %s", path);
        else
            vj_msg(VEEJAY_MSG_WARNING, "Unable to remove saved window configuration %s", path);
    }

    vj_msg(VEEJAY_MSG_INFO, "Restored default window configuration: all notebook pages are attached and the main window was resized to its minimum layout");
    g_free(path);
}
...
static void appearance_preferences_show(GtkMenuItem *item, gpointer user_data)
{
    ...
    ctx = appearance_dialog_new(GTK_IS_WINDOW(mainw) ? GTK_WINDOW(mainw) : NULL);
...
static GtkWidget *window_configuration_menu_item_new(const char *name,
                                                     const char *label,
                                                     const char *tooltip,
                                                     GCallback cb)
{
...
static void init_window_configuration_menu(void)
{
    ...
    menu_item = glade_xml_get_widget_(info->main_window, "menuitem_preference");
    if(GTK_IS_MENU_ITEM(menu_item))
        menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(menu_item));

    if(!menu)
        menu = glade_xml_get_widget_(info->main_window, "menu_preference");

    if(!menu_item && menu)
        menu_item = gtk_widget_get_parent(menu);
    ...
    if(!menu_shell_find_item_named(layout_menu, "save_window_configuration")) {
        GtkWidget *item = window_configuration_menu_item_new(
            "save_window_configuration",
            "Save Window Configuration",
            "Save the current main-window size and detached panel windows to ~/.veejay/reloaded-layout.ini",
            G_CALLBACK(detachable_save_window_configuration));
        gtk_menu_shell_append(GTK_MENU_SHELL(layout_menu), item);
    }

    if(!menu_shell_find_item_named(layout_menu, "restore_window_configuration")) {
        GtkWidget *item = window_configuration_menu_item_new(
            "restore_window_configuration",
            "Restore Window Configuration",
            "Restore detached panel windows from ~/.veejay/reloaded-layout.ini",
            G_CALLBACK(detachable_restore_window_configuration));
        gtk_menu_shell_append(GTK_MENU_SHELL(layout_menu), item);
    }

    if(!menu_shell_find_item_named(layout_menu, "restore_default_window_configuration_separator")) {
        GtkWidget *sep = gtk_separator_menu_item_new();
        gtk_widget_set_name(sep, "restore_default_window_configuration_separator");
        gtk_menu_shell_append(GTK_MENU_SHELL(layout_menu), sep);
    }

    if(!menu_shell_find_item_named(layout_menu, "restore_default_window_configuration")) {
        GtkWidget *item = window_configuration_menu_item_new(
            "restore_default_window_configuration",
            "Restore Default Window Configuration",
            "Reattach all notebook pages and remove ~/.veejay/reloaded-layout.ini",
            G_CALLBACK(detachable_restore_default_window_configuration));
        gtk_menu_shell_append(GTK_MENU_SHELL(layout_menu), item);
    }

    if(GTK_IS_WIDGET(menu_item))
...
static void detachable_notebooks_init(void)
{
    if(!info || !info->main_window)
        return;
    ...
    for(GSList *node = objects; node; node = node->next) {
        if(GTK_IS_NOTEBOOK(node->data))
            detachable_notebook_setup(GTK_NOTEBOOK(node->data), NULL);
    }
...
static void detachable_windows_close_all(void)
{
    while(detached_notebooks_) {
        detached_notebook_t *detached = (detached_notebook_t *) detached_notebooks_->data;

        if(detached) {
            if(GTK_IS_WIDGET(detached->window)) {
                detachable_window_delete(detached->window, NULL, detached);
                continue;
            }
        }

        detached_notebooks_ = g_list_delete_link(detached_notebooks_, detached_notebooks_);
        if(detached) {
            g_list_free(detached->pages);
            g_free(detached);
        }
                 ...
                 gboolean fasterui)
{
    ...
    GtkWidget *box = samplebank_action_layout_mount();
    if(!box)
        box = glade_xml_get_widget_(info->main_window, "sample_bank_hbox");
    ...
    if(!info->sample_bank_view)
        ...
        gtk_widget_set_size_request(info->sample_bank_view, -1, sample_bank_view_h);
   ...
   // set_tooltip_by_widget(info->tl, tooltips[TOOLTIP_TIMELINE].text );
    ...
    if(img_wid) {
        add_class(img_wid, "preview-image-surface");
        g_signal_connect(G_OBJECT(img_wid),
                         "draw",
                         G_CALLBACK(preview_image_draw),
                         NULL);
    }
    ...
    if(GTK_IS_COMBO_BOX(shape_combo)) {
    ...
    if(GTK_IS_SPIN_BUTTON(shape_steps)) {
    ...
    if(GTK_IS_SPIN_BUTTON(shape_seed)) {
    ...
    if(GTK_IS_SPIN_BUTTON(shape_detail)) {
...
int vj_gui_reconnect(char *hostname,char *group_name, int port_num)
{
    ...
    update_slider_gvalue("framerate", ui_clampd((gdouble)info->el.fps, 0.0, 120.0));
...
static void veejay_show_main_ui(vj_gui_t *gui)
{
    ...
    if(info->client &&
       info->watch.state == STATE_PLAYING &&
       !g_object_get_data(G_OBJECT(mw), VJ_DETACH_DATA_RESTORE_ONCE)) {
        g_object_set_data(G_OBJECT(mw), VJ_DETACH_DATA_RESTORE_ONCE, GINT_TO_POINTER(1));
        detachable_restore_window_configuration(NULL, NULL);
    }
}
...
static void sequence_vims_append_pattern_tooltip(char *text,
                                                  size_t text_size,
                                                  int bank,
                                                  int slot)
{
    ...
    if(summary.source_events > 0) {
        used += snprintf(text + used,
                         text_size - used,
                         "\n\nSource pattern: %u event%s on %u frame%s%s",
                         summary.source_events,
                         summary.source_events == 1 ? "" : "s",
                         summary.source_rows,
                         summary.source_rows == 1 ? "" : "s",
                         (summary.flags & GVR_SEQUENCE_PATTERN_SOURCE_HOLD) ?
                            " (includes Hold Frame)" : "");
    }

    if(summary.cell_events > 0 && used < text_size) {
        used += snprintf(text + used,
                         text_size - used,
                         "\nCell overlay: %u event%s on %u frame%s%s",
                         summary.cell_events,
                         summary.cell_events == 1 ? "" : "s",
                         summary.cell_rows,
                         summary.cell_rows == 1 ? "" : "s",
                         (summary.flags & GVR_SEQUENCE_PATTERN_CELL_HOLD) ?
                            " (includes Hold Frame)" : "");
    }

    if(summary.source_events > 0 && used < text_size) {
        used += snprintf(text + used,
                         text_size - used,
                         "\nThe source pattern follows this sample or stream wherever it is used.");
    }

    if(summary.cell_events > 0 && used < text_size) {
        used += snprintf(text + used,
                         text_size - used,
                         "\nThe cell overlay applies only to this sequence slot.");
    }

    if(used < text_size) {
        snprintf(text + used,
                 text_size - used,
                 "\nCyan corner: source pattern · Magenta corner: cell overlay · Amber dot: Hold Frame");
    }
}
                                                    ...
                                                    gpointer user_data)
{
    ...
    if(header) {
        ...
        GvrSequenceBankPatternSummary pattern = { 0 };
        gvr_sequence_bank_view_get_bank_pattern_summary(widget,
                                                        bank,
                                                        &pattern);

        g_strlcat(header_tip,
                  "\n\nPattern target: clicking this bank header edits the complete bank sequence pattern. Click a cell to edit only that cell overlay.",
                  sizeof(header_tip));

        if(pattern.event_count > 0) {
            char pattern_text[256];

            if(pattern.timeline_known)
                snprintf(pattern_text,
                         sizeof(pattern_text),
                         "\nSequence pattern: %u event%s on %u frame%s · timeline %lld frames%s",
                         pattern.event_count,
                         pattern.event_count == 1 ? "" : "s",
                         pattern.row_count,
                         pattern.row_count == 1 ? "" : "s",
                         (long long)pattern.timeline_length,
                         (pattern.flags & GVR_SEQUENCE_PATTERN_HAS_HOLD) ?
                            " · includes Hold Frame" : "");
            else
                snprintf(pattern_text,
                         sizeof(pattern_text),
                         "\nSequence pattern: %u event%s on %u frame%s · timeline unavailable%s",
                         pattern.event_count,
                         pattern.event_count == 1 ? "" : "s",
                         pattern.row_count,
                         pattern.row_count == 1 ? "" : "s",
...
static GtkWidget *sequence_preview_pack_button(GtkWidget *column,
                                                  const char *name)
{
    GtkWidget *button = glade_xml_get_widget_(info->main_window, name);
    GtkWidget *parent;
    ...
    if(parent) {
        ...
        gtk_box_pack_start(GTK_BOX(column), button, FALSE, FALSE, 1);
...
static void sequence_preview_mount_action_columns(void)
{
    static const char *right_buttons[] = {
        "button_054",
        "osdbutton",
    ...
    GtkWidget *frame = glade_xml_get_widget_(info->main_window,
                                             "frame327");
    GtkWidget *legacy_media = glade_xml_get_widget_(info->main_window,
                                                    "vbox418");
    GtkWidget *center;
    GtkWidget *row;
    GtkWidget *right;
    ...
    g_object_ref(center);
    gtk_container_remove(GTK_CONTAINER(frame), center);

    row = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
    right = gtk_box_new(GTK_ORIENTATION_VERTICAL, 1);

    add_class(row, "preview-action-row");
    add_class(right, "preview-action-column");
    ...
    if(legacy_media) {
        gtk_widget_set_no_show_all(legacy_media, TRUE);
        gtk_widget_hide(legacy_media);
    }
...
static void sequence_vims_history_activate(GtkWidget *widget,
                                           const gchar *message,
                                           gpointer user_data)
{
    (void)widget;
    ...
    sequence_vims_sync_target();
    if(!gvr_vims_pattern_view_insert_message(
           info->vims_pattern_view,
           message,
           -1,
           -1,
    ...
    {
        vj_msg(VEEJAY_MSG_INFO,
               "Select an editable Pattern target before placing a VIMS history command");
    }
}

static gboolean sequence_vims_mount_history_page(GtkWidget *history_view)
{
    GtkWidget *page = glade_xml_get_widget_(info->main_window,
                                            "vbox498");
    GtkWidget *tab = glade_xml_get_widget_(info->main_window,
                                           "label877");
    GList *children;
    ...
    if(GTK_IS_LABEL(tab))
        gtk_label_set_markup(GTK_LABEL(tab),
                             "<b>VIMS History</b>");
    ...
    add_class(page, "vims-history-page");
    add_class(history_view, "vims-history-view");
    gtk_widget_set_hexpand(history_view, TRUE);
    gtk_widget_set_vexpand(history_view, TRUE);
...
static gboolean sequence_vims_mount_pattern_page(GtkWidget *pattern_view)
{
    GtkWidget *page = glade_xml_get_widget_(info->main_window, "vbox649");
    ...
    if(GTK_IS_LABEL(tab))
        gtk_label_set_markup(GTK_LABEL(tab), "<b>Pattern</b>");
    ...
    add_class(page, "vims-pattern-page");
    add_class(pattern_view, "vims-pattern-view");
    gtk_widget_set_hexpand(pattern_view, TRUE);
    gtk_widget_set_vexpand(pattern_view, TRUE);
    gtk_box_pack_start(GTK_BOX(page), pattern_view, TRUE, TRUE, 0);
...
static void create_sequencer_slots(int nx, int ny)
{
    ...
    info->vims_pattern_view = gvr_vims_pattern_view_new();
    gvr_vims_pattern_view_set_description_lookup(
        info->vims_pattern_view,
        sequence_vims_description_lookup,
        NULL,
        NULL);
    gtk_widget_set_size_request(info->vims_pattern_view, sequence_view_w, sequence_view_h);
    if(!sequence_vims_mount_pattern_page(info->vims_pattern_view)) {
        GtkWidget *fallback = gtk_frame_new("Pattern");
        add_class(fallback, "vims-pattern-fallback");
        gtk_container_add(GTK_CONTAINER(fallback), info->vims_pattern_view);
        gtk_box_pack_start(GTK_BOX(outer), fallback, TRUE, TRUE, compact ? 0 : 2);
        gtk_widget_show(fallback);
        vj_msg(VEEJAY_MSG_ERROR, "Unable to locate Pattern page container vbox649; mounted Pattern editor in SEQ as fallback");
    }

    sequence_preview_mount_action_columns();

    info->vims_history_view = gvr_vims_history_view_new();
    gtk_widget_set_size_request(info->vims_history_view,
                                sequence_view_w,
                                sequence_view_h);
    g_signal_connect(info->vims_history_view,
                     "message-activated",
                     G_CALLBACK(sequence_vims_history_activate),
                     NULL);
    if(!sequence_vims_mount_history_page(info->vims_history_view)) {
        vj_msg(VEEJAY_MSG_ERROR,
               "Unable to locate Veejay page container vbox498 for VIMS History");
    }
    ...
    g_signal_connect_after(G_OBJECT(info->sequence_bank_view), "button-release-event", G_CALLBACK(sequence_vims_grid_button_release), NULL);
    g_signal_connect_after(G_OBJECT(info->sequence_bank_view), "key-release-event", G_CALLBACK(sequence_vims_grid_key_release), NULL);
