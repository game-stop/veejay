...
static void callback_set_tooltip_if_changed(GtkWidget *w, const char *text)
{
    const char *cached;
    ...
    cached = g_object_get_data(G_OBJECT(w), "gvr-callback-tooltip-cache");
    if(g_strcmp0(cached, text) == 0)
        return;

    vj_gui_widget_set_tooltip_text(w, text);
    g_object_set_data_full(G_OBJECT(w),
                           "gvr-callback-tooltip-cache",
                           g_strdup(text),
                           g_free);
}
...
{
    callback_set_tooltip_if_changed(widget_cache[widget_id], text);
}
...
void	on_toggle_fademethod_toggled(GtkWidget *w, gpointer user_data)
{
	...
	vj_msg(VEEJAY_MSG_INFO,
           "Chain fader uses %s",
           enabled ? "the processed alpha mask" : "normal opacity");
}
...
void	on_button_sample_recordstart_clicked(GtkWidget *widget, gpointer user_data)
{
	...
	if(!callback_playmode_is_sample(info->status_tokens[PLAY_MODE]) ||
	   info->status_tokens[CURRENT_ID] <= 0) {
		vj_msg(VEEJAY_MSG_WARNING, "Play a sample before starting Record to Disk");
		g_free(format);
		return;
	}
	...
	if( autoplay ) {
		vj_msg(VEEJAY_MSG_INFO, "Recording %s from the playing sample; the new sample will play when finished", time1);
	} else {
		vj_msg(VEEJAY_MSG_INFO, "Recording %s from the playing sample", time1);
	}
	free(time1);
	g_free(format);
}
...
static void alpha_composite_update_ui(int enabled, int alpha_value)
{
    GtkWidget *init = glade_xml_get_widget_(info->main_window, "alphacomposite");
    GtkWidget *fill = glade_xml_get_widget_(info->main_window, "toggle_alpha255");

    if(init) {
        callback_set_tooltip_if_changed(
            init,
            enabled
                ? "Initialize alpha when an active FX needs it. Disable to use only alpha generated by effects or preserved from earlier frames."
                : "Alpha initialization is disabled. The performer stays on its non-alpha path until an alpha-producing effect or alpha blend needs the plane.");
    }

    if(fill) {
        gtk_widget_set_sensitive(fill, enabled ? TRUE : FALSE);

        if(!enabled) {
            callback_set_tooltip_if_changed(
                fill,
                "Initial alpha value is unused while alpha initialization is disabled.");
        }
        else if(alpha_value <= 0) {
            callback_set_tooltip_if_changed(
                fill,
                "Initial alpha is transparent (0). Toggle on for opaque (255).");
        }
        else if(alpha_value >= 255) {
            callback_set_tooltip_if_changed(
                fill,
                "Initial alpha is opaque (255). Toggle off for transparent (0).");
        }
        else {
            gchar *tip = g_strdup_printf(
                "Backend initial alpha is %d. Toggle off for transparent (0) or on for opaque (255).",
                alpha_value);
            callback_set_tooltip_if_changed(fill, tip);
            g_free(tip);
        }
    }
}
...
void alpha_composite_sync_from_status(int enabled, int alpha_value)
{
    ...
    alpha_composite_update_ui(enabled, alpha_value);
    info->status_lock = old_lock;
}
...
static void alpha_composite_send_ui_state(const char *midi_widget)
{
    ...
    if(enabled) {
        vj_msg(VEEJAY_MSG_INFO,
               "Alpha initialization enabled: %s (%d)",
               alpha_value == 255 ? "opaque" : "transparent",
               alpha_value);
    }
    else {
        vj_msg(VEEJAY_MSG_INFO,
               "Alpha initialization disabled; existing and generated alpha is preserved");
    }
}
...
static void mixing_source_render_ensure_global(void)
{
    if(is_button_toggled("toggle_subrender"))
        return;

    multi_vims(VIMS_SUB_RENDER, "%d %d", 0, 1);
    info->uc.reload_hint[HINT_ENTRY] = 1;
    info->uc.reload_hint[HINT_CHAIN] = 1;

    vj_msg(VEEJAY_MSG_INFO,
           "Enabling the mixing-source FX master switch for this entry");
}
                                       ...
                                       int model_column)
{
    ...
    switch(model_column)
    {
        ...
        case FXC_SUBRENDER:
            ...
            next = active ? 0 : 1;
            if(next)
                mixing_source_render_ensure_global();
...
{
    single_vims( VIMS_STREAM_OFFLINE_REC_STOP );
    vj_msg(VEEJAY_MSG_INFO, "Stopped stream append capture");
}
...
void on_button_offline_start_sample_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    if(!callback_playmode_is_sample(info->status_tokens[PLAY_MODE]) || cur_id <= 0) {
        vj_msg(VEEJAY_MSG_WARNING, "Select the sample that should receive the stream append capture first");
        return;
    }

    if(stream_id <= 0) {
        vj_msg(VEEJAY_MSG_WARNING, "Stream append capture needs a valid source stream ID");
        return;
    }

    if(n_frames <= 0) {
        vj_msg(VEEJAY_MSG_WARNING, "Stream append capture needs a positive chunk length");
        return;
    }

    multi_vims( VIMS_STREAM_OFFLINE_REC_START, "%d %d %d %d", stream_id, n_frames, 0, cur_id );

    vj_msg(VEEJAY_MSG_INFO, "Started continuous video-only append capture: stream %d to sample %d in %d-frame chunks",
            stream_id, cur_id, n_frames );
}
