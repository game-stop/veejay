...
#define GTK_CUSTOM_PALETTE_WIDTH 5
                       ...
                       const gchar       *tooltip)
{
  ...
  *spinbutton = gtk_spin_button_new (adjust, 10.0, 0);
  gtk_widget_set_size_request(*spinbutton,42,-1);  
		    ...
		    gint               j)
{
  ...
  gtk_widget_set_size_request (priv->custom_palette[i][j], CUSTOM_PALETTE_ENTRY_WIDTH, 0.75*CUSTOM_PALETTE_ENTRY_HEIGHT);
...
gtk_color_selection_init (GtkColorSelection *colorsel)
{
  ...
  gtk_hsv_set_metrics (GTK_HSV (priv->triangle_colorsel), 145, 15);
  ...
  gtk_box_pack_start (GTK_BOX (top_hbox), top_right_vbox, TRUE, TRUE, 0);

  table = gtk_table_new (6, 4, FALSE);    
  gtk_table_set_row_spacings (GTK_TABLE (table), 4);
  gtk_table_set_col_spacings (GTK_TABLE (table), 4);  
  gtk_box_pack_start (GTK_BOX (top_right_vbox), table, TRUE, TRUE, 0);
  
  make_label_spinbutton (colorsel, &priv->hue_spinbutton, _("H:"), table, 0, 0, COLORSEL_HUE,
                         _("Position on the color wheel."));
  gtk_spin_button_set_wrap (GTK_SPIN_BUTTON (priv->hue_spinbutton), TRUE);
  make_label_spinbutton (colorsel, &priv->sat_spinbutton, _("S:"), table, 1, 0, COLORSEL_SATURATION,
                         _("\"Deepness\" of the color."));
  make_label_spinbutton (colorsel, &priv->val_spinbutton, _("V:"), table, 2, 0, COLORSEL_VALUE,
                         _("Brightness of the color."));
  make_label_spinbutton (colorsel, &priv->red_spinbutton, _("R:"), table, 0, 1, COLORSEL_RED,
                         _("Amount of red light in the color."));
  make_label_spinbutton (colorsel, &priv->green_spinbutton, _("G:"), table, 1, 1, COLORSEL_GREEN,
                         _("Amount of green light in the color."));
  make_label_spinbutton (colorsel, &priv->blue_spinbutton, _("B:"), table, 2, 1, COLORSEL_BLUE,
  ...
  for (i = 0; i < GTK_CUSTOM_PALETTE_WIDTH; i++)
	...
	{
	  make_palette_frame (colorsel, palette_table, i, j);
	}
    }
  set_selected_palette (colorsel, 0, 0);
  priv->palette_frame = gtk_frame_new (NULL);
  label = gtk_label_new_with_mnemonic (_("Palette"));
  ...
  gtk_box_pack_start (GTK_BOX (vbox), palette_table, TRUE, TRUE, 0);
