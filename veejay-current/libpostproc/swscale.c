...
#include <libpostproc/swscale_internal.h>
#include <libyuv/yuvconv.h>
#include <libpostproc/rgb2rgb.h>
...
#if (defined (HAVE_ASM_MMX) && !defined (HAVE_ASM_3DNOW) && !defined (HAVE_ASM_MMX2)) 
#define COMPILE_MMX
#endif

#if defined (HAVE_ASM_MMX2)  
#define COMPILE_MMX2
#endif

#if (defined (HAVE_ASM_3DNOW) && !defined (HAVE_ASM_MMX2)) 
	...
	for(i=0; i<dstW/numSplits; i++)
	{
		...
		if((i&3) == 0)
		{
			...
			if(d+1<4)
			{
				...
				veejay_memcpy(funnyCode + fragmentPos, fragmentB, fragmentLengthB);
			...
			else
			{
				...
				veejay_memcpy(funnyCode + fragmentPos, fragmentA, fragmentLengthA);
...
static int PlanarToNV12Wrapper(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
	...
	if(dstStride[0]==srcStride[0])
		veejay_memcpy(dst, src[0], srcSliceH*dstStride[0]);
	else
	{
		int i;
		...
		{
			veejay_memcpy(dstPtr, srcPtr, srcStride[0]);
			srcPtr+= srcStride[0];
			dstPtr+= dstStride[0];
		}
	}
...
static int rgb2rgbWrapper(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
	...
	if(   (isBGR(srcFormat) && isBGR(dstFormat))
	   || (isRGB(srcFormat) && isRGB(dstFormat))){
		...
		default: MSG_ERR("swScaler: internal error %x -> %x converter\n", 
		...
		MSG_ERR("swScaler: internal error %x -> %x converter\n", 
			 srcFormat, dstFormat);
	}
...
static int yvu9toyv12Wrapper(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
	...
	if(srcStride[0]==dstStride[0]) 
		veejay_memcpy(dst[0]+ srcSliceY*dstStride[0], src[0], srcStride[0]*srcSliceH);
	else{
		uint8_t *srcPtr= src[0];
		uint8_t *dstPtr= dst[0] + dstStride[0]*srcSliceY;
		...
		for(i=0; i<srcSliceH; i++)
		{
			veejay_memcpy(dstPtr, srcPtr, c->srcW);
			srcPtr+= srcStride[0];
			dstPtr+= dstStride[0];
		}
	}
...
static int simpleCopy(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
	...
	{
		if(dstStride[0]==srcStride[0])
			veejay_memcpy(dst[0] + dstStride[0]*srcSliceY, src[0], srcSliceH*dstStride[0]);
		else
		{
			int i;
			uint8_t *srcPtr= src[0];
			...
			for(i=0; i<srcSliceH; i++)
			{
				veejay_memcpy(dstPtr, srcPtr, length);
				srcPtr+= srcStride[0];
				dstPtr+= dstStride[0];
			}
		}
		...
		for(plane=0; plane<3; plane++)
		{
			...
			else
			{
				if(dstStride[plane]==srcStride[plane])
					veejay_memcpy(dst[plane] + dstStride[plane]*y, src[plane], height*dstStride[plane]);
				else
				{
					int i;
...
int sws_setColorspaceDetails(SwsContext *c, const int inv_table[4], int srcRange, const int table[4], int dstRange, int brightness, int contrast, int saturation){
	...
	veejay_memcpy(c->srcColorspaceTable, inv_table, sizeof(int)*4);
	veejay_memcpy(c->dstColorspaceTable,     table, sizeof(int)*4);
...
SwsContext *sws_getContext(int srcW, int srcH, int origSrcFormat, int dstW, int dstH, int origDstFormat, int flags,
	...
	{
		MSG_ERR("swScaler: %x is not supported as input format\n", srcFormat);
		return NULL;
	}
	if(!isSupportedOut(dstFormat))
	{
		MSG_ERR("swScaler: %x is not supported as output format\n", dstFormat);
		return NULL;
	}
	...
	if(unscaled && !usesHFilter && !usesVFilter)
	{
		...
		if(c->swScale){
			if(flags&SWS_PRINT_INFO)
				MSG_INFO("SwScaler: using unscaled %x -> %x special converter\n", 
					srcFormat, dstFormat);
			return c;
		}
	}
...
// can't downscale !!!
	...
	if(flags&SWS_PRINT_INFO)
	{
		...
		if(dstFormat==IMGFMT_BGR15 || dstFormat==IMGFMT_BGR16)
			MSG_INFO("from %x to%s %x ", 
				srcFormat, dither, dstFormat);
		else
			MSG_INFO("from %x to %x ", 
				srcFormat, dstFormat);
...
SwsFilter *sws_getDefaultFilter(float lumaGBlur, float chromaGBlur, 
				...
				int verbose)
{
	SwsFilter *filter= (SwsFilter*)vj_malloc(sizeof(SwsFilter));
...
SwsVector *sws_getGaussianVec(double variance, double quality){
	...
	SwsVector *vec= (SwsVector*) vj_malloc(sizeof(SwsVector));
