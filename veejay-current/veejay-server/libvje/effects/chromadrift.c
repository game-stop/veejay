...
vj_effect *chromadrift_init(int w, int h)
{
    ...
    );
    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_SOURCE_MIX, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 180, 820, 82, 100, 10, 520, 0, 5, 0, VJ_BEAT_COST_CHEAP, 86, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_FLOW, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 80, 980, 94, 100, 8, 480, 0, 5, 0, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MOTION_REACT, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 300, 1000, 94, 100, 8, 480, 0, 5, 100, VJ_BEAT_COST_MODERATE, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_GLOW, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_HIGH_ONSET, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 100, 820, 90, 100, 4, 520, 24, 5, 100, VJ_BEAT_COST_MODERATE, 88, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_WARP, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_LOW_ONSET, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 140, 1000, 92, 100, 4, 520, 24, 5, 0, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SIGNED_CURVE, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_SIGN_LOCK | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_SIGNED, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_SOURCE_SIGN, VJ_BEAT_CURVE_LINEAR, -920, 920, 90, 100, 0, 360, 0, 5, 0, VJ_BEAT_COST_CHEAP, 90, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_ENVELOPE, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 560, 1000, 82, 100, 160, 1600, 120, 5, 120, VJ_BEAT_COST_MODERATE, 88, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_COLOR_AMOUNT, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_HIGH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 220, 940, 74, 98, 100, 1100, 0, 5, 120, VJ_BEAT_COST_MODERATE, 76, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SIGNED_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_SIGN_LOCK | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_SIGNED, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_SOURCE_SIGN, VJ_BEAT_CURVE_EASE_OUT, -940, 940, 96, 100, 8, 420, 0, 5, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_INERTIA, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 120, 760, 66, 94, 120, 1200, 0, 5, 120, VJ_BEAT_COST_MODERATE, 62, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
void chromadrift_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    int source_user_i = eh_param1000_to_100(args[P_SOURCE]);
    int drift_user_i = eh_param1000_to_100(args[P_DRIFT]);
    int contour_user_i = eh_param1000_to_100(args[P_CONTOUR]);
    int glow_user_i = eh_param1000_to_100(args[P_GLOW]);
    int pull_user_i = eh_param1000_to_100(args[P_PULL]);
    int curl_user_i = eh_param1000_to_signed100(args[P_CURL]);
    int trail_user_i = eh_param1000_to_100(args[P_TRAIL]);
    int color_user_i = eh_param1000_to_100(args[P_COLOR]);
    int speed_user_i = eh_param1000_to_signed100(args[P_SPEED]);
    int soft_user_i = eh_param1000_to_100(args[P_SOFT]);

    int drift_i = eh_clampi(drift_user_i, 0, 100);
    int contour_i = eh_clampi(contour_user_i, 0, 100);
    int glow_i = eh_clampi(glow_user_i, 0, 100);
    int pull_i = eh_clampi(pull_user_i, 0, 100);
    int curl_i = eh_clampi(curl_user_i, -100, 100);
    int color_i = eh_clampi(color_user_i, 0, 100);
    int speed_i = eh_clampi(speed_user_i, -100, 100);
    int soft_i = eh_clampi(soft_user_i, 0, 100);
    int trail_i = eh_clampi(trail_user_i, 0, 100);

    int build_i = 66;
    int decay_i = eh_clampi(44 + (trail_i * 42 + 50) / 100 + (soft_i * 20 + 50) / 100, 0, 100);
    int density_i = eh_clampi(30 + (glow_i * 22 + 50) / 100 + (contour_i * 18 + 50) / 100 - (soft_i * 12 + 50) / 100, 0, 100);
    int bands_i = eh_clampi(18 + (color_i * 58 + 50) / 100 + (glow_i * 10 + 50) / 100, 0, 100);

    float build_t = (float) build_i * 0.01f;
    float source_t = (float) source_user_i * 0.01f;
    float drift_t = (float) drift_i * 0.01f;
    float contour_t = (float) contour_i * 0.01f;
    float glow_t = (float) glow_i * 0.01f;
    float density_t = (float) density_i * 0.01f;
    float pull_t = (float) pull_i * 0.01f;
    float bands_t = (float) bands_i * 0.01f;
    float trail_t = (float) trail_i * 0.01f;
    float color_t = (float) color_i * 0.01f;
    float soft_t = (float) soft_i * 0.01f;
    float speed_signed = (float) speed_i * 0.01f;
    float speed_mag = eh_absf(speed_signed);
    float speed_curve = speed_mag * speed_mag * (0.42f + speed_mag * 0.58f);

    int tone_mix = (int) ((0.055f + glow_t * 0.22f + color_t * 0.055f) * 255.0f + 0.5f);
    tone_mix = eh_clampi(tone_mix, 0, 255);

    float src_mix = (0.010f + source_t * source_t * 0.135f + source_t * 0.038f) * (1.0f - trail_t * 0.48f);
    src_mix = eh_clampf(src_mix, 0.0030f, 0.22f);
    float chroma_src_mix = src_mix + 0.014f + source_t * 0.025f + (1.0f - color_t) * 0.012f;
    chroma_src_mix = eh_clampf(chroma_src_mix, 0.008f, 0.24f);

    float flow_pixels = (0.50f + contour_t * 7.0f + drift_t * 3.0f + pull_t * 3.2f) * (0.34f + speed_curve * 1.10f);
    float curl_pixels = (0.40f + contour_t * 2.8f + density_t * 1.6f) * (0.55f + speed_curve * 1.20f);
    if (speed_i == 0) {
        flow_pixels *= 0.18f;
        curl_pixels *= 0.12f;
    }

    float global_glow_base = glow_t * (7.0f + density_t * 15.0f) * (1.0f - soft_t * 0.10f);
    float global_detail_base = (0.018f + density_t * 0.050f) * (1.0f - trail_t * 0.52f) * (1.0f - soft_t * 0.36f);
    float global_wet_base = 0.065f + trail_t * 0.105f + soft_t * 0.075f;
    float global_chroma_fade = trail_t * (0.004f + soft_t * 0.003f);

    int motion_gain_global = 14 + (int) (glow_t * 36.0f);
    float force_charge_scale = 12.0f + density_t * 28.0f;
    float max_v_global = 2.0f + contour_t * 7.5f + drift_t * 3.4f + pull_t * 5.5f;
    float max_v2_global = max_v_global * max_v_global;
    float curl_dir = (curl_i < 0 ? -1.0f : 1.0f);
    float band_scale = 0.30f + bands_t * 1.00f;

    int force_update = !((e->frame & 1) && eh_absi(speed_i) < 62 && drift_i < 70 && contour_i < 84 && pull_i < 84);
    int flow_cell = 28 - (contour_i * 8 + 50) / 100 - (drift_i * 4 + 50) / 100;
    if (flow_cell < 12)
        flow_cell = 12;
    else if (flow_cell > 30)
        flow_cell = 30;

    int cell_changed = (e->grid_cell != flow_cell);
    int do_build_force = (cell_changed || e->grid_w <= 0 || e->grid_h <= 0 || force_update);
    float phase_step = (0.0012f + speed_curve * 0.025f + contour_t * 0.004f) * (speed_i < 0 ? -1.0f : 1.0f);
    ...
    #pragma omp single
    {
        ...
        float mature_rate = 0.0008f + build_t * build_t * 0.035f;
        ...
        if (do_build_force) {
            int gw = (w + flow_cell - 1) / flow_cell + 2;
            int gh = (h + flow_cell - 1) / flow_cell + 2;
    ...
    float current_maturity = e->maturity;
    float current_phase = e->phase;
    flow_pixels *= (0.55f + current_maturity * 1.85f);
    ...
    for (int qy = 0; qy < qrows; qy++) {
        ...
        for (qx = 0; qx < qcols; qx++) {
            ...
            band = eh_lut_sin_fast(e, current_phase + ((float) txc * 0.013f + (float) tyc * 0.009f) * band_scale);
            band = band * 0.5f + 0.5f;
            {
                int src_u0 = (int) U[pc] - 128;
                int src_v0 = (int) V[pc] - 128;
                int pal_idx = (int) ((current_phase * 0.049f + fm * 0.24f + band * 0.10f + (float) e->src_y[pc] * 0.0011f + (float) (src_u0 - src_v0) * 0.0009f) * 256.0f);
