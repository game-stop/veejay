...
typedef struct {
    ...
    int cached_skip_processing;
    int cached_update_internal_diff;
    uint8_t *cached_diff;
    int cached_interpolate;
    int cached_motion;
    int cached_value;
    int cached_diff_gain_q8;
    int cached_depth_q;
    int cached_hold_q;
} timedistort_t;
...
vj_effect *timedistort_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_MOTION_REACT, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_EASE_OUT, 5, 76, 88, 100, 8, 480, 0, 1, 0, VJ_BEAT_COST_CHEAP, 90, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 160, 1000, 88, 100, 8, 440, 0, 5, 0, VJ_BEAT_COST_CHEAP, 92, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_WARP, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 0, 1000, 94, 100, 8, 420, 0, 5, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_ENVELOPE, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 420, 1000, 72, 96, 240, 1700, 0, 5, 0, VJ_BEAT_COST_CHEAP, 72, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_WARP, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_BURST, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 1000, 90, 100, 6, 440, 24, 5, 0, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
void timedistort_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    #pragma omp single
    {
        ...
        td->cached_value = clampi(value_s - threshold_drop, 1, 100);
        td->cached_diff_gain_q8 = clampi(256 + ((scratch_s * 560) / 1000) + ((musical_depth * 540) / 1000), 256, 1536);
        td->cached_depth_q = time_depth_s + ((musical_depth * 360 + 500) / 1000);
        td->cached_depth_q = clampi(td->cached_depth_q, 0, 1000);

        td->cached_hold_q = trail_hold_s + ((musical_depth * 130 + 500) / 1000);
        td->cached_hold_q = clampi(td->cached_hold_q, 0, 1000);

        td->cached_skip_processing = 0;
        td->cached_update_internal_diff = 0;
        td->cached_interpolate = 0;
        td->cached_motion = 0;
        td->cached_diff = td->diff;

        if(td->motionmap && motionmap_active(td->motionmap)) {
            int tmp1 = td->cached_value;
            int tmp2 = td->cached_value;
            ...
            if(!td->have_bg) {
                ...
                veejay_memset(td->diff, 0, (size_t)len);
                veejay_memset(td->warptime[0], 0, (size_t)len);
                veejay_memset(td->warptime[1], 0, (size_t)len);

                td->have_bg = 1;
                td->plane = 1 & td->plane_mask;
                td->plane_populated = 1;
                td->cached_skip_processing = 1;
            } else {
                td->cached_update_internal_diff = 1;
                td->cached_diff = td->diff;
            }
        }

        if(!(td->n__ == td->N__ || td->n__ == 0))
            td->cached_interpolate = 1;
    ...
    if(td->cached_skip_processing)
        return;

    if(td->cached_update_internal_diff) {
        timedistort_build_diff(td, Y, td->cached_value, td->cached_diff_gain_q8, len);
    ...
    for(int y = 0; y < height; y++) {
        ...
        for(int x = 0; x < width; x++) {
            ...
            if(sum > 0) {
                const int decay_step = 1 + (((1000 - td->cached_hold_q) * 5 + 500) / 1000);
    ...
    const uint8_t *restrict diff = td->cached_diff;
    const int depth_q = td->cached_depth_q;

    #pragma omp for schedule(static)
    for(int i = 0; i < len; i++) {
        int age = td->warptime[td->warptimeFrame ^ 1][i];
        ...
        if(diff[i]) {
            ...
            td->warptime[td->warptimeFrame ^ 1][i] = (uint8_t)age;
        }
