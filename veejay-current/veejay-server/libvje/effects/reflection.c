...
void reflection_apply(void *ptr, VJFrame *frame, int *args) {
    ...
    #pragma omp single
    {
        if(!move) {
            ...
            r->sin_index %= width;
            r->sin_index2 %= height;
            if(r->sin_index < 0) r->sin_index += width;
            if(r->sin_index2 < 0) r->sin_index2 += height;
        }
        ...
        uint8_t *current_row = (uint8_t*)alloca(width);
        ...
        for(int y = 1; y < height - 1; y++) {
            ...
            for(int x = 0; x < width; x++) {
                current_row[x] = Y[row + x];
            }

            for(int x = 1; x < width - 1; x++) {
                const int i1 = (x == 1) ? (int)current_row[0] : (int)current_row[x];
                const int i2 = (int)current_row[x + 1];
