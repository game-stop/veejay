...
static inline int motionmap_absi(int v) { const int m = v >> 31; return (v + m) ^ m; }
static inline uint8_t motionmap_u8_sat(int v) { return (uint8_t)clampi(v, 0, 255); }
...
vj_effect *motionmap_init(int w, int h)
{
    ...
    ve->param_description = vje_build_param_list(
        ve->num_params,
        "Difference Threshold", "Maximum Motion Energy", "Draw Motion Map",
        "History in frames", "Decay", "Interpolate frames", "Activity Mode", "Activity Decay"
    );
    ...
    if(energy_hi < 500) energy_hi = 500;
    if(energy_hi > ve->limits[1][P_MAX_ENERGY]) energy_hi = ve->limits[1][P_MAX_ENERGY];

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_DETAIL, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_EASE_OUT, 8, 96, 86, 100, 10, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_INTENSITY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_ENVELOPE, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LOG, 300, energy_hi, 72, 96, 60, 1400, 0, 10, 0, VJ_BEAT_COST_CHEAP, 82, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 2, 32, 72, 96, 20, 620, 0, 1, 240, VJ_BEAT_COST_MODERATE, 62, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_INERTIA, VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_ENVELOPE, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LOG, 2, 48, 66, 92, 40, 1200, 0, 1, 240, VJ_BEAT_COST_MODERATE, 58, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
void *motionmap_malloc(int w, int h)
{
    ...
    if(!mm->region) { free(mm); return NULL; }
...
uint8_t *motionmap_interpolate_buffer(void *ptr) { return ((motionmap_t*)ptr)->interpolate_buf; }
uint8_t *motionmap_bgmap(void *ptr) { return ((motionmap_t*)ptr)->binary_img; }
int motionmap_active(void *ptr) { return ptr ? ((motionmap_t*)ptr)->running : 0; }
int motionmap_is_locked(void *ptr) { return ((motionmap_t*)ptr)->scale_lock; }
uint32_t motionmap_activity(void *ptr) { return ((motionmap_t*)ptr)->keyv_; }
int motionmap_instances(void *ptr) { return ((motionmap_t*)ptr)->is_initialized; }

void motionmap_scale_to(void *ptr, int p1max, int p2max, int p1min, int p2min, int *p1val, int *p2val, int *pos, int *len)
{
    motionmap_t *mm = (motionmap_t *) ptr;
    if(mm->global_max == 0 || mm->scale_lock) return;
    if(mm->keyv_ > (uint32_t)mm->global_max) mm->keyv_ = (uint32_t)mm->global_max;
    ...
    if(pu > mm->global_max) pu = mm->global_max;
    if(pu < 0) pu = 0;
...
void motionmap_store_frame(void *ptr, VJFrame *fx)
{
    ...
    motionmap_t *mm = (motionmap_t *) ptr;
    if(mm->running == 0 || !mm->do_interpolation) return;
...
#pragma omp single
        {
            activity_level = motionmap_calc_diff(
                mm->bg_image, mm->prev_img, frame->data[0],
                mm->diff_img, mm->diff_img + len, mm->binary_img,
                len, threshold
            );
        }
...
#pragma omp single
        {
            ...
            if(avg_actlvl < limit) avg_actlvl = 0;
            mm->nframe_++;
