...
#pragma omp section
        { veejay_memcpy(mm->interpolate_buf + (2 * len), fx->data[2], len); }
    }
...
static int32_t motionmap_calc_diff(motionmap_t *mm,
...
#pragma omp for schedule(static)
    ...
    mm->activity_partials[tid] = level;
#pragma omp single
    {
        int64_t total = 0;
    ...
    return (int32_t)(mm->activity_level >> 8);
}
...
int motionmap_prepare(void *ptr, VJFrame *frame)
{
    ...
    veejay_memcpy(mm->bg_image, frame->data[0], len);
...
#pragma omp section
            { veejay_memcpy(frame->data[0], mm->binary_img, len); }
#pragma omp section
            {
                mm->running = 0;
                mm->stored_frame = 0;
