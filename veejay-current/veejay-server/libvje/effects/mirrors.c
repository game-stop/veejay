...
vj_effect *mirrors_init(int width, int height)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            ...
            VJ_BEAT_HINT_V2(VJ_BEAT_GEOMETRY_FREQUENCY, VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 1, beat_hi, 78, 98, 10, 620, 0, 1, 240, VJ_BEAT_COST_MODERATE, 88, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
#pragma omp single
...

    {
        switch(type) {
            case 0:
                mirrors_vertical(frame->data, width, height, m->factor, 0);
                break;
            case 1:
                mirrors_vertical(frame->data, width, height, m->factor, 1);
                break;
            case 2:
                mirrors_horizontal(frame->data, width, height, m->factor, 0);
                break;
            case 3:
                mirrors_horizontal(frame->data, width, height, m->factor, 1);
                break;
        }
    }
