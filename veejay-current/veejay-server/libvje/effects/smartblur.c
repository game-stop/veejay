...
#pragma omp simd
                for( int kx = -radius; kx <= radius; kx ++ ) {
                    ...
                    uSum += maskMultiplier * (srcU[(y + ky) * w + (x + kx)] - 128);
                    vSum += maskMultiplier * (srcV[(y + ky) * w + (x + kx)] - 128);
            ...
            if( pixelCount == 0 ) {
                ...
                srcY[y * w + x] = (uint8_t)(averagedY);
                srcU[y * w + x] = (uint8_t)(averagedU);
                srcV[y * w + x] = (uint8_t)(averagedV);
            }
        }
    }
