...
typedef struct { uint8_t y,u,v; } pixel_t;
...
static inline uint8_t evaluate_pixel_row(
    ...
    uint8_t *premul,uint8_t *Y,int *row_hist,int *row_y_map,int width,int height
){
    ...
    if(!mode){
        for(int y=0;y<height;y++)
            for(int x=0;x<width;x++)
                *(dstY++)=evaluate_pixel_row(x,y,brush_size,max_intensity,Y,Y2,n->row_hist[0],n->row_y_map[0],width,height);
