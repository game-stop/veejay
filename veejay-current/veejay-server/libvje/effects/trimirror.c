...
vj_effect *trimirror_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            ...
            VJ_BEAT_HINT_V2(VJ_BEAT_SIGNED_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_SIGN_LOCK | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_SIGNED, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_SOURCE_SIGN, VJ_BEAT_CURVE_EASE_OUT, -80, 80, 96, 100, 0, 320, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
void trimirror_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    #pragma omp single
    {
        ...
        float user_rot = s->rotation_state * (TRIMIRROR_TWO_PI / 360.0f);
        float base_angle = trimirror_wrap_angle(user_rot + s->phase + spin_drive * 0.16f * sinf(s->phase * 1.37f));
        float zoom_base = s->zoom_state * 0.001f;
        float zoom_breathe = 1.0f + zoom_drive * 0.055f * sinf(s->phase * 2.10f);
        float zoom_pulse = 1.0f + zoom_drive * 0.34f;
        float zoom = trimirror_clampf(zoom_base * zoom_breathe * zoom_pulse, 0.10f, 3.00f);
