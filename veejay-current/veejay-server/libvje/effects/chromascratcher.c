...
vj_effect *chromascratcher_init(int w, int h) {
    ...
    );
    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 1, MAX_SCRATCH_FRAMES - 1, 86, 100, 8, 520, 0, 1, 120, VJ_BEAT_COST_MODERATE, 92, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_ALPHA_OR_OPACITY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 48, 255, 92, 100, 10, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
static void chromastore_frame(chromascratcher_t *c, VJFrame *src, int n, int no_reverse) {
    ...
    if(cnreverse) cnframe--;
    else cnframe++;

    if(cnframe >= n) {
        if(no_reverse == 0) { cnreverse = 1; cnframe = n - 1; }
        else { cnframe = 0; }
    }
    if(cnframe <= 0) { cnframe = 0; cnreverse = 0; }
...
#pragma omp for schedule(static)
    for(int i = 0; i < len; i++) {
        int take = 0;
        switch(mode) {
            case 0: take = SY[i] < Y[i]; break;
            case 1: take = SY[i] > Y[i]; break;
            case 2: take = (SY[i] * op_a) < (Y[i] * op_b); break;
            case 3: take = (SY[i] * op_a) > (Y[i] * op_b); break;
        }
        if(take) {
            Y[i] = SY[i];
            Cb[i] = SU[i];
...
void chromascratcher_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    #pragma omp single
    {
        if(n <= 0) {
            ...
            c->offset = len * c->cnframe;

            veejay_memcpy(&c->_tmp, frame, sizeof(VJFrame));
            c->_tmp.data[0] = c->cframe[0] + c->offset;
            c->_tmp.data[1] = c->cframe[1] + c->offset;
            c->_tmp.data[2] = c->cframe[2] + c->offset;
            c->_tmp.data[3] = NULL;
            c->_tmp.len = len;
            c->_tmp.uv_len = len;
            c->_tmp.ssm = 1;
    ...
    if(!c->skip_processing) {
        if(mode > 3) {
            ...
            chromascratcher_apply_simple(c, frame, mode, opacity, c->offset);
        }
