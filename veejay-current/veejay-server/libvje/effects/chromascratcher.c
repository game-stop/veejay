...
static void chromastore_frame(chromascratcher_t *c, VJFrame *src, int n, int no_reverse)
{
    ...
    if(!cnreverse) {
        veejay_memcpy(dest[0], src->data[0], len);
        veejay_memcpy(dest[1], src->data[1], len);
        veejay_memcpy(dest[2], src->data[2], len);
    }
    else {
        veejay_memcpy(src->data[0], dest[0], len);
        veejay_memcpy(src->data[1], dest[1], len);
        veejay_memcpy(src->data[2], dest[2], len);
    }
