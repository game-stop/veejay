...
static inline uint8_t halftone_dot_radius(int v, int half_radius) { return (uint8_t)(1 + ((v * half_radius + 127) / 255)); }
static inline unsigned int halftone_hash_u32(unsigned int x) { x ^= x << 13; x ^= x >> 17; x ^= x << 5; return x; }
...
static inline void halftone_berserk_coords(int gx, int gy, int radius, int w, int h, int *bx, int *by, double timecode)
{
    ...
    if(*bx < 0) *bx = 0; else if(*bx > max_x) *bx = max_x;
    if(*by < 0) *by = 0; else if(*by > max_y) *by = max_y;
}
...
vj_effect *halftone_init(int w, int h)
{
    ...
    int soft_min = 4;
    if(max_radius < 2) max_radius = 2;
    if(soft_min > max_radius) soft_min = 2;
    if(soft_max < soft_min) soft_max = soft_min;
    if(soft_max > max_radius) soft_max = max_radius;
...
void *halftone_malloc(int w, int h)
{
    ...
    if(!s->wrad_grid) { free(s); return NULL; }
...
static void halftone_prepare_cells(halftone_t *s, VJFrame *frame, int radius, int mode, int orientation, int parity, int x_inf, int y_inf, int grid_w, int grid_h)
{
    ...
    for(int gy = 0; gy < grid_h; gy++) {
        for(int gx = 0; gx < grid_w; gx++) {
            ...
            if(parity == 3) halftone_berserk_coords(gx, gy, radius, w, h, &bx, &by, frame->timecode);
            const int sx0 = bx < 0 ? 0 : (bx >= w ? w - 1 : bx);
            const int sy0 = by < 0 ? 0 : (by >= h ? h - 1 : by);
            int sx1 = bx + radius;
            int sy1 = by + radius;
            if(sx1 < 0) sx1 = 0; else if(sx1 > w) sx1 = w;
            if(sy1 < 0) sy1 = 0; else if(sy1 > h) sy1 = h;
            if(sx1 <= sx0 || sy1 <= sy0) {
                wrad_grid[g_idx] = 0; y_grid[g_idx] = pixel_Y_lo_; u_grid[g_idx] = 128; v_grid[g_idx] = 128;
                continue;
            }
            if(mode == 5) {
                uint8_t brightest = 0;
                for(int y = sy0; y < sy1; y++) { const int row = y * w; for(int x = sx0; x < sx1; x++) { const uint8_t v = Y[row + x]; if(v > brightest) brightest = v; } }
                wrad_grid[g_idx] = halftone_dot_radius(brightest, half_r);
                y_grid[g_idx] = pixel_Y_hi_; u_grid[g_idx] = 128; v_grid[g_idx] = 128;
            } else if(mode == 6) {
                uint8_t darkest = 255;
                for(int y = sy0; y < sy1; y++) { const int row = y * w; for(int x = sx0; x < sx1; x++) { const uint8_t v = Y[row + x]; if(v < darkest) darkest = v; } }
                wrad_grid[g_idx] = halftone_dot_radius(255 - darkest, half_r);
                y_grid[g_idx] = pixel_Y_lo_; u_grid[g_idx] = 128; v_grid[g_idx] = 128;
            } else {
                uint32_t sum_y = 0; uint32_t hit = 0; uint64_t total_y = 0; int64_t sum_u = 0; int64_t sum_v = 0;
                for(int y = sy0; y < sy1; y++) {
                    const int row = y * w;
                    for(int x = sx0; x < sx1; x++) {
                        const int pos = row + x; const uint8_t yv = Y[pos];
                        sum_y += yv; hit++;
                        if(mode == 4 || mode == 8) { total_y += yv; sum_u += (int64_t)((int)U[pos] - 128) * (int64_t)yv; sum_v += (int64_t)((int)V[pos] - 128) * (int64_t)yv; }
                    }
                }
                if(hit > 0) {
                    const int avg = (int)(sum_y / hit);
                    int dot_y = avg;
                    if(mode == 0) dot_y = pixel_Y_hi_; else if(mode == 1) dot_y = pixel_Y_lo_; else if(mode == 7) dot_y = 255 - avg;
                    wrad_grid[g_idx] = halftone_dot_radius((mode == 7) ? (255 - avg) : avg, half_r);
                    y_grid[g_idx] = (uint8_t)dot_y;
                    if(mode == 3) { const int p = sy0 * w + sx0; u_grid[g_idx] = U[p]; v_grid[g_idx] = V[p]; }
                    else if((mode == 4 || mode == 8) && total_y > 0) {
                        const int avg_u = 128 + (int)(sum_u / (int64_t)total_y); const int avg_v = 128 + (int)(sum_v / (int64_t)total_y);
                        if(mode == 8) { u_grid[g_idx] = (uint8_t)clampi(avg_v, 0, 255); v_grid[g_idx] = (uint8_t)clampi(avg_u, 0, 255); }
                        else { u_grid[g_idx] = (uint8_t)clampi(avg_u, 0, 255); v_grid[g_idx] = (uint8_t)clampi(avg_v, 0, 255); }
                    } else { u_grid[g_idx] = 128; v_grid[g_idx] = 128; }
                } else { wrad_grid[g_idx] = 0; y_grid[g_idx] = pixel_Y_lo_; u_grid[g_idx] = 128; v_grid[g_idx] = 128; }
            }
        }
    }
}

static void halftone_render_cells(halftone_t *s, VJFrame *frame, int radius, int mode, int x_inf, int y_inf, int grid_w, int grid_h)
{
    uint8_t *restrict Y = frame->data[0];
    uint8_t *restrict U = frame->data[1];
    uint8_t *restrict V = frame->data[2];
...
void halftone_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    int x_inf = 0, y_inf = 0, x_sup = frame->width, y_sup = frame->height;
    ...
    while(grid_w > 1 && grid_w * grid_h > s->max_cells) grid_w--;
    while(grid_h > 1 && grid_w * grid_h > s->max_cells) grid_h--;
