...
#pragma omp single
    {
        if(!s->eff_initialized) {
            ...
            spherize_smooth_i(&s->eff_strength, strength_arg, geo_attack, geo_release);
            spherize_smooth_angle_deg(&s->eff_angle, (float)angle_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_radius, radius_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_ratio_x, ratio_x_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_ratio_y, ratio_y_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_center_x, center_x_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_center_y, center_y_arg, slow_attack, slow_release);
            spherize_smooth_i(&s->eff_warp_drive, warp_drive_arg, geo_attack, geo_release);
            spherize_smooth_i(&s->eff_radius_drive, radius_drive_arg, geo_attack, geo_release);
        }
    }

    strength_arg = clampi((int)(s->eff_strength + 0.5f), 0, 100);
    angle_arg = (int)(s->eff_angle + 0.5f);
    radius_arg = clampi((int)(s->eff_radius + 0.5f), 1, radius_max);
    ratio_x_arg = clampi((int)(s->eff_ratio_x + 0.5f), 10, 200);
    ratio_y_arg = clampi((int)(s->eff_ratio_y + 0.5f), 10, 200);
    center_x_arg = clampi((int)(s->eff_center_x + 0.5f), 0, width);
    center_y_arg = clampi((int)(s->eff_center_y + 0.5f), 0, height);

    const int warp_drive = clampi((int)(s->eff_warp_drive + 0.5f), 0, 1000);
    const int radius_drive = clampi((int)(s->eff_radius_drive + 0.5f), 0, 1000);
