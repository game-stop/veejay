...
vj_effect *videoplay_init(int w, int h)
{
    ...
    ve->limits[1][P_PHOTOS] = max_grid > 0 ? max_grid : 2;
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_GRID_SIZE, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL | VJ_BEAT_F_REBUILDS_STATE, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_EASE_OUT, 1, 96, 88, 100, 8, 520, 0, 1, 80, VJ_BEAT_COST_CHEAP, 78, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_BURST, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 1000, 96, 100, 6, 440, 24, 5, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_DRIFT, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 0, 1000, 94, 100, 8, 520, 0, 5, 0, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
static void release_filmstrip(videowall_t *vw)
{
    ...
    if(vw->frame_region) free(vw->frame_region);
    if(vw->pictures) free(vw->pictures);
    if(vw->video_list) free(vw->video_list);
    if(vw->rt) free(vw->rt);
...
{
    if(!vw || grid <= 0 || w <= 0 || h <= 0)
        return NULL;

    const int box_w = w / grid;
    const int box_h = h / grid;
    const int film_length = grid * grid;
    const size_t plane_len = (size_t)(box_w > 0 ? box_w : 1) * (size_t)(box_h > 0 ? box_h : 1);
    const size_t frame_len = plane_len * 3u;
    uint8_t *planes;

    vw->video_list = (picture_t**) vj_calloc(sizeof(picture_t*) * (size_t)(film_length + 1));
    if(!vw->video_list) {
        release_filmstrip(vw);
        return NULL;
    }
    ...
    vw->rt = (int*) vj_calloc(sizeof(int) * (size_t)film_length);
    if(!vw->rt) {
        release_filmstrip(vw);
        return NULL;
    }
    ...
    for(int i = 0; i < film_length; i++) {
        ...
        vw->video_list[i] = pic;
        pic->w = box_w > 0 ? box_w : 1;
        pic->h = box_h > 0 ? box_h : 1;
        pic->data[0] = planes;
...
{
    if(index < 0 || index >= vw->num_videos || !vw->video_list || !vw->video_list[index])
        return;
...
static void put_video(videowall_t *vw,
                      ...
                      int boost_y)
{
    if(!vw || !dst || index < 0 || index >= vw->num_videos || !vw->video_list || !vw->video_list[index])
        return;
...
void videoplay_apply(void *ptr, VJFrame *frame, VJFrame *B, int *args)
{
    ...
    #pragma omp single copyprivate(skip_processing, amp_x, amp_y, global_x, global_y, boost_y, matrix_placement)
    {
        ...
        if(!skip_processing) {
            ...
            int a = 0, b = 0;
            if(capture_now) {
                a = vw->frame_counter % vw->num_videos;
                b = (vw->frame_counter + 1) % vw->num_videos;
                ...
                a = (vw->frame_counter - 1) % vw->num_videos;
                b = vw->frame_counter % vw->num_videos;
