...
vj_effect *photoplay_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_GRID_SIZE, VJ_BEAT_F_DISCRETE | VJ_BEAT_F_REBUILDS_STATE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 2, 7, 62, 88, 20, 1200, 0, 1, 2200, VJ_BEAT_COST_MODERATE, 64, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
#pragma omp single
    ...
    const int ready = p->num_photos > 0;
    const int current_slot = ready ? p->frame_counter % p->num_photos : 0;
    const matrix_f matrix_placement = get_matrix_func(mode);
    uint8_t *restrict dst[3] = { frame->data[0], frame->data[1], frame->data[2] };

#pragma omp for schedule(static)
    for(int plane = 0; plane < 3; plane++) {
        if(ready)
            take_photo(p, dst[plane], p->photo_list[current_slot]->data[plane],
                       width, height, current_slot);
    }

#pragma omp for schedule(static)
    for(int plane = 0; plane < 3; plane++) {
        if(ready)
            veejay_memset(dst[plane], plane == 0 ? pixel_Y_lo_ : 128, len);
    }
...
#pragma omp for schedule(static)
    for(int i = 0; i < p->num_photos; i++) {
        if(ready) {
            ...
            const matrix_t matrix = matrix_placement(display_idx, p->grid_size, width, height);

            put_photo(p, dst[0], p->photo_list[i]->data[0], width, height, i, matrix);
            put_photo(p, dst[1], p->photo_list[i]->data[1], width, height, i, matrix);
            put_photo(p, dst[2], p->photo_list[i]->data[2], width, height, i, matrix);
        }
    }
