...
vj_effect *maskstop_init(int width, int height)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            ...
            VJ_BEAT_HINT_V2(VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SRC_NONE, VJ_BEAT_OP_NONE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_LINEAR, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, 0, 0, VJ_BEAT_COST_STRUCTURAL, -1000, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 8, 255, 90, 100, 8, 420, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MOTION_REACT, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_BURST, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 4, 220, 84, 100, 0, 420, 0, 1, 0, VJ_BEAT_COST_CHEAP, 90, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
#pragma omp single
    ...
    const int copy_frame = v->frq_frame > 255;
    const int copy_mask = v->frq_mask > 255;
    uint8_t *const frame_dst[3] = {
        v->vvmaskstop_buffer[0], v->vvmaskstop_buffer[1], v->vvmaskstop_buffer[2]
    };
    const uint8_t *const frame_src[3] = { Ydest, Udest, Vdest };

#pragma omp for schedule(static)
        for(int plane = 0; plane < 3; plane++)
            if(copy_frame)
                veejay_memcpy(frame_dst[plane], frame_src[plane], len);

#pragma omp single
    if(copy_frame)
        v->frq_frame = 0;

    uint8_t *const mask_dst[3] = {
        v->vvmaskstop_buffer[3], v->vvmaskstop_buffer[4], v->vvmaskstop_buffer[5]
    };
    const uint8_t *const mask_src[3] = { Ydest, Udest, Vdest };

#pragma omp for schedule(static)
        for(int plane = 0; plane < 3; plane++)
            if(copy_mask)
                veejay_memcpy(mask_dst[plane], mask_src[plane], len);
