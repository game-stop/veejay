...
#ifdef _OPENMP
#include <omp.h>
#define NB2_THREAD_ID() omp_get_thread_num()
#else
...
vj_effect *neighbours2_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_WINDOW_RADIUS, VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 2, 16, 82, 100, 10, 480, 0, 1, 80, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
void *neighbours2_malloc(int w, int h)
{
    ...
    n->scratch = (int*) vj_calloc(
        sizeof(int) * NB2_SCRATCH_STRIDE * (size_t)omp_get_max_threads());
