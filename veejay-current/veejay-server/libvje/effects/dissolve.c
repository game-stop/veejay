...
vj_effect *dissolve_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_ALPHA_OR_OPACITY, VJ_BEAT_F_CONTINUOUS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 8, 248, 84, 100, 15, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
void dissolve_apply(void *ptr, VJFrame *frame, VJFrame *frame2, int *args)
{
    ...
    if(opacity == 255)
    {
        #pragma omp sections
        {
            ...
            #pragma omp section
            { veejay_memcpy(Cb, Cb2, len); }
            #pragma omp section
            { veejay_memcpy(Cr, Cr2, len); }
        }
        return;
    }
