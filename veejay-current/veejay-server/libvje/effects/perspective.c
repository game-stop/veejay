...
#pragma omp single
    ...
    if(p->perspective_vp_) {
        veejay_memcpy(p->perspective_private_[0], frame->data[0], len);
        veejay_memcpy(p->perspective_private_[1], frame->data[1], len);
        veejay_memcpy(p->perspective_private_[2], frame->data[2], len);
