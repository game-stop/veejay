...
vj_effect *volumetrictelemetry_init(int w, int h)
{
    vj_effect *ve = (vj_effect *) vj_calloc(sizeof(vj_effect));
    if (!ve) return NULL;

    ve->num_params = VOLUMETRICPARAMS;
    ve->defaults = (int *) vj_calloc(sizeof(int) * ve->num_params);
    ve->limits[0] = (int *) vj_calloc(sizeof(int) * ve->num_params);
    ve->limits[1] = (int *) vj_calloc(sizeof(int) * ve->num_params);

    ve->description = "Phosphor Hologram";
    ve->sub_format = 1;
    ve->extra_frame = 0;
    ve->has_user = 0;
    ...
    ve->limits[0][P_THICKNESS]      = 1;    ve->limits[1][P_THICKNESS]      = 32;
    ...
    ve->param_description = vje_build_param_list(
        ...
        "Contour Interval", "Particle Size", "Elevation Scale", "Source Deposit",
...
void *volumetrictelemetry_malloc(int w, int h)
{
    volumetrictelemetry_t *c = (volumetrictelemetry_t *) vj_calloc(sizeof(volumetrictelemetry_t));
    if (!c) return NULL;
...
void volumetrictelemetry_free(void *ptr)
{
    volumetrictelemetry_t *c = (volumetrictelemetry_t *) ptr;
    free(c->prev_y); 
    free(c);
}

void volumetrictelemetry_apply(void *ptr, VJFrame *frame, int *args)
{
    volumetrictelemetry_t *c = (volumetrictelemetry_t *) ptr;
    ...
    int t_rad = (int)(c->eff_thickness + 0.5f) / 2;
    if (t_rad < 1) t_rad = 1;
    float max_r = (float)t_rad + 0.5f;
    ...
    float time_f = (float)c->frame * 0.1f;
        ...
        for (int x = step_size; x < w - step_size; x += step_size) {
            ...
            if (z_int > 24) {
                ...
                if (c_curr != (z_l / interval) || c_curr != (z_u / interval)) {
                    ...
                    if (scan_intensity > 0.05f) {
                        float jitter = sinf(orig_z * 300.0f + time_f * 5.0f);
                        sx += (int)(jitter * scan_intensity * 14.0f); // Max 14px horizontal tear
                    }

                    float shimmer = 0.6f + 0.4f * sinf(orig_z * 40.0f - time_f * 2.0f + px * 20.0f);

                    for(int dy = -t_rad; dy <= t_rad; dy++) {
                        for(int dx = -t_rad; dx <= t_rad; dx++) {
                            ...
                            if (dist <= max_r) {
                                ...
                                if (draw_x >= 0 && draw_x < w && draw_y >= 0 && draw_y < h) {
                                    ...
                                    float dot_falloff = 1.0f - (dist / max_r);
                                    
                                    float depth_fade = fmaxf(0.0f, 1.0f - ((final_z - 0.5f) / 2.5f));

                                    float final_intensity = ((0.4f * shimmer) + (scan_intensity * 0.7f)) * depth_fade * dot_falloff;
