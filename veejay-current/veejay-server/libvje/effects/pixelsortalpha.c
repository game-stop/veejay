...
#ifdef _OPENMP
#include <omp.h>
#define PSA_OMP_FOR _Pragma("omp for schedule(dynamic, 1)")
...
vj_effect *pixelsortalpha_init(int w, int h)
{
    ...
    ve->param_description = vje_build_param_list(ve->num_params, "Mode", "Pass");

    ve->hints = vje_init_value_hint_list(ve->num_params);
    vje_build_value_hint_list(ve->hints, ve->limits[1][P_MODE], P_MODE, "Non-black Alpha", "White Alpha", "Non-white Alpha");
    vje_build_value_hint_list(ve->hints, ve->limits[1][P_PASS], P_PASS, "Columns first", "Rows first", "Columns Only", "Rows Only");
...
{
    pixelsortalpha_t *p = (pixelsortalpha_t*) vj_calloc(sizeof(pixelsortalpha_t));
    ...
    p->workers = (pixelsortalpha_worker_t*) vj_calloc(
        sizeof(pixelsortalpha_worker_t) * (size_t)worker_count);
...
static void psa_csort32_range(pixelsortalpha_worker_t *wrk,
...
static inline int psa_need_sort_x_range(const uint8_t *restrict Y,
                                        unsigned int base,
                                        unsigned int n,
                                        unsigned int *lo_y,
                                        unsigned int *hi_y)
...
static inline int psa_need_sort_y_range(const uint8_t *restrict Y,
                                        unsigned int pos,
                                        unsigned int width,
                                        unsigned int n,
                                        unsigned int *lo_y,
...
static inline void psa_pack_x(uint8_t *P[4],
...
static inline void psa_unpack_x(uint8_t *P[4],
...
static inline void psa_pack_y(uint8_t *P[4],
                              uint32_t *restrict dst,
                              unsigned int n,
                              unsigned int x,
                              unsigned int y,
...
static inline void psa_unpack_y(uint8_t *P[4],
                                const uint32_t *restrict src,
                                unsigned int n,
                                unsigned int x,
                                unsigned int y,
...
static inline void psa_sort_x(pixelsortalpha_t *p,
...
static inline void psa_sort_y(pixelsortalpha_t *p,
                              pixelsortalpha_worker_t *wrk,
                              uint8_t *P[4],
                              unsigned int width,
                              unsigned int x,
...
#define PSA_ACTIVE_NONBLACK(pos) (P[3][(pos)] > pixel_Y_lo_)
#define PSA_ACTIVE_WHITE(pos)    (P[3][(pos)] >= pixel_Y_hi_)
#define PSA_ACTIVE_NONWHITE(pos) (P[3][(pos)] < pixel_Y_hi_)
...
{                                                                          \
    ...
    for(int yi = 0; yi < (int)height; yi++) {                              \
        ...
        pixelsortalpha_worker_t *wrk = &p->workers[PSA_THREAD_ID()];       \
                                                                           \
        while(x < width) {                                                 \
            const unsigned int nx = psa_row_run_##NAME(p, wrk, P, width, x, y); \
            x = nx <= x ? x + 1 : nx;                                      \
        }                                                                  \
    }                                                                      \
}
...
{                                                                          \
    ...
    for(int xi = 0; xi < (int)width; xi++) {                               \
        ...
        pixelsortalpha_worker_t *wrk = &p->workers[PSA_THREAD_ID()];       \
                                                                           \
        while(y < height) {                                                \
            const unsigned int ny = psa_column_run_##NAME(p, wrk, P, width, height, x, y); \
            y = ny <= y ? y + 1 : ny;                                      \
        }                                                                  \
    }                                                                      \
}
...
static inline void psa_rows(pixelsortalpha_t *p,
                            uint8_t *P[4],
                            unsigned int width,
                            unsigned int height,
                            int mode)
...
static inline void psa_columns(pixelsortalpha_t *p,
                               uint8_t *P[4],
                               unsigned int width,
                               unsigned int height,
                               int mode)
