...
vj_effect *radialblur_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_WINDOW_RADIUS, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 4, 72, 88, 100, 10, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 92, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_INTENSITY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_BURST, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 8, 92, 100, 4, 420, 20, 1, 120, VJ_BEAT_COST_MODERATE, 90, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
static void radialblur_h(uint8_t *restrict dst, uint8_t *restrict src, int w, int h, int radius, int power)
{
...
static void radialblur_v(uint8_t *restrict dst, uint8_t *restrict src, int w, int h, int radius, int power)
{
#pragma omp for schedule(static)
    for(int x = 0; x < w; x++)
