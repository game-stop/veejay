 ...
 * Linux VeeJay
 *
 * Copyright(C)2023-2026 Niels Elburg <nwelburg@gmail.com>
 *
...
static inline uint8_t vintagefilm_u8(int v) {
    if((unsigned int)v > 255U) return (v < 0) ? 0 : 255;
    return (uint8_t)v;
}
...
static inline float vintagefilm_smoothf(float oldv, float target, float attack, float release) {
    return target > oldv ? oldv + (target - oldv) * attack : oldv + (target - oldv) * release;
}
...
vj_effect *vintagefilm_init(int w, int h) {
    ...
    ve->defaults[P_SCRATCH_INTENSITY] = 10; ve->defaults[P_DUST_INTENSITY] = 20;
    ve->defaults[P_FLICKER_INTENSITY] = 5;  ve->defaults[P_FLICKER_FREQUENCY] = 50;
    ve->defaults[P_GRAIN_STRENGTH] = 5;     ve->defaults[P_VIGNETTE_STRENGTH] = 10;
    ve->defaults[P_SCRATCH_LIFESPAN] = 5;   ve->defaults[P_DIRT_DRIVE] = 620;
    ...
    ve->extra_frame = 0; ve->sub_format = 1; ve->has_user = 0;
    ve->param_description = vje_build_param_list(ve->num_params, "Scratch Intensity", "Dust Intensity", "Flicker Intensity", "Flicker Frequency", "Grain Strength", "Vignette Strength", "Scratch Lifespan", "Dirt Drive", "Flicker Drive");
    return ve;
}
...
void *vintagefilm_malloc(int w, int h) {
    ...
    if(!vf->region) { free(vf); return NULL; }
    ...
    vf->scratch_env = 0.0f; vf->dust_env = 0.0f; vf->flicker_env = 0.0f;
    vf->flicker_freq_env = 0.0f; vf->grain_env = 0.0f; vf->vignette_env = 0.0f;
    vf->lifespan_env = 0.0f; vf->dirt_drive_env = 0.0f; vf->flicker_drive_env = 0.0f;
    vf->smooth_ready = 0;
    vf->width = w; vf->height = h;
    vf->n_threads = vje_advise_num_threads(len);
    //FIXME: can build in parallel 
...
void vintagefilm_apply(void *ptr, VJFrame *frame, int *args) {
    ...
    #pragma omp single
    {
        ...
        int flicker_freq = clampi((int)(vf->flicker_freq_env + 0.5f), 0, 500);
        int grain_strength    = clampi((int)(vf->grain_env + 0.5f), 0, 50);
        int vignette_strength = clampi((int)(vf->vignette_env + 0.5f), 0, 100);
        int scratch_lifespan = clampi((int)(vf->lifespan_env + 0.5f), 0, 50);
        int dirt_q = clampi((int)(vf->dirt_drive_env + 0.5f), 0, 1000);
        int flicker_q = clampi((int)(vf->flicker_drive_env + 0.5f), 0, 1000);
        ...
        vf->grain_strength    = clampi(grain_strength    + (dirt_q * 34 + 500) / 1000, 0, 50);
        ...
        if(scratch_intensity > 0) {
            ...
            for(int i = 0; i < num_scratches; i++) {
                ...
                vf->scratch_map[x] = (uint8_t)clampi(life, 1, 255);
            }
        }
        ...
        vf->global_gain = clampi(global_gain, 72, 384);
        
        vf->frame_seed = vintagefilm_hash((uint32_t)vf->framecounter ^ 0x12345678u);
        vf->vignette_q8 = (vignette_strength * 256 + 50) / 100;
        vf->dust_intensity = dust_intensity;
        vf->dust_gate = dust_intensity * 5 + (dirt_q >> 2);
        vf->scratch_cut = 46 - ((dirt_q * 18 + 500) / 1000);
        vf->dirt_q = dirt_q;
    }

    const int global_gain = vf->global_gain;
    const uint32_t frame_seed = vf->frame_seed;
    const int vignette_q8 = vf->vignette_q8;
    const int dust_intensity = vf->dust_intensity;
    const int dust_gate = vf->dust_gate;
    const int scratch_cut = vf->scratch_cut;
    const int grain_strength = vf->grain_strength;
    const int dirt_q = vf->dirt_q;
    ...
    for(int y = 0; y < height; y++) {
        ...
        for(int x = 0; x < width; x++) {
            ...
            if(vignette_q8 >0) {
                const int vig_reduction = ((int)vf->vignette_lut[idx] * vignette_q8 + 32768) >> 16;
            ...
            if(vf->scratch_map[x] > 0 && ((p_noise & 0xffu) > (uint32_t)scratch_cut))
            ...
            yv = (yv * final_gain) >> 8;
            frame->data[0][idx] = vintagefilm_u8(yv);
            int u = frame->data[1][idx];
            int v = frame->data[2][idx];
            u = (u * 4 + 110 * 6) / 10;
            v = (v * 4 + 150 * 6) / 10;
            frame->data[1][idx] = vintagefilm_u8(128 + (((u - 128) * final_gain) >> 8));
            frame->data[2][idx] = vintagefilm_u8(128 + (((v - 128) * final_gain) >> 8));
        }
    }
}