...
typedef int (*vintagefilm_gain_fn)(const uint16_t *vignette,
                                   int idx,
                                   int global_gain,
                                   int vignette_q8);

static inline int vintagefilm_gain_plain(const uint16_t *vignette,
                                         int idx,
                                         int global_gain,
                                         int vignette_q8)
{
...
static inline int vintagefilm_gain_vignette(const uint16_t *vignette,
                                            int idx,
                                            int global_gain,
                                            int vignette_q8)
{
    const int vig_reduction = ((int)vignette[idx] * vignette_q8 + 32768) >> 16;
    int final_gain = global_gain - vig_reduction;
...
typedef int (*vintagefilm_dust_fn)(int yv,
                                   uint32_t p_noise,
                                   int dust_gate,
                                   int dirt_q);

static inline int vintagefilm_dust_none(int yv,
                                        uint32_t p_noise,
                                        int dust_gate,
                                        int dirt_q)
{
...
static inline int vintagefilm_dust_apply(int yv,
                                         uint32_t p_noise,
                                         int dust_gate,
                                         int dirt_q)
{
    if((p_noise & 0x7fffu) < (uint32_t)dust_gate) {
...
typedef int (*vintagefilm_grain_fn)(int yv, uint32_t p_noise, int grain_strength);

static inline int vintagefilm_grain_none(int yv, uint32_t p_noise, int grain_strength)
{
    (void)p_noise;
    (void)grain_strength;
    return yv;
...
static inline int vintagefilm_grain_apply(int yv, uint32_t p_noise, int grain_strength)
{
...
vj_effect *vintagefilm_init(int w, int h)
{
    ...
    );
    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_TURBULENCE, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 0, 100, 90, 100, 8, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 94, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_DETAIL, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_HIGH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 0, 100, 78, 100, 100, 900, 0, 1, 0, VJ_BEAT_COST_CHEAP, 88, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_GLOW, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_HIGH_ONSET, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 50, 94, 100, 4, 440, 24, 1, 0, VJ_BEAT_COST_CHEAP, 98, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_SPEED, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_EASE_OUT, 4, 180, 78, 100, 8, 620, 0, 1, 80, VJ_BEAT_COST_CHEAP, 76, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_TURBULENCE, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_HIGH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 0, 50, 70, 96, 120, 900, 0, 1, 0, VJ_BEAT_COST_CHEAP, 70, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_CONTRAST, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_ENVELOPE, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_SMOOTHSTEP, 0, 100, 62, 92, 220, 1500, 0, 1, 0, VJ_BEAT_COST_CHEAP, 60, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_MEMORY, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_DISCRETE | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_VELOCITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_EASE_OUT, 1, 24, 64, 94, 20, 760, 0, 1, 120, VJ_BEAT_COST_CHEAP, 68, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_TURBULENCE, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_BURST, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 1000, 96, 100, 6, 440, 24, 5, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
            VJ_BEAT_HINT_V2(VJ_BEAT_GLOW, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_HIGH_ONSET, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_POSITIVE, VJ_BEAT_CURVE_PUNCH, 0, 1000, 94, 100, 4, 420, 24, 5, 0, VJ_BEAT_COST_CHEAP, 96, 0, 0, VJ_BEAT_GROUP_NONE, 0)
        };
...
#pragma omp single
    {
        ...
        if(flicker_intensity > 0 && flicker_freq > 0 && (vf->framecounter % flicker_freq) == 0) {
            ...
            vf->global_gain = (256 * (100 + flick_mod)) / 100;
        }
        ...
        if(flicker_q > 0) {
            ...
            vf->global_gain += (256 * drive_mod) / 100;
        }

        vf->global_gain = clampi(vf->global_gain, 72, 384);
        ...
        vf->frame_seed = vintagefilm_rng(&vf->rng_state);
    }

    const uint32_t frame_seed = vf->frame_seed;
    const int global_gain = vf->global_gain;
    ...
    const vintagefilm_gain_fn gain_fn =
        vignette_q8 > 0 ? vintagefilm_gain_vignette : vintagefilm_gain_plain;
    const vintagefilm_dust_fn dust_fn =
        dust_intensity > 0 ? vintagefilm_dust_apply : vintagefilm_dust_none;
    const vintagefilm_grain_fn grain_fn =
        grain_strength > 0 ? vintagefilm_grain_apply : vintagefilm_grain_none;
...
#pragma omp for schedule(static)
    for(int y = 0; y < height; y++) {
        ...
        for(int x = 0; x < width; x++) {
            ...
            const int final_gain = gain_fn(vignette, idx, global_gain, vignette_q8);
            ...
            yv = dust_fn(yv, p_noise, dust_gate, dirt_q);
            yv = grain_fn(yv, p_noise, grain_strength);
