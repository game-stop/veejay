...
#pragma omp for schedule(static)
    for (int plane = 0; plane < 4; plane++) {
        uint8_t *dst = plane == 0 ? t->life[1] :
                       plane == 1 ? t->nutrient[1] :
                       plane == 2 ? t->charge[1] : t->pigment[1];
        const uint8_t *src = plane == 0 ? t->life[0] :
                             plane == 1 ? t->nutrient[0] :
                             plane == 2 ? t->charge[0] : t->pigment[0];
        veejay_memcpy(dst, src, t->len);
    }
