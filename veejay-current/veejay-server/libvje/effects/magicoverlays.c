...
vj_effect *overlaymagic_init(int w, int h)
{
	...
	ve->beat_hints = vje_build_beat_hint_list(
		...
		VJ_BEAT_SELECTOR, VJ_BEAT_F_REJECT | VJ_BEAT_F_STRUCTURAL, VJ_BEAT_SOFT_UNSET, VJ_BEAT_SOFT_UNSET, 0, 0, 0, 0, 0, -1000  /* Keep or clear color */
	);
...
#pragma omp parallel for simd schedule(static) num_threads(n_threads)
	for (int i = 0; i < len; i++)
	{
		...
		int b = Y2[i];
		int c = a * b >> 8;
		int d = c + a * ((255 - (((255 - a) * (255 - b)) >> 8) - c) >> 8);	//8
		Y[i] = CLAMP_Y(d);
	}
}
...
void overlaymagic_freeze(VJFrame *frame, VJFrame *frame2, int n_threads )
{
    ...
    const int safe_lo = (pixel_Y_lo_ < 1) ? 1 : pixel_Y_lo_;
#pragma omp parallel for simd schedule(static) num_threads(n_threads)
    for (int i = 0; i < len; i++)
    {
        int a = Y[i];
