...
#pragma omp simd reduction(min:a) reduction(max:b)
    for(int i = 0; i < len; i++) {
...
static int shapewipe_apply_internal(shape_t *s,
                                    VJFrame *frame,
                                    VJFrame *frame2,
                                    double timecode,
                                    int shape,
                                    ...
                                    int mix_drive)
{
    ...
    const int musical_active = softness > 0 || edge_glow > 0 || wipe_drive > 0 || mix_drive > 0 || threshold > 0;
...
#pragma omp single
    {
        ...
        if(musical_active) {
            ...
            if(range <= 0) range = 1;
            double tc = timecode;
            if(tc < 0.0) tc = 0.0; else if(tc > 1.0) tc = 1.0;
            if(direction) {
                if(automatic) base_threshold = (int)(tc * (double)range) + s->shape_min + threshold;
            } else {
                if(automatic) base_threshold = (int)((1.0 - tc) * (double)range) + s->shape_min - threshold;
            }
    ...
    if(src && src->data[0]) {
        if(!musical_active) {
            ...
            if(direction) {
                if(automatic) done_threshold = (int)(tc * (double)range) + s->shape_min;
                shape_wipe_1(frame->data, frame2->data, src->data[0], frame->len, done_threshold);
            } else {
                if(automatic) done_threshold = (int)((1.0 - tc) * (double)range) + s->shape_min;
                shape_wipe_2(frame->data, frame2->data, src->data[0], frame->len, done_threshold);
            }
            
#pragma omp single
            {
                if(direction) s->shape_completed = (done_threshold >= s->shape_max);
                else          s->shape_completed = (done_threshold <= s->shape_min);
            }
        } 
        else {
...
#pragma omp single
            {
                ...
                if(direction) {
                    if(automatic) done_threshold = (int)(tc * (double)range) + s->shape_min;
                    s->shape_completed = (done_threshold >= s->shape_max);
                } else {
                    if(automatic) done_threshold = (int)((1.0 - tc) * (double)range) + s->shape_min;
                    s->shape_completed = (done_threshold <= s->shape_min);
                }
            }
        }
    }
...
static void shape_update_active_shape(shape_t *s, int shape, int w, int h)
{
    shape = clampi(shape, 0, s->shapeidx - 1);
    
    if(shape != s->currentshape) {
#pragma omp critical (shape_update_crit)
        {
            if(shape != s->currentshape) {
                void *newshape = change_shape(s, s->selected_shape, shape, w, h);
                if(newshape != s->selected_shape || s->currentshape == shape) {
                    s->selected_shape = newshape;
                    s->currentshape = shape;
                    s->have_smooth = 0;
                            ...
                            int mix_drive)
{
    ...
    shape_update_active_shape(s, shape, frame->width, frame->height);

    /* 
     * FX Chain entry point: OpenMP parallel region is already active 
     * from the caller, so we drop straight into the shared worker.
     */
    return shapewipe_apply_internal(s, frame, frame2, timecode, shape, threshold,
                                    direction, automatic, softness, edge_glow, wipe_drive, mix_drive);
}
                      ...
                      int automatic)
{
    ...
    /* 
     * Transition sample entry point: Because this runs outside an active 
     * FX parallel region, we explicitly spawn a team here so the inner loops 
     * and workshares utilize all CPU threads.
     */
#pragma omp parallel if(vje_is_parallel_enabled()) num_threads(vje_max_threads(frame->len))
    {
        int res = shapewipe_apply_internal(
            s,
            frame,
            frame2,
            timecode,
