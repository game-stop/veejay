...
vj_effect *squares_init(int w, int h)
{
    ...
    ve->param_description = vje_build_param_list(
        ve->num_params, "Radius", "Mode", "Orientation", "Parity", "Phase X", "Phase Y", "Size Drive", "Mix Drive"
    );
    
    ve->hints = vje_init_value_hint_list(ve->num_params);
    vje_build_value_hint_list(ve->hints, ve->limits[1][P_MODE], P_MODE, "Average", "Min", "Max");
    ...
    vje_build_value_hint_list(ve->hints, ve->limits[1][P_PARITY], P_PARITY, "Even", "Odd", "No parity");
                                 ...
                                 int n_threads)
{
    ...
    grid_getbounds_from_orientation(radius, (vj_effect_orientation)orientation, (vj_effect_parity)parity, &x_inf, &y_inf, &x_sup, &y_sup, w, h);
    ...
    if(x_sup <= x_inf || y_sup <= y_inf) return;
    ...
    for(int by = 0; by < ny; by++) {
        ...
        for(int bx = 0; bx < nx; bx++) {
            ...
            int sum_y = 0, sum_u = 0, sum_v = 0, count = 0;
            ...
            uint8_t out_y = (mode == 1) ? min_y : ((mode == 2) ? max_y : (uint8_t)((sum_y + (count >> 1)) / count));
            const uint8_t out_u = squares_u8(128 + ((sum_u >= 0) ? ((sum_u + (count >> 1)) / count) : -((-sum_u + (count >> 1)) / count)));
            const uint8_t out_v = squares_u8(128 + ((sum_v >= 0) ? ((sum_v + (count >> 1)) / count) : -((-sum_v + (count >> 1)) / count)));
...
void squares_apply(void *ptr, VJFrame *frame, int *args)
{
    ...
    #pragma omp single
    {
        ...
        int base_radius = clampi(base_radius_arg, 1, max_radius);
        int size_headroom = max_radius - base_radius;
        int size_drive = clampi(size_drive_arg, 0, 1000);
        
        int target_radius = base_radius;
        if(size_headroom > 0) target_radius += (size_drive * size_headroom + 500) / 1000;
        ...
        int phase_x = squares_smooth_i(&s->phase_x_state, phase_x_target, fast * 1.30f, slow);
        int phase_y = squares_smooth_i(&s->phase_y_state, phase_y_target, fast * 1.30f, slow);
        mix_target = squares_smooth_i(&s->mix_drive_state, mix_target, fast * 1.18f, slow);
        
        s->radius = clampi(radius, 1, max_radius);
        s->phase_x = phase_x;
        s->phase_y = phase_y;
        s->source_mix_q8 = clampi((mix_target * 256 + 500) / 1000, 0, 256);
    }
    
    squares_apply_blocks(frame, s->buf[0], s->buf[1], s->buf[2], s->radius, mode, orientation, parity, s->phase_x, s->phase_y, s->source_mix_q8, s->n_threads);
}