...
typedef uint8_t (*squares_luma_fn)(int sum_y, int count, uint8_t min_y, uint8_t max_y);
typedef void (*squares_store_fn)(uint8_t *Y, uint8_t *U, uint8_t *V,
                                 const uint8_t *srcY, const uint8_t *srcU,
                                 const uint8_t *srcV, int idx, uint8_t out_y,
                                 uint8_t out_u, uint8_t out_v, int q8);

static uint8_t squares_luma_average(int sum_y, int count, uint8_t min_y, uint8_t max_y)
{
    (void)min_y;
    (void)max_y;
    return (uint8_t)((sum_y + (count >> 1)) / count);
}

static uint8_t squares_luma_minimum(int sum_y, int count, uint8_t min_y, uint8_t max_y)
{
    (void)sum_y;
    (void)count;
    (void)max_y;
...
static uint8_t squares_luma_maximum(int sum_y, int count, uint8_t min_y, uint8_t max_y)
{
    (void)sum_y;
    (void)count;
    (void)min_y;
...
static void squares_store_direct(uint8_t *Y, uint8_t *U, uint8_t *V,
                                 const uint8_t *srcY, const uint8_t *srcU,
                                 const uint8_t *srcV, int idx, uint8_t out_y,
                                 uint8_t out_u, uint8_t out_v, int q8)
{
...
static void squares_store_mixed(uint8_t *Y, uint8_t *U, uint8_t *V,
                                const uint8_t *srcY, const uint8_t *srcU,
                                const uint8_t *srcV, int idx, uint8_t out_y,
                                uint8_t out_u, uint8_t out_v, int q8)
{
    Y[idx] = squares_blend_u8(out_y, srcY[idx], q8);
    U[idx] = squares_blend_u8(out_u, srcU[idx], q8);
    V[idx] = squares_blend_u8(out_v, srcV[idx], q8);
}
                                 ...
                                 int source_mix_q8)
{
    ...
    squares_luma_fn luma;
    const squares_store_fn store =
        source_mix_q8 > 0 ? squares_store_mixed : squares_store_direct;

    switch(mode) {
        case 1:
            luma = squares_luma_minimum;
            break;
        case 2:
            luma = squares_luma_maximum;
            break;
        case 0:
        default:
            luma = squares_luma_average;
            break;
    }
...
#pragma omp for schedule(static)
    for(int by = 0; by < ny; by++) {
        ...
        for(int bx = 0; bx < nx; bx++) {
            ...
            const uint8_t out_y = luma(sum_y, count, min_y, max_y);
            ...
            for(int yy = y0; yy < y1; yy++) {
                ...
                for(int xx = x0; xx < x1; xx++) {
                    ...
                    store(Y, U, V, srcY, srcU, srcV, idx, out_y, out_u, out_v,
                          source_mix_q8);
                }
            }
        }
...
#pragma omp single
    {
        if(!s->initialized) {
            ...
            squares_smooth_i(&s->radius_state, target_radius, fast, slow);
            squares_smooth_i(&s->phase_x_state, phase_x_target, fast * 1.30f, slow);
            squares_smooth_i(&s->phase_y_state, phase_y_target, fast * 1.30f, slow);
            squares_smooth_i(&s->mix_drive_state, mix_target, fast * 1.18f, slow);
        }
    }

    int radius = clampi((int)(s->radius_state + 0.5f), 1, max_radius);
    phase_x = (int)(s->phase_x_state + 0.5f);
    phase_y = (int)(s->phase_y_state + 0.5f);
    mix_target = (int)(s->mix_drive_state + 0.5f);
