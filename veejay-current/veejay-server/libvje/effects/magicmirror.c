...
#pragma omp single
    ...
    motion = motionmap_active(m->motionmap);
    if(motion) {
        x_wave = m->cx1;
        y_wave = m->cx2;
    }
...
#pragma omp sections
    {
#pragma omp section
        { veejay_memcpy(m->magicmirrorbuf[0], frame->data[0], len); }
#pragma omp section
        { veejay_memcpy(m->magicmirrorbuf[1], frame->data[1], len); }
#pragma omp section
        { veejay_memcpy(m->magicmirrorbuf[2], frame->data[2], len); }
#pragma omp section
        {
            if(alpha)
                veejay_memcpy(m->magicmirrorbuf[3], frame->data[3], len);
        }
    }
