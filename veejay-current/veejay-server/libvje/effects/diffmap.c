...
vj_effect *differencemap_init(int w, int h)
{
...

    {
        const vj_beat_param_hint_t beat_hints[] = {
            VJ_BEAT_HINT_V2(VJ_BEAT_DETAIL, VJ_BEAT_F_CONTINUOUS | VJ_BEAT_F_NO_ZERO_CROSS, VJ_BEAT_SRC_SCRATCH_ACTIVITY, VJ_BEAT_OP_MAP_RANGE, VJ_BEAT_POLARITY_NEGATIVE, VJ_BEAT_CURVE_EASE_OUT, 8, 180, 78, 100, 15, 520, 0, 1, 0, VJ_BEAT_COST_CHEAP, 100, 0, 0, VJ_BEAT_GROUP_NONE, 0),
...
void differencemap_apply(void *ptr, VJFrame *frame, VJFrame *frame2, int *args)
{
    ...
    if(show)
    {
        ...
        {
            #pragma omp section
            { veejay_memcpy(Y, binary_img, len); }
            #pragma omp section
