...
#define EL_FILE_BITS        12ULL
#define EL_FRAME_BITS       52ULL
#define EL_FILE_MASK        0xfffULL
#define EL_FRAME_MASK       0xfffffffffffffULL

#define N_EL_FRAME(x)       ((uint64_t)(x) & EL_FRAME_MASK)
#define N_EL_FILE(x)        ((uint32_t)(((uint64_t)(x) >> EL_FRAME_BITS) & EL_FILE_MASK))
...
editlist *vj_el_init_with_args(char **filenames,
                               int n,
                               int flags,
                               int deinter,
                               int force,
...
editlist *vj_el_dummy(int flags,
                      int deinterlace,
                      int chroma,
                      char norm,
                      int width,
...
int open_video_file(char *filename,
                    editlist *el,
                    int preserve_pathname,
                    int deinter,
                    int force,
                    char override_norm,
                    int w,
...
int vj_el_auto_detect_scenes(editlist *el,
                             uint8_t *tmp[4],
                             int w,
                             int h,
                             int dl_threshold);
