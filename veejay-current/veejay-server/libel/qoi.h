...
void *qoi_encode(const unsigned char *pixels[4], const qoi_desc *desc, int *out_len, unsigned char *bytes, int max_size)
{
	...
	if (
		...
		max_size < QOI_HEADER_SIZE + sizeof(qoi_padding) ||
	...
	{
		px.rgba.r = pixels[0][px_pos];
		px.rgba.g = pixels[1][px_pos / 2];
		px.rgba.b = pixels[2][px_pos / 2];

		if (channels == 4)
		{
			px.rgba.a = pixels[3][px_pos];
		}
...
/*			else if (b1 == QOI_OP_RGBA) {
		...
		pixels[0][px_pos] = px.rgba.r;
		pixels[1][px_pos >>1 ] = px.rgba.g;
		pixels[2][px_pos >>1 ] = px.rgba.b;
		
/*		if (channels == 4) {
			pixels[3][px_pos] = px.rgba.a;
		}*/
	}
