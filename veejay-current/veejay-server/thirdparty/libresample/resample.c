...
#include <libresample/audioconvert.h>
#include <libavutil/opt.h>
#include <libavutil/mem.h>
#include <libavutil/samplefmt.h>
#include <libavutil/common.h>
#include <libresample/common.h>
                                        ...
                                        int linear, double cutoff)
{
	...
	int i;
	int nb_samples = 256 * 1024;
    for (i = 0; i < s->filter_channels; i++) {
        s->bufin[i] = av_malloc_array(nb_samples, sizeof(short));
        s->bufout[i] = av_malloc_array(nb_samples, sizeof(short));

        if (!s->bufin[i] || !s->bufout[i]) {
    ...
    s->resample_context = vj_av_resample_init(output_rate, input_rate,
...
void vj_audio_resample_close(void *ptr)
{
	ReSampleContext *s = (ReSampleContext*) ptr;
    int i;
    vj_av_resample_close(s->resample_context);
    for (i = 0; i < s->filter_channels; i++)
        av_freep(&s->temp[i]);
    av_freep(&s->buffer[0]);
    av_freep(&s->buffer[1]);
