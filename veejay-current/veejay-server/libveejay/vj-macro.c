...
static inline int vj_macro_valid_bank(int bank)
{
    return bank >= 0 && bank <= MAX_MACRO_BANKS;
}

static int vj_macro_ensure_bank(vj_macro_t *macro, int bank)
{
    if(!macro || !vj_macro_valid_bank(bank))
        return 0;

    if(macro->macro_bank[bank] == NULL) {
        macro->macro_bank[bank] = vpn(VEVO_ANONYMOUS_PORT);
        if(!macro->macro_bank[bank])
            return 0;
    }
...
void vj_macro_free(void *ptr)
{
    ...
    if(macro->playback_bank) {
        vpf(macro->playback_bank);
        macro->playback_bank = NULL;
    }
...
static int vj_macro_bank_playback_clear(void *ptr)
{
    vj_macro_t *macro = (vj_macro_t*) ptr;
    if(!macro)
        return 0;

    if(macro->playback_bank != NULL) {
        vpf(macro->playback_bank);
        macro->playback_bank = NULL;
    }

    macro->playback_bank = vpn(VEVO_ANONYMOUS_PORT);
    return macro->playback_bank != NULL;
}

static void vj_macro_free_block(macro_block_t* in);

static macro_block_t *vj_macro_clone_block(macro_block_t *in)
{
    if(!in)
        return NULL;

    macro_block_t *m = (macro_block_t*) vj_calloc(sizeof(macro_block_t));
    if(!m)
        return NULL;

    for(int i = 0; i < in->num_msg; i++) {
        if(!in->msg[i])
            continue;

        m->msg[m->num_msg] = strdup(in->msg[i]);
        if(!m->msg[m->num_msg]) {
            vj_macro_free_block(m);
            return NULL;
        }
        m->num_msg++;
    }
...
static void vj_macro_clear_bank_item(void *bank, char *item)
{
    if(!bank || !item)
        return;

    void *data = NULL;
    int error = vevo_property_get(bank, item, 0, &data);
    if(error == VEVO_NO_ERROR)
        vj_macro_free_block(data);

    vevo_property_del(bank, item);
}

static int vj_macro_clone_bank_item(void *input, void *output, char *item)
{
    if(!input || !output || !item)
        return 0;

    void *data = NULL;
    int error = vevo_property_get(input, item, 0, &data);
    if(error != VEVO_NO_ERROR)
        return 0;

    macro_block_t *blk = vj_macro_clone_block((macro_block_t*) data);
    if(!blk) {
        return 0;
    }

    error = vevo_property_set(output, item, VEVO_ATOM_TYPE_VOIDPTR, 1, &blk);
	if( error != VEVO_NO_ERROR ) {
		vj_macro_free_block(blk);
		return 0;
	}
...
static int vj_macro_bank_prepare_playback(void *ptr)
{
    vj_macro_t *macro = (vj_macro_t*) ptr;
    if(!macro || !vj_macro_ensure_bank(macro, macro->current_bank))
        return 0;
    ...
    if(!vj_macro_bank_playback_clear(ptr)) {
    ...
    macro->play_bank = macro->current_bank;

    int i;
    for(i = 0; items[i] != NULL; i++) {
        vj_macro_clone_bank_item(macro->macro_bank[macro->current_bank], macro->playback_bank, items[i]);
        free(items[i]);
    }
    free(items);
...
{
	snprintf(key,key_len,"%08ld%02d%08d", frame_num, at_dup, at_loop );
}
...
char **vj_macro_play_event(void *ptr, char *key)
{
    ...
    if(!macro->playback_bank || vevo_port_is_empty(macro->playback_bank)) {
		if( vj_macro_bank_prepare_playback( ptr ) == 0 )
				return NULL;
	} 

	void *mb = NULL;
	int error = vevo_property_get(macro->playback_bank, key, 0, &mb );
	if( error != VEVO_NO_ERROR )
		return NULL;
...
void vj_macro_finish_event(void *ptr, char *key)
{
    ...
    if(!macro)
        return;
    vj_macro_clear_bank_item(macro->playback_bank, key);
}
...
{
    vj_macro_t *macro = (vj_macro_t*) ptr;
    if(!macro || !message || !vj_macro_ensure_bank(macro, macro->current_bank))
        return 0;
    ...
    if(error != VEVO_NO_ERROR) {
        ...
        if(vevo_property_set(macro->macro_bank[macro->current_bank], key, VEVO_ATOM_TYPE_VOIDPTR, 1, &m) != VEVO_NO_ERROR) {
            vj_macro_free_block(m);
            return 0;
        }
    }
    ...
    if(macro->current_bank == macro->play_bank && macro->playback_bank)
        vj_macro_clone_bank_item(macro->macro_bank[macro->current_bank], macro->playback_bank, key);
...
static  void vj_macro_reclaim_block( macro_block_t *blk, int seq_no )
{
    ...
    if( seq_no < 0 || seq_no >= blk->num_msg )
        return;
...
void    vj_macro_del(void *ptr, long frame_num, int at_dup, int at_loop, int seq_no)
{
	...
	if( macro->play_bank == macro->current_bank ) {
		vj_macro_clear_bank_item( macro->playback_bank, key );
	}
...
{
    vj_macro_t* macro = (vj_macro_t*) ptr;
    if(!macro || !vj_macro_valid_bank(macro->current_bank) || !macro->macro_bank[macro->current_bank])
        return;
    ...
    for(i = 0; items[i] != NULL; i++) {
        ...
        if(vevo_property_get(macro->macro_bank[macro->current_bank], items[i], 0, &mb) == VEVO_NO_ERROR)
            vj_macro_free_block((macro_block_t*)mb);
        free(items[i]);
    }
	free(items);
...
int vj_macro_select( void *ptr, int slot )
{
    ...
    if(!vj_macro_valid_bank(slot))
        return 0;

    if(!vj_macro_ensure_bank(macro, slot))
        return 0;
...
{
    vj_macro_t *macro = (vj_macro_t*) ptr;
    if(!macro || !vj_macro_valid_bank(macro->current_bank))
        return 1;

    return 1 + macro->loop_stat_stop[macro->current_bank];
}
...
int vj_macro_set_loop_stat_stop( void *ptr, int stop)
{
    ...
    else if(stop > macro->loop_stat_stop[macro->current_bank])
...
{
	int i;
    char **messages = vj_macro_pull(ptr, frame_num, at_dup, at_loop);
    if(messages != NULL) {
        int msglen = 0;
...
static char *vj_macro_flatten_messages(void *ptr, char **messages, long frame_num, int at_dup, int at_loop, int len)
{
    ...
    for(i = 0; messages[i] != NULL; i++) {
        ...
        int written = snprintf(buf_ptr,
                               (size_t)(len - (buf_ptr - buf)) + 1u,
                               "%08ld%02d%08d%02d%03d%s",
                               frame_num,
                               at_dup,
                               at_loop,
                               i,
...
char* vj_macro_serialize_macro(void *ptr, long frame_num, int at_dup, int at_loop )
{
	...
	char *res;
	if(len == 0 || messages == NULL) {
		if(messages)
			free(messages);
		return NULL;
	}		
	res = vj_macro_flatten_messages( ptr, messages, frame_num, at_dup, at_loop, len );
	free(messages);
	return res;
}
...
char *vj_macro_serialize(void *ptr)
{
    ...
    char header[9];
    snprintf(header, sizeof(header), "%08d", (int)len);
    memcpy(buf, header, 8);
...
static void vj_macro_load_messages( void *ptr, char *key, xmlDocPtr doc, xmlNodePtr cur )
{
    ...
    if( sscanf( key, "%08ld%02d%08d", &frame_num, &at_dup, &at_loop ) == 3 ) {
	    ...
	    if(frame_num == 0 && at_loop == 0 && at_dup == 0 ) {
			//this is not valid
			return;
    	}
