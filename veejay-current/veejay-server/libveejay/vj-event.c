...
static int to_master_capacity = 128;
static int to_master_n_queued = 0;
...
static void init_vims_forward_cache(void) {
    ...
    for( int i = 0; dont_forward_vims[i].vims_id > 0; i ++ ) {
        never_forward_vims_cache_[
            dont_forward_vims[i].vims_id
        ] = 1;
    }

    memset(maybe_forward_vims_cache_, 0, sizeof(maybe_forward_vims_cache_));
    for( int i = 0; optional_forward_vims[i].vims_id > 0; i ++ ) {
        maybe_forward_vims_cache_[
...
static inline int SAMPLE_PLAYING(veejay_t *v)
{
    return vj_get_playback_mode(v) == VJ_PLAYBACK_MODE_SAMPLE;
}

static inline int STREAM_PLAYING(veejay_t *v)
{
    return vj_get_playback_mode(v) == VJ_PLAYBACK_MODE_TAG;
}

static inline int PLAIN_PLAYING(veejay_t *v)
{
    return vj_get_playback_mode(v) == VJ_PLAYBACK_MODE_PLAIN;
}
...
\
 ...
 if(args == 0) args = (v->remote_id > 0 ? v->remote_id : v->uc->sample_id);\
}
...
static void vj_event_reanchor_live_sequence_marker(veejay_t *v, int sample_id)
{
    ...
    sample_set_resume_override(sample_id, target);

    vj_perform_reset_transition(v);

    if((long long)target != cur)
        veejay_set_frame(v, target);
}
...
void    vj_event_fire_net_event(veejay_t *v, int net_id, char *str_arg, int *args, int arglen, int prefixed)
{
    ...
    vj_event_vevo_inline_fire( (void*) v, net_id,   
                ...
                vims_arguments[16].value,
                vims_arguments[17].value,
                vims_arguments[18].value,
                vims_arguments[19].value
            );
    fmt_offset = 1;
    for ( i = 0; i < np ; i ++ )
    {
...
int vj_event_parse_msg( void *ptr, char *msg, int msg_len )
{
    ...
    if(msg_len < MSG_MIN_LEN) {
        veejay_msg(VEEJAY_MSG_ERROR, "Dropped VIMS message that was too small");
        return 0;
    }

    int effective_len = msg_len;
    while(effective_len > 0 &&
         (msg[effective_len-1] == '\n' ||
          msg[effective_len-1] == '\r' ||
          msg[effective_len-1] == ';'  ||
          msg[effective_len-1] == ' '))
    {
        effective_len--;
    ...
    if(effective_len >= 3 && strncasecmp(msg, "BUN", 3) == 0) {
        char *tmp = vj_strndup(msg, effective_len);
        vj_event_parse_bundle(v, tmp);
        free(tmp);
        return 1;
    }

    if(effective_len >= 3 && strncasecmp(msg, "KEY", 3) == 0) {
        vj_event_parse_kf(v, (char*)msg, effective_len);
        return 1;
    }
    ...
    const char *current_msg = msg;
    int current_len = effective_len;

    int remote_id, remote_fx_list_entry;

    /* optional rmodes parsing: support "%03d:%d %d %d:" */
    if (current_len > 5 && current_msg[3] == ':')
    {
        int rmode = -1;
        int parsed_offset = 0;
    ...
    if(net_id <= 0 || net_id >= VIMS_MAX) {
        veejay_msg(VEEJAY_MSG_ERROR, "VIMS Selector %d invalid in message '%s'", net_id,current_msg);
        return 0;
    }

    if (net_id >= VIMS_BUNDLE_START && net_id < VIMS_BUNDLE_END) {
        vj_msg_bundle *bun = vj_event_bundle_get(net_id);
        if(bun) vj_event_parse_bundle(v, bun->bundle);
        return 1;
    }
    ...
    else if(STREAM_PLAYING(v)) macro = vj_tag_get_macro(v->uc->sample_id);

    if(macro && vj_macro_get_status(macro) == MACRO_REC) {
        if(vj_macro_is_vims_accepted(net_id)) {
            int at_dup = 0, at_loop = 0;
            vj_event_macro_get_loop_dup(v, &at_dup, &at_loop);

            char *macro_tmp = vj_strndup(msg, effective_len);
            vj_macro_put(macro, macro_tmp, atomic_load_long_long(&v->settings->current_frame_num), at_dup, at_loop);
            free(macro_tmp);
        }
    }
...
static int vj_parse_and_queue_to_master(
    veejay_t *v,
    const char *buf,
    int len,
    char **to_master_buf,
    int *to_master_size,
    int *to_master_capacity)
{
    if(!v || !buf || len <= 0)
        return 0;
    ...
    char *dst = *to_master_buf + *to_master_size;

    memcpy(dst, buf, prefix_len);
    dst += prefix_len;
...
char *vj_bundle_format_messages(
    int msg_count,
    const char *messages_in)
{
    ...
    size_t required_len = 4 + 3 + 1 + strlen(messages_in) + 2;

    char *bundle_buf = (char *)malloc(required_len);
    if (!bundle_buf)
        return NULL;

    int written = snprintf(bundle_buf, required_len, "BUN:%03d{%s}", msg_count, messages_in);

    if (written > 0 && written < (int)required_len) {
        return bundle_buf;
    }
...
int vj_event_parse_and_maybe_requeue_events(void *ptr, char *buf, int len) {
    ...
    if( to_master_buf == NULL ) {
        to_master_buf = malloc( sizeof(char) * to_master_capacity);
        if(to_master_buf) {
            to_master_buf[0] = '\0';
        }
    }
    ...
    if( to_master_buf != NULL ) {
        ret = vj_parse_and_queue_to_master(
            v,
            buf,
            len,
            &to_master_buf,
...
void vj_event_dispatch_requeued_events(veejay_t *v) {
    ...
    if(to_master_buf) {
        free(to_master_buf);
        to_master_buf = NULL;
        to_master_capacity = 128;
        to_master_size = 0;
        to_master_n_queued = 0;
    }
}
...
void vj_event_update_remote(void *ptr)
{
    ...
    if( v->settings->use_vims_mcast )
    {
        ...
        if( res > 0 ) {
            ...
            if(res > 0)
            {
                ...
                char *buf = NULL;
                int len =0;
                while( ( buf = vj_server_retrieve_msg( v->vjs[VEEJAY_PORT_MAT], 0, &len )) != NULL )
                {   
                    vj_event_parse_and_maybe_requeue_events( v, buf,len );
                }
            }
        }
    }
    ...
    {   
        if( vj_server_link_can_read( v->vjs[VEEJAY_PORT_CMD], i ) )
        {
            ...
            if(res>0)
            {
                ...
                int len = 0;
                char *buf  = NULL;
                while( (buf= vj_server_retrieve_msg(v->vjs[VEEJAY_PORT_CMD],i, &len))!= NULL )
                {
                    vj_event_parse_and_maybe_requeue_events( v, buf, len);
                }
            }
    ...
    {   
        if( vj_server_link_can_read( v->vjs[VEEJAY_PORT_DAT], i ) )
        {
            ...
            if(res>0)
            {
                ...
                int len = 0;
                char *buf  = NULL;
                while( (buf = vj_server_retrieve_msg(v->vjs[VEEJAY_PORT_DAT],i, &len))!= NULL )
                {
                    vj_event_parse_and_maybe_requeue_events( v, buf,len );
                }
            }
    ...
    vj_event_dispatch_requeued_events(v);
...
static void vj_event_xml_parse_config( veejay_t *v, xmlDocPtr doc, xmlNodePtr cur )
{
    ...
    while( cur != NULL )
    {
        ...
        get_istr( doc, cur, (const xmlChar*) XML_CONFIG_SETTING_BEZERK, &(v->bezerk) );
...
{
    veejay_t *v = (veejay_t*) ptr;
    if(v->bezerk) v->bezerk = 0; else v->bezerk = 1;
    if(v->bezerk==0)
        veejay_msg(VEEJAY_MSG_INFO,"Bezerk Off  :No sample-restart when changing input channels");
    else
        veejay_msg(VEEJAY_MSG_INFO,"Bezerk On :Sample-restart when changing input channels"); 
}
...
void vj_event_vims_message_forwarding( void *ptr, const char format[], va_list ap )
{
    ...
    if(!v->is_master) {
        veejay_msg(VEEJAY_MSG_INFO, "%s trickplay/sample switching to master veejay",
            (v->uc->vims_mirror == 0 ? "Not forwarding" : "Forwarding"));
    } else {
        veejay_msg(VEEJAY_MSG_INFO, "%s trickplay events from other veejays",
            (v->uc->vims_mirror == 0 ? "Ignoring" : "Consuming"));
    }
}
    ...
    if(vj_event_valid_mode(args[1]))
    {
        if(args[1] == VJ_PLAYBACK_MODE_PLAIN) 
        {
            if( vj_has_video(v,v->edit_list) )
                veejay_change_playback_mode_transition(v, args[1], 0);
            else
                veejay_msg(VEEJAY_MSG_ERROR,
        ...
        if(args[1] == VJ_PLAYBACK_MODE_SAMPLE) 
        {
            ...
            {
                veejay_change_playback_mode_transition(v,args[1] ,args[0]);
            }
            else
            {   
                p_no_sample(args[0]);
...
void vj_event_send_keylist(void *ptr, const char format[], va_list ap)
{
...
#ifdef HAVE_SDL
    size_t blob_size = 1024 * 64;
    char *blob = vj_calloc(blob_size);
    if (!blob)
        return;
        ...
        while ((node = hash_scan_next(&scan)) != NULL)
        {
            ...
            if (ev->event_id >= VIMS_BUNDLE_START && ev->event_id < VIMS_BUNDLE_END)
            {
                ...
                if (vj_event_bundle_exists(ev->event_id))
                {
                    ...
                    {
                        skip = 0;
                        snprintf(message, sizeof(message), "%s", mm->bundle);
                    }
                }
            }
            else
            {
                if (ev->arguments)
                    snprintf(message, sizeof(message), "%03d:%s;", ev->event_id, ev->arguments);
                else
                    snprintf(message, sizeof(message), "%03d:;", ev->event_id);
            }

            if (!skip)
            {
                int msg_len = (int)strlen(message);
                int n = snprintf(p, blob_size - (p - blob),
                                 "%04d%03d%03d%03d%s",
                                 ev->event_id,
                                 ev->key_mod,
                                 ev->key_symbol,
    ...
    char header[7];
    snprintf(header, sizeof(header), "%06d", len);
    SEND_MSG(v, header);
...
void vj_event_send_bundles(void *ptr, const char format[], va_list ap)
{
    ...
    for (int i = 0; i <= 600; i++)
    {
        if (i >= VIMS_BUNDLE_START && i < VIMS_BUNDLE_END)
        {
            ...
            vj_msg_bundle *m = vj_event_bundle_get(i);
            if (!m || !m->bundle)
                continue;
...
void vj_event_sample_select(void *ptr, const char format[], va_list ap)
{
    ...
    if(sample_exists(args[0]))
    {
        veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_SAMPLE,args[0] );
    }
    else
    {
        p_no_sample(args[0]);
...
void vj_event_tag_select(void *ptr, const char format[], va_list ap)
{
    ...
    if(vj_tag_exists(args[0]))
    {
        veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_TAG,args[0]);
    }
    else
    {
        p_no_tag(args[0]);
...
void vj_event_switch_sample_tag(void *ptr, const char format[], va_list ap)
{
    ...
    if(!STREAM_PLAYING(v) && !SAMPLE_PLAYING(v))
    {
        ...
        {
            veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_SAMPLE, v->last_sample_id);
            return;
        }
        if(vj_tag_exists(v->last_tag_id))
        {
            veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_TAG, v->last_tag_id);
            return;
        }
        if(sample_size() <= 0)
        {
            ...
            {
                veejay_change_playback_mode_transition( v, VJ_PLAYBACK_MODE_TAG, last_tag);
                return;
            }
        }   
    }
    ...
    else if(STREAM_PLAYING(v))
    {
        ...
        {
            veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_SAMPLE,last_sample);
        }
    }
}
...
void vj_event_sample_set_volume(void *ptr, const char format[], va_list ap)
{
    ...
    if(args[1] < 0 || args[1] > 100) {
        veejay_msg(VEEJAY_MSG_ERROR, "Use a value between 0-100 for sample audio volume");
        return;
    }

    if(sample_set_audio_volume(args[0], args[1]) == -1) {
        veejay_msg(VEEJAY_MSG_ERROR, "Cannot set sample %d audio volume", args[0]);
        return;
    }
...
{
    veejay_t *v = (veejay_t*) ptr;
    int args[1] = { VJ_AUDIO_MIX_FOLLOW_ROUTE };
    int mode;

    P_A(args, sizeof(args), NULL, 0, format, ap);

    mode = vj_perform_set_audio_mix_mode(v, args[0]);
    ...
    switch(mode) {
        ...
        default:
            veejay_msg(VEEJAY_MSG_INFO, "Audio mixer mode set to follow active route");
            break;
    }
}
...
void vj_event_set_play_mode(void *ptr, const char format[], va_list ap)
{
    ...
    if(vj_event_valid_mode(args[0]))
    {
        ...
        if(mode == VJ_PLAYBACK_MODE_TAG)
        {
            ...
            if(!vj_tag_exists(target_id))
                target_id = last_id;
            if(vj_tag_exists(target_id))
            {
                veejay_change_playback_mode_transition(
                    v, VJ_PLAYBACK_MODE_TAG, target_id);
            }
        }
        if(mode == VJ_PLAYBACK_MODE_PLAIN)
        {
            if(vj_has_video(v,v->edit_list) )
                veejay_change_playback_mode_transition( v, VJ_PLAYBACK_MODE_PLAIN, 0);
            else
                veejay_msg(VEEJAY_MSG_ERROR,
                 "There are no video files in the editlist");
        }
...
static const char *vj_event_record_audio_source_name(int source)
{
    switch(source)
    {
...
void vj_event_record_audio_source(void *ptr, const char format[], va_list ap)
{
    ...
    veejay_t *v = (veejay_t *)ptr;
    int args[1] = { VJ_RECORD_AUDIO_SOURCE_AUTO };
    int source;
...
void vj_event_sample_rec_start( void *ptr, const char format[], va_list ap)
{
    ...
    if( args[0] == 0 )
    {
        ...
        else
        {
            ...
            args[0] = vj_sequence_selected_duration(v);
...
static void vj_event_cancel_transition_target(veejay_t *v,
                                               int target_type,
                                               int target_id)
{
    if(v && v->settings &&
       atomic_load_int(&v->settings->transition.active) &&
       v->settings->transition.next_type == target_type &&
       v->settings->transition.next_id == target_id)
    {
        vj_perform_reset_transition(v);
    }
}
...
void vj_event_sample_del(void *ptr, const char format[], va_list ap)
{
    ...
    vj_event_cancel_transition_target(v,
...
void vj_event_sample_clear_all(void *ptr, const char format[], va_list ap)
{
    ...
    if(v->settings && v->settings->transition.next_type == VJ_PLAYBACK_MODE_SAMPLE &&
       v->settings->transition.next_id > 0 &&
       atomic_load_int(&v->settings->transition.active))
    {
        vj_perform_reset_transition(v);
    }
...
void vj_event_chain_entry_set_narg_val(void *ptr,const char format[], va_list ap)
{
    ...
    if(SAMPLE_PLAYING(v)) 
    {
        ...
        if(sample_exists(args[0]))
        {
            ...
            float min = vje_get_param_min_limit(effect, args[2]);
            float max = vje_get_param_max_limit(effect, args[2]);
...
static inline int vj_audio_beat_event_q1000(float v)
{
    if(v < 0.0f)
        v = 0.0f;
    else if(v > 9999.0f)
...
static inline int vj_audio_beat_event_q100(float v)
{
    if(v < 0.0f)
        v = 0.0f;
    else if(v > 9999.0f)
...
static const char *vj_audio_beat_event_auto_mode_name(int mode)
{
    switch(mode)
    {
        case VJ_AUDIO_BEAT_AUTO_OFF:
            return "off";
        case VJ_AUDIO_BEAT_AUTO_PRIMARY:
            return "primary";
        case VJ_AUDIO_BEAT_AUTO_PRIMARY_MOTION:
            return "primary+motion";
        case VJ_AUDIO_BEAT_AUTO_PRIMARY_MOTION_MEMORY:
            return "primary+motion+memory";
        case VJ_AUDIO_BEAT_AUTO_CHAOS:
            return "chaos";
        default:
            return "unknown";
    }
...
void vj_event_audio_beat_fx_entry(void *ptr, const char format[], va_list ap)
{
    ...
    {
        veejay_msg(VEEJAY_MSG_ERROR,
                   "[AUDIO-BEAT] FX entry beat toggle is only available in sample or stream mode");
        return;
    }
...
void vj_event_audio_beat_ui_config(void *ptr, const char format[], va_list ap)
{
    ...
    vj_audio_beat_set_action(&v->settings->audio_beat, args[1]);
    vj_audio_beat_set_pulse_ms(&v->settings->audio_beat, args[6]);
    ...
    {
        veejay_msg(VEEJAY_MSG_ERROR,
                   "[AUDIO-BEAT] Unable to %s beat detector after UI config",
                   enabled ? "enable" : "disable");
        return;
    }
...
void vj_event_audio_beat_action(void *ptr, const char format[], va_list ap)
{
#ifdef HAVE_JACK
    veejay_t *v = (veejay_t *)ptr;
    int args[1] = { VJ_AUDIO_BEAT_ACTION_AUTO_FX };
    ...
    veejay_msg(VEEJAY_MSG_INFO,
            "[AUDIO-BEAT] Action mode set to %s(%d)",
            vj_audio_beat_event_action_name(args[0]),
            args[0]);
#else
    (void)ptr;
    (void)format;
...
void vj_event_audio_beat_pulse(void *ptr, const char format[], va_list ap)
{
    ...
    vj_audio_beat_set_pulse_ms(&v->settings->audio_beat, args[0]);

    veejay_msg(VEEJAY_MSG_INFO,
               "[AUDIO-BEAT] Beat pulse duration set to %d ms",
               vj_audio_beat_event_clampi(args[0], 20, 2000));
#else
    (void)ptr;
    (void)format;
    (void)ap;
...
void vj_event_audio_beat_gate(void *ptr, const char format[], va_list ap)
{
    ...
    vj_audio_beat_set_gate_ms(&v->settings->audio_beat, args[0]);

    veejay_msg(VEEJAY_MSG_INFO,
               "[AUDIO-BEAT] Beat gate duration set to %d ms",
               vj_audio_beat_event_clampi(args[0], 10, 1000));
#else
    (void)ptr;
    (void)format;
    (void)ap;
...
void vj_event_audio_beat_auto_mode(void *ptr, const char format[], va_list ap)
{
#ifdef HAVE_JACK
    veejay_t *v = (veejay_t *)ptr;
    int args[1] = { VJ_AUDIO_BEAT_AUTO_PRIMARY_MOTION };
    ...
    args[0] = vj_audio_beat_event_clampi(args[0], VJ_AUDIO_BEAT_AUTO_OFF, VJ_AUDIO_BEAT_AUTO_CHAOS);
    vj_audio_beat_set_auto_mode(&v->settings->audio_beat, args[0]);
    vj_audio_beat_event_auto_mode_cache = args[0];

    veejay_msg(VEEJAY_MSG_INFO,
               "[AUDIO-BEAT] Auto-fx mode set to %s(%d)",
               vj_audio_beat_event_auto_mode_name(args[0]),
               args[0]);
#else
    (void)ptr;
    (void)format;
...
void vj_event_audio_beat_auto_reset(void *ptr, const char format[], va_list ap)
{
    ...
    vj_audio_beat_auto_reset(&v->settings->audio_beat);

    veejay_msg(VEEJAY_MSG_INFO,
               "[AUDIO-BEAT] Auto-fx mapping reset");
#else
    (void)ptr;
    (void)format;
    (void)ap;
...
void vj_event_audio_beat_state(void *ptr, const char format[], va_list ap)
{
    ...
    n = snprintf(payload,
                 ...
                 snap.hits,
                 snap.beat_age_ms,
                 vj_audio_beat_event_q1000(snap.level),
                 vj_audio_beat_event_q1000(snap.envelope),
                 vj_audio_beat_event_q1000(snap.transient),
                 vj_audio_beat_event_q1000(snap.flux),
                 vj_audio_beat_event_q1000(snap.bass),
                 vj_audio_beat_event_q1000(snap.mid),
                 vj_audio_beat_event_q1000(snap.high),
                 vj_audio_beat_event_q1000(snap.band_balance),
                 vj_audio_beat_event_q1000(snap.beat_pulse),
                 vj_audio_beat_event_q1000(snap.beat_gate),
                 vj_audio_beat_event_q1000(snap.beat_toggle),
                 vj_audio_beat_event_q100(snap.bpm),
                 vj_audio_beat_event_q1000(snap.beat_trail_length),
                 vj_audio_beat_event_q1000(snap.beat_density),
                 atomic_load_int(&s->action_mode),
                 atomic_load_int(&s->freeze_ms),
                 atomic_load_int(&s->cooldown_ms),
                 atomic_load_int(&s->threshold),
                 atomic_load_int(&s->input_channels_request),
...
void vj_event_resume_id(void *ptr, const char format[], va_list ap)
{
    ...
    if(STREAM_PLAYING(v)) {
        ...
        if(sample_exists(sample_id)) {
            veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_SAMPLE, sample_id);
            
            int transition_armed = v->uc && atomic_load_int(&v->settings->transition.active) &&
                                   v->settings->transition.next_type == VJ_PLAYBACK_MODE_SAMPLE &&
                                   v->settings->transition.next_id == sample_id;
...
void vj_event_select_id(void *ptr, const char format[], va_list ap)
{
    ...
    else
    {
        ...
        if(vj_tag_exists(sample_id ))
        {
            veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_TAG ,sample_id);
        }
        else
        {
...
void vj_event_send_track_list(void *ptr, const char format[], va_list ap)
{
    ...
    p += 5; // reserve space for total length
...
static char *_vj_event_gatter_sample_info(veejay_t *v, int id)
{
    ...
    mpeg_timecode(&tc, (end_frame - start_frame + 1),
                  mpeg_framerate_code(ratio), v->current_edit_list->video_fps);
    ...
    int n = snprintf(p, rem, "%08d", body_len);
    p += n; rem -= n;

    p = format_msg_safe(p, rem, description);
    rem = (s_print_buf + (8 + body_len + 1)) - p;

    p = format_msg_safe(p, rem, timecode);
    rem = (s_print_buf + (8 + body_len + 1)) - p;
...
static char * _vj_event_gatter_stream_info(veejay_t *v, int id)
{
    ...
    size_t tlen = strlen(source);
    if (tlen > 999) tlen = 999;

    int body_len = 3 + (int)dlen + 3 + (int)tlen + 2 + 2;
    ...
    p = format_msg_safe(p, rem, source);
    rem = (s_print_buf + (8 + body_len + 1)) - p;

    snprintf(p, rem, "%02d%02d", stream_type, id);
...
}void vj_event_send_working_dir(void *ptr, const char format[], va_list ap)
{
    ...
    else
    {
        ...
        {
            if (list->files[i])
                total_len += 4 + strlen(list->files[i]);
        }

        s_print_buf = get_print_buf(total_len + 9);
        char *p = s_print_buf;
        snprintf(p, 9, "%08zu", total_len);
        p += 8;
...
void vj_event_send_sample_list(void *ptr, const char format[], va_list ap)
{
    ...
    if (n > 1)
    {
        ...
        for (int i = start_from_sample; i <= n; i++)
        {
            if (!sample_exists(i))
                continue;
...
void vj_event_send_sample_stack(void *ptr, const char format[], va_list ap)
{
    ...
    if (sample_id == 0)
        sample_id = v->uc->sample_id;

    int channel, source, fx_id, offset, sample_len;

    if (SAMPLE_PLAYING(v))
    {
        for (int i = 0; i < SAMPLE_MAX_EFFECTS; i++)
        {
            fx_id = sample_get_effect_any(sample_id, i);
            if (fx_id <= 0)
                continue;

            channel = sample_get_chain_channel(sample_id, i);
            source  = sample_get_chain_source(sample_id, i);
            offset  = sample_get_resume(sample_id);

            sample_len = (source == 0) ? sample_video_length(channel) : vj_tag_get_n_frames(channel);

            char line[64];
            snprintf(line, sizeof(line), "%02d%04d%02d%08d%08d",
                     i, channel, source, offset, sample_len);
    ...
    else if (STREAM_PLAYING(v))
    {
        for (int i = 0; i < SAMPLE_MAX_EFFECTS; i++)
        {
            fx_id = vj_tag_get_effect_any(sample_id, i);
            if (fx_id <= 0)
                continue;

            channel = vj_tag_get_chain_channel(sample_id, i);
            source  = vj_tag_get_chain_source(sample_id, i);
            offset  = 0;
    ...
    if (STREAM_PLAYING(v))
    {
        ...
        if (vj_tag_get_type(args[0]) == VJ_TAG_TYPE_GENERATOR)
        {
            ...
            *p++ = ' ';
            ...
            for (int i = 0; i < n_args; i++)
            {
                p = vj_sprintf(p, tagargs[i]);
                *p++ = ' ';
            }

            p = vj_sprintf(p, tagargs[n_args]);
...
void vj_event_send_generator_list(void *ptr, const char format[], va_list ap)
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        char line[128];
        snprintf(line, sizeof(line), "%03zu%s", strlen(name), name);
        p = APPEND_MSG(p, line);
...
void vj_event_send_chain_list(void *ptr, const char format[], va_list ap)
{
    ...
    if (SAMPLE_PLAYING(v) || STREAM_PLAYING(v))
    {
        const int is_sample = SAMPLE_PLAYING(v);
        ...
        const int max_effects = SAMPLE_MAX_EFFECTS;
        const size_t p_size = ((size_t)VIMS_CHAIN_LIST_ENTRY_LENGTH * max_effects) + 1;

        char *print_buf = (char*)vj_malloc(p_size);
        if (!print_buf)
            return;
        ...
        for (int i = 0; i < max_effects; i++)
        {
            ...
            int video_on = is_sample ? sample_get_chain_status(args[0], i)
                                     : vj_tag_get_chain_status(args[0], i);

            int ch_src = is_sample ? sample_get_chain_source(args[0], i)
                                   : vj_tag_get_chain_source(args[0], i);

            int ch_chan = is_sample ? sample_get_chain_channel(args[0], i)
                                    : vj_tag_get_chain_channel(args[0], i);
            ...
            if (is_sample) {
                kf_status = sample_get_kf_status(args[0], i, &kf_type);
                sample_get_subrender(args[0], i, &subrender);
            } else {
                kf_status = vj_tag_get_kf_status(args[0], i, &kf_type);
            ...
            curr += VIMS_CHAIN_LIST_ENTRY_LENGTH;
            count++;
        }

        *curr = '\0';

        const size_t total_payload_len =
            (size_t)count * VIMS_CHAIN_LIST_ENTRY_LENGTH;

        s_print_buf = get_print_buf(total_payload_len + 5);
        if (s_print_buf) {
            snprintf(s_print_buf,
                     total_payload_len + 5,
                     "%04zu%s",
                     total_payload_len,
                     print_buf);
    ...
    {
        s_print_buf = get_print_buf(5);
        if (s_print_buf)
            strcpy(s_print_buf, "0000");
    }
...
void vj_event_send_shm_info(void *ptr, const char format[], va_list ap)
{
    ...
    char *buf = get_print_buf(128 + 4);
    if (!buf)
        return;

    int n = snprintf(buf + 3, 128, "%d %d %d %d",
    ...
    if (args[0] >= 0)
    {
        veejay_msg(VEEJAY_MSG_INFO, "Binding this instance to screen %d of remote", args[0]);
    }
}
...
void vj_event_send_chain_entry(void *ptr, const char format[], va_list ap)
{
    ...
    for(int i = 0; i < num_params; i++) {
        int value = entry->arg[i];

        if(v && v->settings && entry->kf_status && entry->kf) {
            int kf_value = value;
            long long n_frame = atomic_load_long_long(&v->settings->current_frame_num);

            if(get_keyframe_value(entry->kf, n_frame, i, &kf_value))
                value = kf_value;
        }
...
void vj_event_send_effect_list(void *ptr, const char format[], va_list ap)
{
    ...
    long total_alloc_len = 6; 
    for (int i = 0; i < n_fx; i++) {
        int s_len = vje_get_summarylen(i);
        if (s_len > 0) {
            total_alloc_len += (4 + s_len);
        }
    }

    char *priv_msg = (char*)vj_malloc(total_alloc_len + 1);
    if (!priv_msg) return;

    char *ptr_curr = priv_msg;
    size_t rem = total_alloc_len + 1;

    int header_n = snprintf(ptr_curr, rem, "%06ld", total_alloc_len - 6);
    ptr_curr += header_n;
    rem -= header_n;
    ...
    for (int i = 0; i < n_fx; i++) {
        int s_len = vje_get_summarylen(i);
        if (s_len > 0) {
            // write the 4-digit individual length prefix
            int pref_n = snprintf(ptr_curr, rem, "%04d", s_len);
            ptr_curr += pref_n;
            rem -= pref_n;

            // write the summary directly into the main buffer
            int written = vje_get_summary(i, ptr_curr, rem);
            if (written > 0) {
                ptr_curr += written;
                rem -= written;
            }
...
void vj_event_input_route_select(void *ptr, const char format[], va_list ap)
{
    ...
    veejay_change_playback_mode_transition(v, VJ_PLAYBACK_MODE_TAG, id);
    int transition_armed = v->uc && atomic_load_int(&v->settings->transition.active) &&
                           v->settings->transition.next_type == VJ_PLAYBACK_MODE_TAG &&
                           v->settings->transition.next_id == id;
...
void vj_event_input_route_remove(void *ptr, const char format[], va_list ap)
{
    ...
    vj_event_cancel_transition_target(v, VJ_PLAYBACK_MODE_TAG, id);
...
static void vj_event_sample_next1( veejay_t *v )
{
    if( v->seq->active ){
        ...
        int playback_mode = (next_type == 0 || next_type == VJ_PLAYBACK_MODE_SAMPLE) ? VJ_PLAYBACK_MODE_SAMPLE : VJ_PLAYBACK_MODE_TAG;

        veejay_reset_sample_positions(v, -1);
        veejay_change_playback_mode(v, playback_mode, next_id);
        vj_event_release_beat_transport_for_source_switch(v);
        return;
    }
    if( SAMPLE_PLAYING(v)) {
...
static int vj_sequence_all_bank_mask(void)
{
...
static int vj_sequence_selected_bank_mask(sequencer_t *s)
{
    ...
    mask = s->selected_bank_mask & vj_sequence_all_bank_mask();
    if(mask == 0)
        mask = (1 << bank);

    mask |= (1 << bank);
    s->selected_bank_mask = mask;

    return mask;
}

static int vj_sequence_selected_slot_count(sequencer_t *s)
{
    int mask = vj_sequence_selected_bank_mask(s);
    int count = 0;
    ...
    for(int bank = 0; bank < VJ_SEQUENCE_BANKS; bank++) {
        if(!(mask & (1 << bank)))
            continue;
        count += vj_sequence_count_slots(s->banks[bank].samples);
    }
...
static int vj_sequence_selected_duration(veejay_t *v)
{
    ...
    for(int bank = 0; bank < VJ_SEQUENCE_BANKS; bank++) {
        ...
        for(int i = 0; i < MAX_SEQUENCES; i++) {
            int id = s->banks[bank].samples[i].sample_id;
            int type = s->banks[bank].samples[i].type;
            ...
            if(type == 0) {
                if(sample_exists(id))
                    frames += sample_get_frame_length(id);
            }
            else {
                if(vj_tag_exists(id))
                    frames += vj_tag_get_n_frames(id);
            }
        }
    }
...
static void vj_sequence_store_active_bank(sequencer_t *s)
{
    ...
    s->banks[bank].size = vj_sequence_count_slots(s->samples);
    s->banks[bank].current = s->current;
    s->size = s->banks[bank].size;
    vj_sequence_selected_bank_mask(s);
}

static void vj_sequence_touch_active_bank(sequencer_t *s)
{
    int bank = s->active_bank;
...
static void vj_sequence_load_bank(sequencer_t *s, int bank)
{
    ...
    s->size = vj_sequence_count_slots(s->samples);
...
static void vj_event_release_beat_transport_for_source_switch(veejay_t *v)
{
    ...
    vj_audio_beat_release_transport(v, &v->settings->audio_beat);
#else
    (void)v;
#endif
}
...
static void vj_event_clear_sequence_boundary_state(veejay_t *v)
{
    ...
    atomic_store_int(&v->settings->sequence_boundary, 0);
    v->settings->sequence_random_id = 0;
    v->settings->sequence_random_ticks_left = 0;
}
...
{
    veejay_t *v = (veejay_t*)ptr;
    const int bank_payload = 22 + (MAX_SEQUENCES * 6);
    const int payload_len = 18 + (VJ_SEQUENCE_BANKS * bank_payload);
    char *s_print_buf = get_print_buf(32 + payload_len);
    char tmp[64];

    vj_sequence_store_active_bank(v->seq);

    snprintf(s_print_buf, 32 + payload_len, "%06d%04d%04d%010u",
             payload_len, v->seq->active_bank, v->seq->active, v->seq->revision);

    for(int bank = 0; bank < VJ_SEQUENCE_BANKS; bank++) {
        sequence_bank_t *b = &(v->seq->banks[bank]);

        snprintf(tmp, sizeof(tmp), "%04d%04d%04d%010u",
                 bank, b->current, b->size, b->revision);
        veejay_strncat(s_print_buf, tmp, 22);

        for(int i = 0; i < MAX_SEQUENCES; i++) {
            snprintf(tmp, sizeof(tmp), "%04d%02d", b->samples[i].sample_id, b->samples[i].type);
            veejay_strncat(s_print_buf, tmp, 6);
        }
    }
...
void vj_event_sequence_select(void *ptr, const char format[], va_list ap)
{
    ...
    if(requested_mask > 0) {
        int mask = requested_mask & vj_sequence_all_bank_mask();
        if(mask == 0)
            mask = (1 << bank);
        mask |= (1 << bank);
        if(mask != old_mask) {
            v->seq->selected_bank_mask = mask;
            mask_changed = 1;
        }
    }
    else {
    ...
    veejay_msg(VEEJAY_MSG_INFO,
               "Selected sequence bank %d (%d slots, mask 0x%x, previous %d)",
               bank,
               v->seq->size,
               v->seq->selected_bank_mask,
               old_bank);
    ...
    if(v->seq->active) {
        int next_type = 0;
        int slot = v->seq->current;
        int next_id = vj_perform_get_next_sequence_id(v, &next_type, v->seq->current, &slot);
        ...
        if(next_id <= 0) {
            ...
            veejay_msg(VEEJAY_MSG_ERROR, "Selected sequence banks are empty; sequencer disabled");
            return;
        }

        v->seq->current = slot;
        v->seq->banks[v->seq->active_bank].current = slot;
...
void vj_event_sequence_copy(void *ptr, const char format[], va_list ap)
{
    ...
    if(!vj_sequence_bank_valid(src) || !vj_sequence_bank_valid(dst)) {
        veejay_msg(VEEJAY_MSG_ERROR, "Invalid sequence bank copy %d -> %d", src, dst);
        return;
    }

    if(src == dst) {
        veejay_msg(VEEJAY_MSG_INFO, "Sequence bank copy %d -> %d ignored", src, dst);
        return;
    }

    vj_sequence_store_active_bank(v->seq);

    veejay_memcpy(v->seq->banks[dst].samples, v->seq->banks[src].samples, sizeof(seq_sample_t) * MAX_SEQUENCES);
    v->seq->banks[dst].current = v->seq->banks[src].current;
    v->seq->banks[dst].size = vj_sequence_count_slots(v->seq->banks[dst].samples);
    v->seq->banks[dst].revision = vj_sequence_next_revision(v->seq->banks[dst].revision);
    v->seq->revision = vj_sequence_next_revision(v->seq->revision);
...
void vj_event_sequence_clear_all(void *ptr, const char format[], va_list ap)
{
    ...
    for(int bank = 0; bank < VJ_SEQUENCE_BANKS; bank++) {
        veejay_memset(v->seq->banks[bank].samples, 0, sizeof(seq_sample_t) * MAX_SEQUENCES);
        v->seq->banks[bank].size = 0;
        v->seq->banks[bank].current = 0;
        v->seq->banks[bank].revision = vj_sequence_next_revision(v->seq->banks[bank].revision);
    }

    veejay_memset(v->seq->samples, 0, sizeof(seq_sample_t) * MAX_SEQUENCES);
    v->seq->active = 0;
    v->seq->current = 0;
    v->seq->size = 0;
    v->seq->selected_bank_mask = (1 << v->seq->active_bank);
...
void vj_event_sample_sequencer_active(void *ptr, const char format[], va_list ap)
{
    ...
    {
        veejay_msg(VEEJAY_MSG_ERROR, "Selected sequence banks are empty. Please add samples first");
        return;
    }
    ...
    if(vj_sequence_bank_valid(v->seq->active_bank))
        v->seq->banks[v->seq->active_bank].current = v->seq->current;
