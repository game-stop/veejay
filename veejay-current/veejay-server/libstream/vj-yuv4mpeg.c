...
static int vj_yuv_stream_start_read1(vj_yuv * yuv4mpeg, int fd, int width, int height)
{
	...
	veejay_msg(VEEJAY_MSG_DEBUG, "Frame: %dx%d, number of planes: %d, chroma: %d",w,h,plane_count, y4m_chroma_keyword( yuv4mpeg->chroma ));
	...
	yuv4mpeg->src = yuv_yuv_template( yuv4mpeg->buf[0], yuv4mpeg->buf[1], yuv4mpeg->buf[2],w,h, src_fmt );
	yuv4mpeg->dst = yuv_yuv_template( NULL,NULL,NULL, yuv4mpeg->width,yuv4mpeg->height, yuv4mpeg->format );

	sws_template sws_tem;
	veejay_memset(&sws_tem, 0,sizeof(sws_template));
	sws_tem.flags = yuv_which_scaler();
	yuv4mpeg->scaler = yuv_init_swscaler( yuv4mpeg->src,yuv4mpeg->dst, &sws_tem, yuv_sws_get_cpu_flags());
	if(!yuv4mpeg->scaler) {
		veejay_msg(VEEJAY_MSG_ERROR, "yuv4mpeg: unable to initialize scaler context" );
		return -1;
	}
...
int vj_yuv_stream_write_header(vj_yuv * yuv4mpeg, VJFrame *frame, int out_chroma)
{
    ...
    y4m_ratio_t dar2 = y4m_guess_sar(yuv4mpeg->width,yuv4mpeg->height,yuv4mpeg->dar);
    ...
    yuv4mpeg->dst = yuv_yuv_template( yuv4mpeg->buf[0],yuv4mpeg->buf[1],yuv4mpeg->buf[2], frame->width,frame->height, get_pixfmt_from_chroma(out_chroma));
...
{	
    if( vj_yuv_stream_write_header(yuv4mpeg, frame, get_chroma_from_pixfmt(frame->format) ) == 0 )
			return 1;
...
{
	int i = y4m_read_frame(yuv4mpeg->fd, &(yuv4mpeg->streaminfo),&(yuv4mpeg->frameinfo), yuv4mpeg->buf);
 	if( i != Y4M_OK ) {
		if( i == Y4M_ERR_EOF ) {
	...
	yuv4mpeg->dst->data[0] = dst[0];
	yuv4mpeg->dst->data[1] = dst[1];
	yuv4mpeg->dst->data[2] = dst[2];

	yuv_convert_and_scale( yuv4mpeg->scaler, yuv4mpeg->src, yuv4mpeg->dst );
...
int vj_yuv_put_frame(vj_yuv * vjyuv, uint8_t **src)
{
	...
	vjyuv->src->data[0] = src[0];
	vjyuv->src->data[1] = src[1];
	vjyuv->src->data[2] = src[2];

	yuv_convert_and_scale( vjyuv->scaler, vjyuv->src, vjyuv->dst );
