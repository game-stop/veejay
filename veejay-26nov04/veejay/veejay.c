...
/* Get options */
    ...
    while ((n =
	    getopt_long(argc, argv,
			"o:G:O:a:H:V:s:c:t:l:C:p:m:x:y:nFPXY:ugrvdibIjf:",
			long_options, &option_index)) != EOF)
#else
    while ((n =
	    getopt(argc, argv,
		   "o:G:s:O:a:c:t:l:t:C:x:y:m:p:nFPXY:vudgibrIjf:")) != EOF)
#endif
    {
	switch (n) {
#ifdef HAVE_GETOPT_LONG
...
static void print_license()
{
	veejay_msg(VEEJAY_MSG_INFO,
	    "Veejay -<|Jetta|>- %s Copyright (C) Niels Elburg and others",VERSION);
	veejay_msg(VEEJAY_MSG_INFO,
