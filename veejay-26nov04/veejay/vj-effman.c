...
// still a mess, vj_clip_instr should be renewed, together with FrameInfo

void vj_effman_apply_image_effect(
	VJFrame **frames,
	VJFrameInfo *frameinfo,
	vj_clip_instr *todo_info,
	int *arg,
	int e,
	int entry)
...
void vj_effman_apply_video_effect( VJFrame **frames, VJFrameInfo *frameinfo ,vj_clip_instr *todo_info, int *arg, int e,int entry) {
	...
	//  yuvchannelmix_apply(frames[0],frames[1],frameinfo->width,frameinfo->height,arg[0],arg[1],arg[2],
	...
	diff_apply( vj_effects[entry]->vjed, frames[0], frames[1],
	...
	if(arg[2] == 1) { /* auto increment as option in effect*/
	   todo_info->tmp[0] ++; /* val / max frames */
	   if(todo_info->tmp[0] > (frameinfo->height/16)) todo_info->tmp[0] = 1;
	} else { /* arg1 = off , copy arg*/
	     todo_info->tmp[0] = arg[1];
	}

	slidingdoor_apply(frames[0], frames[1], frameinfo->width,
			  frameinfo->height, arg[0], todo_info->tmp[0]);
	break;
			  ...
			  VJFrame **frames, VJFrameInfo *frameinfo, int e, int c, int n_frame)
{
    ...
    int num_args = vj_effect_get_num_params(e);
    int *args = NULL;


    if (num_args == VJ_EFFECT_ERROR)
	return -1;
    if (!frames[0] || !frames)
	return -1;
    ...
    if(num_args > 0)
	{
    	args = (int*) malloc(sizeof(int) *( num_args + 1));
		memset( args, 0, (num_args+1));
	}

    if (todo_info->is_tag)
	{
		// get effect parameters from stream
		if (!vj_tag_get_all_effect_args(todo_info->clip_id, c, args, num_args))
		{
		    if(args) free(args);
		    return -1;
		}
	...
	else
	{
		...
		if (!clip_get_all_effect_arg(todo_info->clip_id, c, args, num_args,n_frame))
		{
			if(args) free(args);
		    return -1;
		}
    ...
    if(e>200)
	{
		vj_effman_apply_video_effect( frames, frameinfo,todo_info, args,e,entry);
	}
    else
	{
		vj_effman_apply_image_effect( frames, frameinfo, todo_info, args,e,entry);
	}
