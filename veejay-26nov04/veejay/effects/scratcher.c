...
void store_frame(VJFrame *src, int w, int h, int n, int no_reverse)
{
	...
	memcpy(frame[0] + (w * h * nframe), Y, (w * h));
	memcpy(frame[1] + (uv_len * nframe), Cb, uv_len);
	memcpy(frame[2] + (uv_len * nframe), Cr, uv_len);
    } else {
	//printf("copy frame to buffer at pos %d", (w*h*nframe)); 
	memcpy(Y, frame[0] + (w * h * nframe), (w * h));
	memcpy(Cb, frame[1] + (uv_len * nframe), uv_len);
	memcpy(Cr, frame[2] + (uv_len * nframe), uv_len);
    }
		     ...
		     int no_reverse)
{
    ...
    store_frame(frame, width, height, n, no_reverse);
