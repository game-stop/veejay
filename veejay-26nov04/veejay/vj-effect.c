...
int vj_effect_activate(int effect_id)
{
	...
	if( vj_effect_ready[seq] == 0 )
	{
		...
		if(simple_effect_index[index].mem_init( max_width, max_height )!=1)
		{
...
//    vj_effects[33] = channelmix_init(width, int height);
	...
	//	veejay_msg(VEEJAY_MSG_DEBUG, "Registered %s (%d)", vj_effects[i]->description,
	//			vj_effect_get_real_id(i));
	}
...
{
	veejay_msg(VEEJAY_MSG_DEBUG, "Try entry %d ", entry);
	...
	veejay_msg(VEEJAY_MSG_DEBUG, "REAL EFFECT ID = %d",
	...
	{
		bzero(tmp,20);
		veejay_msg(VEEJAY_MSG_DEBUG, "Effect [%s] has %d params",
			vj_effects[entry]->description , vj_effects[entry]->num_params);
		sprintf(tmp,
			"%06d%06d%06d",
