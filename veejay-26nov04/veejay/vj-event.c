...
void	vj_event_clip_add_wave( void *ptr,	const char format[], va_list ap)
{
	...
	if(CLIP_PLAYING(v))
	{
		...
		if( !clip_start_external( v->uc->clip_id, file_name, v->edit_list ) )
		{
...
//	diff_set_background( ptr[0], v->edit_list->video_width * v->edit_list->video_height );
	v->uc->take_bg = 1;
...
void vj_event_init_gui_screen(void *ptr, const char format[], va_list ap)
{
	...
	else
	{
		...
		if(args[0]==1)
		{
			...
			sprintf(msg, "%ld %ld", v->edit_list->video_width, v->edit_list->video_height); 
