...
livido_process_f		process_instance( livido_port_t *my_instance, double timecode )
{
	...
	const int hei = w[1];
        ...
        double contrast  = lvd_extract_param_number(
                                my_instance,
                                "in_parameters",
                                0 );

	int c = (100.0 *contrast );
	
	
	contrast_y_apply(O[0], A[0], len, c);

	contrast_uv_apply(
			O[1], O[2], A[1], uv_len, A[2], c);        	
...

{
	...
	port = filter;
		...
		livido_set_string_value( port, "description", "Software Contrast");
