...
#define XMLTAG_FREEZEMODE  "freezemode"
#define XMLTAG_FREEZENFRAME "freeze"
#define XMLTAG_FREEZEPFRAME "unfreeze" 
...
extern void vj_effect_new_key_frame(vj_effect_key_frame *,int, int, int,int);
extern int vj_effect_apply_key_frame(vj_effect_key_frame *, int );
extern int vj_effect_clear_key_frame(vj_effect_key_frame *);
extern int vj_effect_enable_key_frame(vj_effect_key_frame *);
extern int vj_effect_disable_key_frame(vj_effect_key_frame *);
...
static void	vj_close_wav_file( vj_wav_file *wav )
{
	if(wav)
	{
		if(wav->fd) close(wav->fd);
		free(wav);
	}
}

static vj_wav_file *vj_open_wav_file(char *filename, float video_fps)
{
	vj_wav_file *wav = (vj_wav_file*) malloc(sizeof(vj_wav_file));
	int n;
	long data[64];
	int header_len;
	if(!wav) return NULL;

	wav->fd = open( filename, O_RDONLY );
	if(wav->fd < 0)
	{
		veejay_msg(VEEJAY_MSG_ERROR, "while opening wav file: %s", strerror(errno));
		if(wav) free(wav);
		return NULL;
	}

	n = read( wav->fd, (char*) data, 20 );
	if(data[0] != FOURCC_RIFF || data[2] != FOURCC_WAVE || data[3] != FOURCC_FMT || data[4] > sizeof(data) )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "in WAV header");
		if(wav) free(wav);
		return NULL;
	}
	header_len = data[4];
	n = read( wav->fd, (char*) data, header_len );
	if ( n != header_len )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "reading WAV header");
		if(wav) free(wav);
		return NULL;
	}
	if ( (data[0] & 0xffff ) != 1 )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "WAV file is not in PCM format");
		if(wav) free(wav);
		return NULL;
	}

	wav->channels 	= (data[0] >> 16) & 0xffff;
	wav->rate 	= data[1];
	wav->bits	= (data[3] >> 16) & 0xffff;
	wav->bps	= (wav->channels * wav->bits+7)/8;

	if(wav->bps==0) wav->bps = 1; 

	n = read( wav->fd, (char*) data, 8);
	if(n != 8 )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "read WAV header: %s", strerror(errno));
		if(wav) free(wav);
		return NULL;
	}
	...
	{
		veejay_msg(VEEJAY_MSG_ERROR, "error in WAV header");
	}

	wav->num_samples = data[1] / wav->bps;
	wav->seconds = (float)wav->num_samples /(float) wav->rate;

	veejay_msg(VEEJAY_MSG_DEBUG, "PCM WAVE File: %s", filename);
	veejay_msg(VEEJAY_MSG_DEBUG, "  samples : %8ld", wav->num_samples);
	veejay_msg(VEEJAY_MSG_DEBUG, "  channels: %8d",  wav->channels);
	veejay_msg(VEEJAY_MSG_DEBUG, "  bits    : %8d",  wav->bits);
	veejay_msg(VEEJAY_MSG_DEBUG, "  rate    : %8d",  wav->rate);
	veejay_msg(VEEJAY_MSG_DEBUG, "  length  : %15.3f seconds", wav->seconds);

	wav->buf_size 	= (wav->rate / video_fps + 0.5) * wav->bps;
...
clip_info *clip_skeleton_new(long startFrame, long endFrame)
{
    ...
    si->freeze_nframes = 0;
    si->freeze_pframes = 0;
    si->freeze_val_nframes = 0;
    si->freeze_val_pframes = 0;
    si->fader_active = 0;
    si->fader_val = 0;
    si->fader_inc = 0;
   ...
   // si->encoder_buf = (char*) malloc(sizeof(char) * 10 * 65535 + 16);
	...
	for (j = 0; j < CLIP_MAX_PARAMETERS; j++) {
	    ...
	    si->effect_chain[i]->ekf[j] = (vj_effect_key_frame*) vj_malloc(sizeof(vj_effect_key_frame));
	    if(!si->effect_chain[i]->ekf[j])
		{
			veejay_msg(VEEJAY_MSG_ERROR, "Cannot allocate memory for effect key frames");
			return NULL;
		}
	    vj_effect_clear_key_frame(si->effect_chain[i]->ekf[j]);
	}
...
{
	clip_info *c = clip_get(clip_id);
	if(!c) return 0;
	if(c->external_wave==1)
		return 1;
	return 0;
}
...
int	clip_close_external(int clip_id)
{
	clip_info * c= clip_get(clip_id);
	if(!c) return 0;
	if(!c->external_wave) return 0;
	vj_close_wav_file(c->wav);
	c->external_wave = 0;
	clip_update(c, clip_id);
	return 1;	
}
...
int clip_read_external(int clip_id, uint8_t *dst)
{
	...
	int n = read(c->wav->fd, dst, c->wav->buf_size);
	if(n > 0)
	{
		res = ( n / c->wav->bps);
	}
	...
	if( error )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "reading wav file %s", strerror(errno));
		vj_close_wav_file(c->wav);
		c->external_wave =0;
		clip_update( c, clip_id);
		return 0;
...
int clip_start_external(int clip_id, char *filename, EditList *el)
{
	...
	if(c->wav)
	{ // already initialized
		veejay_msg(VEEJAY_MSG_ERROR,"Closing existing external wav file..");
		vj_close_wav_file( c->wav );
		c->wav = NULL;
	}
	c->wav = vj_open_wav_file(filename, (float)el->video_fps);
	// verify length of wav file
	if(c->wav==NULL)
	{
		veejay_msg(VEEJAY_MSG_ERROR, "opening external wave file %s",filename);
		return 0;
	}	

	long num_frames = c->last_frame[c->active_render_entry] - c->first_frame[c->active_render_entry]; 
	float spvf = 1.0 / el->video_fps;
	float seconds = num_frames * spvf;

	if( c->wav->seconds < seconds )
	{
		veejay_msg(VEEJAY_MSG_ERROR, "external PCM WAV file is too short (%15.3f < %15.3f )",
			c->wav->seconds,seconds);
		vj_close_wav_file(c->wav);
		c->wav= NULL;
	} 
	else
	{
		if(el->has_audio)
		{
		if(c->wav->rate != el->audio_rate )
		{
			veejay_msg(VEEJAY_MSG_ERROR, "WAV file has a rate of %d, while current rate is %d",c->wav->rate, el->audio_rate);
			vj_close_wav_file(c->wav);
			return 0;
		}
		if(c->wav->channels != el->audio_chans)
		{
			veejay_msg(VEEJAY_MSG_ERROR, "WAV file has %d channel(s), while current number of channels is %d", c->wav->channels,el->audio_chans);
			vj_close_wav_file(c->wav);
			return 0;
		}
		if(c->wav->bps != el->audio_bps)
		{
			veejay_msg(VEEJAY_MSG_ERROR, "WAV file has %d bytes per sample, whille current bytes per sample is %d",c->wav->bps, el->audio_bps);
			vj_close_wav_file(c->wav);
			return 0;
		}
		}
		veejay_msg(VEEJAY_MSG_INFO, "Clip recorder will use %15.3f seconds of WAV file %s",
			seconds, filename);
		c->external_wave = 1;
		return ( clip_update(c, clip_id));
	}
	return 0;
}
...
/* freeze_nframe, freeze_pframes, freeze_mode */
int clip_set_freeze_mode(int s1, int mode, int pframes, int nframes) {
   clip_info *si = clip_get(s1);
   if(!si) return -1;
   if(mode >= 0 && nframes >= 0 && pframes >= 0) {
	si->freeze_mode = mode;
	si->freeze_nframes = nframes;
	si->freeze_pframes = pframes;
	si->freeze_val_nframes = nframes;
	si->freeze_val_pframes = pframes;
	return ( clip_update(si,s1));
	}
   return -1;
}
...
int clip_reset_freeze(int s1) {
  clip_info *si = clip_get(s1);
  if(!si) return -1;
  si->freeze_nframes = si->freeze_val_nframes;
  si->freeze_pframes = si->freeze_val_pframes;
  return ( clip_update(si,s1)) ;
}
...
int clip_get_freeze_mode(int s1) {
   clip_info *si = clip_get(s1);
   if(!si) return -1;
   return ( si->freeze_mode );
}

int clip_get_freeze_nframes(int s1 ) {
   clip_info *si = clip_get(s1);
   if(!si) return -1;
   return ( si->freeze_nframes);
}

int clip_get_freeze_pframes(int s1) {
   clip_info *si = clip_get(s1);
   if(!si) return -1;
   return ( si->freeze_pframes );
}
...
int clip_get_all_effect_arg(int s1, int position, int *args, int arg_len, int n_frame)
{
	...
	return -1;
    for (i = 0; i < arg_len; i++) {
	vj_effect_key_frame *ekf = clip->effect_chain[position]->ekf[i];
	int start = clip->first_frame[clip->active_render_entry] + ekf->clip_offset;
	if(ekf->enabled && n_frame != -1 && (n_frame > start))
	{
		args[i] = vj_effect_apply_key_frame(ekf, (abs(start - n_frame)));
	}
	else
	{
	...
	if (vj_effect_get_extra_frame(effect_nr) == 1) {
		  ...
		  if(clip_exists(s2))
		{
			...
			veejay_msg(VEEJAY_MSG_DEBUG,"Effect %s on entry %d overlaying with clip %d",
				vj_effect_get_description(clip->effect_chain[c]->effect_id),c,s2);
				}
		  else
		{	
		      s2 = s1 + 1;
...
int clip_set_effect_key_frame(int s1, int position, int arg_num, int end_val, int length)
{
	...
	int max_length;
	clip_info *si = clip_get(s1);
	vj_effect_key_frame *ekf;
	if(!si) return -1;
	if(position < 0 || position >= CLIP_MAX_EFFECTS) return -1;
	
	ekf = si->effect_chain[position]->ekf[arg_num];

	start_val = si->effect_chain[position]->arg[arg_num];

	max_length = (si->last_frame[si->active_render_entry] + si->offset) -  si->first_frame[si->active_render_entry];
	
	if(max_length < length)
	{
		 veejay_msg(VEEJAY_MSG_WARNING, "Overriding length %d with clip's length %d",
			length,max_length);
		 length = max_length;
	}
	vj_effect_new_key_frame( ekf, start_val, end_val, length, si->offset );
...
int clip_enable_effect_key_frame(int s1, int position, int arg_num)
{
	...
	ekf = si->effect_chain[position]->ekf[arg_num];
	return ( vj_effect_enable_key_frame(ekf) );
...
int clip_disable_effect_key_frame(int s1, int position, int arg_num)
{
	...
	ekf = si->effect_chain[position]->ekf[arg_num];
	return ( vj_effect_disable_key_frame(ekf) );
...
int clip_clear_effect_key_frame(int s1, int position, int arg_num)
{
	...
	/* disable effect key frame */
	ekf->enabled = 0;
	/* restore old effect value */
	si->effect_chain[position]->arg[arg_num] = vj_effect_apply_key_frame(ekf,0);
	/* clear the key frame */
	if( vj_effect_clear_key_frame(ekf) == 1 )
	{
		/* update clip on success*/
		return (clip_update(si,s1));
	}
	return -1;
}
...
int clip_apply_effect_key_frame(int s1, int position, int arg_num, int n_frame)
{
	...
	clip = clip_get(s1);
    	...
    	if (position >= CLIP_MAX_EFFECTS)
		return -1;
    	if (arg_num < 0 || arg_num > CLIP_MAX_PARAMETERS)
		return -1;

	n_elapsed = n_frame - clip->first_frame[clip->active_render_entry];
	
	ekf = clip->effect_chain[position]->ekf[arg_num];

	if(ekf->enabled == 1)
	{
    		clip->effect_chain[position]->arg[arg_num] = vj_effect_apply_key_frame(ekf, n_elapsed);
		return (clip_update(clip,s1));
	}
...
/* 11 */    clip->max_loops,
	    ...
	    clip->audio_volume,
	    clip->freeze_mode,
	    clip->freeze_val_nframes,
/*20 */	    clip->freeze_val_pframes,
	    clip->encoder_active,
...
void ParseClip(xmlDocPtr doc, xmlNodePtr cur, clip_info * skel)
{
	...
	if (!xmlStrcmp(cur->name,(const xmlChar *) XMLTAG_FREEZEMODE)) {
		xmlTemp = xmlNodeListGetString(doc,cur->xmlChildrenNode,1);
		chTemp = UTF8toLAT1(xmlTemp);
		if(chTemp) {
		 skel->freeze_mode = atoi(chTemp);
		 free(chTemp);
		}
	        if(xmlTemp) xmlFree(xmlTemp);
	}
	if (!xmlStrcmp(cur->name,(const xmlChar *) XMLTAG_FREEZENFRAME)) {
		xmlTemp = xmlNodeListGetString(doc,cur->xmlChildrenNode,1);
		chTemp = UTF8toLAT1(xmlTemp);
		if(chTemp) {
		 skel->freeze_nframes = atoi(chTemp);
		 free(chTemp);
		}	
		if(xmlTemp) xmlFree(xmlTemp);
	}
	if (!xmlStrcmp(cur->name,(const xmlChar *) XMLTAG_FREEZEPFRAME)) {
		xmlTemp = xmlNodeListGetString(doc,cur->xmlChildrenNode,1);
		chTemp = UTF8toLAT1(xmlTemp);
		if(chTemp) {
		 skel->freeze_pframes = atoi(chTemp);
		 free(chTemp);
		}	
	        if(xmlTemp) xmlFree(xmlTemp);
	}
...
void CreateClip(xmlNodePtr node, clip_info * clip)
{
	...
	xmlNewChild(node,NULL,(const xmlChar *) XMLTAG_FADER_DIRECTION,
		(const xmlChar *) buffer);
	sprintf(buffer,"%d",clip->freeze_mode);
	xmlNewChild(node,NULL,(const xmlChar *) XMLTAG_FREEZEMODE,
		(const xmlChar *) buffer);
	sprintf(buffer,"%d",clip->freeze_nframes);
	xmlNewChild(node,NULL,(const xmlChar *) XMLTAG_FREEZENFRAME,
		(const xmlChar *)buffer );

	sprintf(buffer,"%d",clip->freeze_pframes);
	xmlNewChild(node,NULL,(const xmlChar *) XMLTAG_FREEZEPFRAME,
		(const xmlChar *)buffer);
