...
#include "sampleadm.h"		/* for definitions and chain information */
...
typedef struct {
    ...
    vj_raw *raw[VJ_TAG_MAX_STREAM_IN];
    int width;
    int height;
    int depth;
} vj_tag_data;
...
int vj_tag_init(int w, int h);

int vj_tag_get_last_tag();

/* create a new tag, type is yuv4mpeg or v4l or vloopback
...
int _vj_tag_new_v4l(vj_tag *t, int a , int b , int c, int d, int e, int f, int g);
