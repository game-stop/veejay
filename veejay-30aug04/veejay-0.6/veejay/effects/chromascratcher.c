...
#include "common.h"
#include "vj-common.h"
//extern void *(* ay_memcpy)(void *to, const void *from, size_t len);
...
vj_effect *chromascratcher_init(int w, int h)
{
    ...
    ve->limits[1][1] = 255;
    ve->limits[0][2] = 0;
    ve->limits[1][2] = MAX_SCRATCH_FRAMES - 1;
    ve->limits[0][3] = 0;
    ve->limits[1][3] = 1;
...
int	chromascratcher_malloc(int w, int h)
{
    ...
    memset( cframe[0], 16, w * h * MAX_SCRATCH_FRAMES);
    cframe[1] =
	(uint8_t *) vj_malloc(w * h * sizeof(uint8_t) * MAX_SCRATCH_FRAMES );
    if(!cframe[1]) return 0;			   
    memset( cframe[1], 128, w * h * MAX_SCRATCH_FRAMES);
    cframe[2] =
	(uint8_t *) vj_malloc(w * h * sizeof(uint8_t) * MAX_SCRATCH_FRAMES );
    if(!cframe[2]) return 0;			   
    memset( cframe[2], 128, w * h * MAX_SCRATCH_FRAMES);
...
void chromascratcher_apply(uint8_t * yuv1[3],
			   ...
			   int n, int no_reverse)
{
    ...
    //printf("mode = %d, frame in buffer = %d, limit = %d\n",mode,cnframe,n);
    if (cnframe == 0) {
	veejay_memcpy(cframe[0] + (len * cnframe), yuv1[0], len);
	veejay_memcpy(cframe[1] + (len * cnframe), yuv1[1], len);
	veejay_memcpy(cframe[2] + (len * cnframe), yuv1[2], len);
    }
    if(mode>3) {
	mode-=3;
       chromamagick_apply( yuv1,cframe,width, height,mode,opacity);
    }
    else {
	...
	for (i = 0; i < len; i++) {
	    if (cframe[0][offset + i] < yuv1[0][i]) {
		yuv1[0][i] = cframe[0][offset + i];
		yuv1[1][i] = cframe[1][offset + i];
		yuv1[2][i] = cframe[2][offset + i];
	    }
	}
	break;
    case 1:
    ...
    case 2:
	for (i = 0; i < len; i++) {
	    if ((cframe[0][offset + i] * op_a) < (yuv1[0][i] * op_b)) {
		yuv1[0][i] = cframe[0][offset + i];
		yuv1[1][i] = cframe[1][offset + i];
		yuv1[2][i] = cframe[2][offset + i];
	    }
...
/*    if (cnframe > 0 && no_reverse == 1) {
	memset(cframe[0] + (len * (cnframe - 1)), 16, len);
	memset(cframe[1] + (len * (cnframe - 1)), 128, len);
	memset(cframe[2] + (len * (cnframe - 1)), 128, len);
 } */  

}
    chromastore_frame(yuv1, width, height, n, no_reverse);
}
