...
void noiseadd_apply(uint8_t *src[3], int width, int height, int type, int coeef) {

	switch(type) {
	case 0:
	noiseblur1x3_maskapply(src,width,height,coeef);	break;
	case 1:
	noiseblur3x3_maskapply(src,width,height,coeef);	break;
	case 2:
	noiseneg3x3_maskapply(src,width,height,coeef);	 break;
	}
}
