...
vj_effect *diff_init(int width, int height)
{
    ...
    // calculate pixel distances and put them into this table:
    for(i=0; i < 256; i ++)
    {
...
int diff_malloc(vj_effect_data *d, int width, int height)
{
	...
	veejay_msg(VEEJAY_MSG_INFO, "diff_malloc: no vj_effect_data");
	return 0;
}
...
void diff_apply(vj_effect_data *ed, uint8_t * yuv1[3],
		...
		int K_level, int noise_level,int noise_level2, int mode)
{
	...
	for(i = 0 ; i < len ; i ++ )
	{
		d = sqrt_table[ (static_bg[0][i]) ][ (yuv1[0][i]) ];
		if(d > level1)
		{
			dst[i] = 0xff;
		}
		...
		d = sqrt_table[ (static_bg[0][i])][ (yuv2[0][i]) ];
		if(d > level2)
		{
			dst[i] = 0xf0;
		}
	...
	if(mode == 0)
	{
		...
		for( i = 0; i < len ; i++)
		{
			...
			{
				yuv1[0][i] = yuv2[0][i];
				yuv1[1][(i>>2)] = yuv2[1][(i>>2)];
				yuv1[2][(i>>2)] = yuv2[2][(i>>2)];
			}
		}
	}
	else
