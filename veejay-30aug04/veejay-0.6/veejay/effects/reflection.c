...
void reflection_apply(uint8_t * yuv1[3], int width, int height, int index1,
		      int index2, int move)
{
    unsigned int normalx, normaly, x, y;
    unsigned int lightx, lighty, temp;
    uint8_t *row = yuv1[0] + width + 1;
    ...
    for (x = 0; x < width; x++) {
	reflection_buffer[x] = yuv1[0][x];
    }
	// FIXME: UV mapping not correct 
	...
	for (x = 0; x < width; x++) {
	    int i1 = (int) p;
	    int i2 = yuv1[0][x + 1 + (y * width)];	/* deviate */
