 ...
 * Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA  02111-1307, USA
 *
 * $Id: radialblur.c,v 1.1 2004/07/30 18:48:24 niels Exp $
 *
 * mplayer's boxblur
...
int	radialblur_malloc(int w, int h)
{
	...
	radial_src[1] = (uint8_t*) vj_malloc(sizeof(uint8_t) * ((w*h)/4));
	if(!radial_src[1]) return 0;
	radial_src[2] = (uint8_t*) vj_malloc(sizeof(uint8_t) * ((w*h)/4));
...
void radialblur_apply(uint8_t * yuv1[3], int width, int height, int radius, int power, int direction)
{
	if(radius == 0) return;
	// inplace
	veejay_memcpy( radial_src[0] , yuv1[0], (width*height));
	veejay_memcpy( radial_src[1] , yuv1[1], (width*height)/4); 
	veejay_memcpy( radial_src[2] , yuv1[2], (width*height)/4);	

	switch(direction)
	{
		case 0: rhblur_apply( yuv1[0], radial_src[0],width, height, radius, power );
			rhblur_apply( yuv1[1], radial_src[1],width/2, height/2, radius, power );
			rhblur_apply( yuv1[2], radial_src[2],width/2, height/2, radius, power );
			break;
		case 1: rvblur_apply( yuv1[0], radial_src[0],width, height, radius, power ); 
			rvblur_apply( yuv1[1], radial_src[1],width/2, height/2, radius, power );
			rvblur_apply( yuv1[2], radial_src[2],width/2, height/2, radius, power );
			break;
		case 2:
			rhblur_apply( yuv1[0], radial_src[0],width, height, radius, power );
			rhblur_apply( yuv1[1], radial_src[1],width/2, height/2, radius, power );
			rhblur_apply( yuv1[2], radial_src[2],width/2, height/2, radius, power );
			rvblur_apply( yuv1[0], radial_src[0],width, height, radius, power ); 
			rvblur_apply( yuv1[1], radial_src[1],width/2, height/2, radius, power );
			rvblur_apply( yuv1[2], radial_src[2],width/2, height/2, radius, power );
			break;
