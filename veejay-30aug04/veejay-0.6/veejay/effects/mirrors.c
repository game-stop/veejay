...
void _mirrors_v( uint8_t *yuv[3], int width, int height, int factor, int swap)
{
	...
	unsigned int nr = width / line_width;
	unsigned int i=0;
...
void mirrors_apply(uint8_t * yuv[3], int width, int height, int type,
		   int factor )
{
    switch (type) {
    case 0:
	_mirrors_v(yuv, width, height, factor, 0);
	break;
    case 1:
	_mirrors_v(yuv,width,height,factor,1);
	break;
    case 2:
	_mirrors_h(yuv,width,height,factor,0);
	break;
    case 3:
	_mirrors_h(yuv,width,height,factor,1);
	break;
	}
}
void mirrors_free(){}
