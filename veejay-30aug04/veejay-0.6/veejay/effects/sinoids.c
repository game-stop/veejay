...
{
	int i = 0;
   sinoids_X = (int*) malloc(sizeof(int) * width);
...
void sinoids_apply(uint8_t *yuv1[3], int width, int height, int m, int s) {
	...
	if(m==0) {
  	  for( r=width ; r < len-width; r+=width) {
             for( c = 0; c < width; c++) {
		yuv1[0][r+c] = yuv1[0][(r+c+sinoids_X[c])];
		yuv1[1][r+c] = yuv1[1][(r+c+sinoids_X[c])];
		yuv1[2][r+c] = yuv1[2][(r+c+sinoids_X[c])];
	     }
	  }
	}
        else {
		/* on copy */
	      for(r=0; r < len ;r++) {
		sinoid_frame[0][r] = yuv1[0][r];
		sinoid_frame[1][r] = yuv1[1][r];	
		sinoid_frame[2][r] = yuv1[2][r];
		}
	      for(r=width; r < len-width; r+= width) {
	        for(c=0; c < width; c++) {
	  	  yuv1[0][r+c] = sinoid_frame[0][(r+c+sinoids_X[c])];
		  yuv1[1][r+c] = sinoid_frame[1][(r+c+sinoids_X[c])];
		  yuv1[2][r+c] = sinoid_frame[2][(r+c+sinoids_X[c])];
	        }
	      }
	}
}
