...
void slice_apply(uint8_t *yuv[3], int width, int height, int val, int re_init) {
  unsigned int x,y,r,dx,dy,xshift[height], yshift[width];
  unsigned int len = (width*height);

  if(re_init==1) slice_recalc(width,height,val);

  veejay_memcpy( slice_frame[0], yuv[0], len);
  veejay_memcpy( slice_frame[1], yuv[1], len);
  veejay_memcpy( slice_frame[2], yuv[2], len);

  for(y=0; y < height; y++){ 
    for(x=0; x < width; x++) {
	dx = x + slice_xshift[y] ; dy = y + slice_yshift[x];
	if(dx < width && dy < height && dx >= 0 && dy >= 0) {
		yuv[0][(y*width)+x] = slice_frame[0][(dy*width)+dx];
		yuv[1][(y*width)+x] = slice_frame[1][(dy*width)+dx];
		yuv[2][(y*width)+x] = slice_frame[2][(dy*width)+dx];
	}
     }
  }
}
