...
void lumamask_apply(uint8_t * yuv1[3], uint8_t * yuv2[3], int width,
		   int height, int v_scale, int h_scale)
{
	...
	memcpy(buf[0], yuv1[0], width * height );
	memcpy(buf[1], yuv1[1], (width * height) );
	memcpy(buf[2], yuv1[2], (width * height) );
	...
	for(y=0; y < height; y++)
	{
		for(x=0; x < width ; x++)
		{
			...
			yuv1[0][y*width+x] = buf[0][ny * width + x];
			yuv1[1][y*width+x] = buf[1][ny * width + x];
			yuv1[2][y*width+x] = buf[2][ny * width + x];
		}
	}
}
void lumamask_free()
