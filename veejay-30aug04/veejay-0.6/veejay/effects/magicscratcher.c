...
{
    if (!m_reverse) {
	memcpy(mframe + (w * h * m_frame), yuv1[0], (w * h));
    } else {
	memcpy(yuv1[0], mframe + (w * h * m_frame), (w * h));
    }
    if (m_reverse)
	m_frame--;
    else
...
void magicscratcher_apply(uint8_t * yuv1[3],
			  ...
			  int no_reverse)
{
...
#else
	memset( yuv1[1], 128, (width*height)/4);
	memset( yuv1[2], 128, (width*height)/4);
#endif
    ...
    if (m_frame == 0) {
	int i=0;
	memcpy(mframe + (len * m_frame), yuv1[0], len);
	m_frame++;
    }

    for (x = 0; x < len; x++) {
	yuv1[0][x] = func_y( mframe[offset + x], yuv1[0][x]);
    }
    if (m_frame > 0) {
	memset(mframe + (len * (m_frame - 1)), 16, len);
    }

    store_mframe(yuv1, width, height, n, no_reverse);
