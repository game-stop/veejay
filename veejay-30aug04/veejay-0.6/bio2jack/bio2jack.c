...
#define OUTFILE stderr
    ...
    #define ERR(...) fprintf(OUTFILE, "\033[0:31mE:  %s:\033[0m", __FUNCTION__),\
         fprintf(OUTFILE, __VA_ARGS__),				\
         fflush(OUTFILE);
...
typedef struct jack_driver_s
{
  ...
  int              volume[MAX_OUTPUT_PORTS];      /* percentage of sample value to preserve, 100 would be no attenuation */
...
/* volume is a pointer to an array of integers with the volume for each channel */
static void volume_effect(void *buffer, int frames, int channels, int* volume)
{
...
static int JACK_callback (nframes_t nframes, void *arg)
{
  ...
  if(this->state == PLAYING)
  {
    ...
    volume_effect(this->sound_buffer, (nframes - jackFramesAvailable), this->num_output_channels, this->volume);
...
static int JACK_OpenDevice(jack_driver_t* this)
{
  ...
  snprintf(client_name, sizeof(client_name), "veejay_%d_%d",num++, getpid());
  TRACE("client name '%s'\n", client_name);
...
static int JACK_SetVolumeForChannelFromThis(jack_driver_t *this, unsigned int channel, unsigned int volume)
{
...
int JACK_SetAllVolume(int deviceID, int volume)
{
  jack_driver_t *this = getDriver(deviceID);
  int i;
